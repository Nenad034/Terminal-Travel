...
|                          | Stanje                                                                                   |
| :----------------------- | :--------------------------------------------------------------------------------------- |
| Testovi                  | 1071 (backend) + 13 (panel), svi prolaze                                                 |
...
- **Naziv agencije u sistemskim upitima AI asistenata** — 8 mesta i dalje zakucano. Nije zamena
  teksta: ti upiti imaju ogradu protiv prompt-injection-a, pa vrednost koja se menja kroz panel
  traži odluku o čišćenju. Vidi M1 spec §3.9c, odeljak „Poznat nedostatak".
- **Izmišljena kontakt polja na listi rezervacija** (`buyerEmail`, `buyerPhone`, `branch`,
  `assignedUser`) — izgledaju kao pravi podaci, a nisu. Čekaju da API vrati prava polja. Dok. 42.
- **31 ranjivost u bibliotekama (9 ozbiljnih)** + CORS, ograničenje prijave, Row-Level Security —
  svesno odloženo do servera (dok. 39, poglavlje 6).
