...
describe('ArticleSourcesService (M23 spec §4b/§9)', () => {
  ...
  it('approve odbija ako actorId pripada AI_AGENT nalogu', async () => {
    ...
    await expect(service.approve('a1', 's1', 'ai-user-1')).rejects.toBeInstanceOf(
      ForbiddenException,
    );
  ...
  it('approve postavlja status=APPROVED i approved_by/approved_at za ljudski nalog', async () => {
    ...
    prisma.articleSource.findUnique.mockResolvedValue({
      id: 's1',
      articleId: 'a1',
      status: 'CANDIDATE',
    });
    prisma.articleSource.update.mockImplementation(({ data }: any) => ({
      id: 's1',
      articleId: 'a1',
      ...data,
    }));
