...
describe('BookingsService (M5 spec §4/§6.4)', () => {
  function makeService() {
    const prisma: any = {
      ...
      product: {
        ...
        findUnique: jest.fn(),
        create: jest.fn().mockResolvedValue({ id: 'p-manual' }),
      },
      ...
      supplier: {
        findUnique: jest.fn().mockResolvedValue({ id: 'sup-1', name: 'Kombi prevoz doo' }),
      },
      markupRule: {
        findUnique: jest.fn().mockResolvedValue({ id: 'mr-1', percentage: 0, fixedAmount: 0 }),
      },
      bookingItemGuest: {
        findMany: jest.fn(),
        findFirst: jest.fn(),
    ...
    const supplierManifests = {
      supersedeIfOnSentManifest: jest.fn(),
      prepareForBooking: jest.fn(),
    };
  ...
  describe('confirmQuote — sve ili ništa (§4, korak 3)', () => {
    it('oslobađa već rezervisanu prvu stavku kad druga stavka ne uspe, ne kreira Booking', async () => {
      ...
      prisma.product.findUniqueOrThrow.mockResolvedValue({
        sourceContract: { defaultTipNastupanja: 'POSREDNIK' },
      });

      prisma.rateLine.findUniqueOrThrow.mockImplementation(({ where }: any) => {
        if (where.id === 'rl1')
          return Promise.resolve({ contractPeriodId: 'period-1', contractPeriod: {} });
      ...
      contractPeriods.reserve.mockImplementation((periodId: string) => {
        if (periodId === 'period-1')
          return Promise.resolve({ reserved: true, unitsSold: 1, remaining: 0 });
        return Promise.reject(new BadRequestException('Nema dovoljno preostalog kapaciteta'));
      });

      await expect(
        service.confirmQuote('quote-1', {} as any, { userId: 'actor-1' }),
    ...
    it('odbija potvrdu kad Quote.client_account_id nije popunjen', async () => {
      ...
      await expect(
        service.confirmQuote('quote-2', {} as any, { userId: 'actor-1' }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('korak 1a — poziva M11 proveru garancije putovanja i blokira ORGANIZATOR potvrdu kad M11 vrati allowed=false', async () => {
      ...
      compliance.checkTravelGuaranteeUtilization.mockResolvedValue({
        allowed: false,
        reason: 'test-blokada-garancije',
      });
      prisma.quote.findUnique.mockResolvedValue({
        id: 'quote-3',
        status: 'DRAFT',
      ...
      prisma.product.findUniqueOrThrow.mockResolvedValue({
        sourceContract: { defaultTipNastupanja: 'ORGANIZATOR' },
      });

      await expect(
        service.confirmQuote('quote-3', {} as any, { userId: 'actor-1' }),
      ).rejects.toThrow('test-blokada-garancije');
      expect(compliance.checkTravelGuaranteeUtilization).toHaveBeenCalledWith({
        bookingTotalPrice: 10000,
        currency: 'EUR',
      });
      expect(prisma.booking.create).not.toHaveBeenCalled();
    });
    ...
    it('korak 1a — ne poziva M11 proveru garancije za POSREDNIK rezervacije (M11 spec §2.2)', async () => {
      ...
      prisma.rateLine.findUniqueOrThrow.mockResolvedValue({
    ...
    it('rezerviše broj jedinica iz QuoteItem.unitCount, ne pretpostavlja uvek 1 (§4.2 dopuna v1.14)', async () => {
      ...
      prisma.booking.findUnique
        ...
        .mockResolvedValue({ id: 'booking-multi', items: [] });
    ...
    it('gost NE može da potvrdi tuđu Ponudu — 404 (§6.2 dopuna)', async () => {
      ...
      await expect(
        service.confirmQuote('quote-tudj', {} as any, { userId: 'guest-1' }),
  ...
  describe('cancel — provera duplikata pre otkazivanja (§6.4)', () => {
    it('vraća upozorenje bez otkazivanja kad postoji podudaranje po imenu i preklapajućim datumima', async () => {
      ...
      prisma.bookingItemGuest.findMany.mockResolvedValue([
        { guestFirstName: 'Petar', guestLastName: 'Petrović' },
      ]);
  ...
  describe('cancel — oslobađanje kapaciteta i refund% (§6, §4.2 dopuna v1.14)', () => {
    it('oslobađa TAČAN broj rezervisanih jedinica (unit_count), ne uvek 1 — regresioni test za bivši bug', async () => {
      ...
      prisma.rateLine.findUnique.mockResolvedValue({
        contractPeriodId: 'period-multi',
        contractPeriod: { cancellationRules: [] },
      });
    ...
    it('računa refund% za API stavku iz cancellation_policy_snapshot, deterministički (§4.2 dopuna v1.14)', async () => {
      ...
      expect(prisma.bookingItem.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ cancellationRefundPercentage: 50 }),
        }),
      );
    });
  });
  ...
  describe('findOne — kontekst i vlasništvo (§6.2 dopuna, priprema za M8)', () => {
    const bookingItem = {
      ...
      guests: [
        { id: 'g1', guestFirstName: 'Marko', guestLastName: 'Marković', guestProfileId: 'gp1' },
      ],
    };
  ...
  describe('findAll — ownership se nameće za gost/B2B kontekst (§6.2 dopuna)', () => {
    it('gost ne može da vidi tuđe rezervacije slanjem tuđeg clientAccountId parametra — parametar se ignoriše', async () => {
      ...
      expect(prisma.booking.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ clientAccountId: 'client-1' }),
        }),
      );
    });
  ...
  describe('findAll — productName (6.9.2026 dopuna, isti obrazac kao calendar-summary)', () => {
    it('filtrira preko ProductTranslation.name (svi jezici, insensitive)', async () => {
      ...
      expect(where.items.some.product.translations).toEqual({
        some: { name: { contains: 'Sunce', mode: 'insensitive' } },
      });
    });
  });
  ...
  describe('findAll/findOne — VIEW_ALL vidljivost (§6.6, 31.8.2026)', () => {
      ...
      expect(prisma.booking.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({
            OR: [{ ownerId: 'staff-1' }, { assignedToId: 'staff-1' }],
          }),
        }),
      );
    });
    ...
    it('findOne — predstavnik (VODIC) VIDI rezervaciju na kojoj mu je stavka dodeljena', async () => {
      ...
      expect(prisma.bookingItem.count).toHaveBeenCalledWith({
        where: { bookingId: 'b1', assignedGuideId: 'vodic-1' },
      });
    });
    ...
    it('findAll — franšizni STAFF nalog dobija dodatni filter na sopstveni franchise_subagent_id, čak i sa VIEW_ALL=true', async () => {
      ...
      prisma.subagent.findUnique.mockResolvedValue({
        id: 'subagent-fr-1',
        privilegeLevel: 'FRANCHISE',
      });
      prisma.booking.findMany.mockResolvedValue([]);
      permissions.hasPermission.mockResolvedValue(true);
      ...
      expect(prisma.booking.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ franchiseSubagentId: 'subagent-fr-1' }),
        }),
      );
    });
  });
  ...
  describe('cancel/modify/updatePaymentStatus/voucherOverride/assignGuide — IDOR ispravka (31.8.2026)', () => {
    it('gost NE može da otkaže tuđu rezervaciju — vraća 404', async () => {
      ...
      await expect(service.cancel('b1', {}, { userId: 'guest-1' })).rejects.toThrow(
        NotFoundException,
      );
    ...
    it('gost NE može da izmeni stavku tuđe rezervacije — vraća 404', async () => {
      ...
      await expect(
        service.modify('b1', { bookingItemId: 'item-1' } as any, { userId: 'guest-1' }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('gost NE može da promeni status plaćanja tuđe rezervacije — vraća 404', async () => {
      ...
      await expect(
        service.updatePaymentStatus('b1', 'PAID' as any, { userId: 'guest-1' }),
      ).rejects.toThrow(NotFoundException);
      expect(prisma.booking.update).not.toHaveBeenCalled();
    });
    ...
    it('gost NE može da izdejstvuje voucher override na tuđoj rezervaciji — vraća 404', async () => {
      ...
      await expect(service.voucherOverride('b1', 'razlog', { userId: 'guest-1' })).rejects.toThrow(
        NotFoundException,
      );
      expect(prisma.booking.update).not.toHaveBeenCalled();
    });
    ...
    it('sužen STAFF (bez VIEW_ALL, nije vlasnik ni zadužen) NE može da dodeli vodiča na stavci tuđe rezervacije — vraća 404', async () => {
      ...
      await expect(service.assignGuide('item-1', 'guide-1', { userId: 'staff-1' })).rejects.toThrow(
        NotFoundException,
      );
      expect(prisma.bookingItem.update).not.toHaveBeenCalled();
    });
  ...
  describe('getVoucherContent (§6, dopuna 2.9.2026)', () => {
    ...
    it('vraća maskiran sadržaj sa kontaktom predstavnika, bez supplier polja', async () => {
      ...
      prisma.user.findMany.mockResolvedValue([
        { id: 'guide-1', fullName: 'Ana Vodić', phone: '+381601234567', email: 'ana@tt.rs' },
      ]);
      ...
      expect(result.groups[0].items[0].representative).toEqual({
        fullName: 'Ana Vodić',
        phone: '+381601234567',
        email: 'ana@tt.rs',
      });
        ...
        items: [
          {
            ...
            product: {
              ...
              sourceProvider: null,
              supplierId: null,
              sourceContract: { supplierId: 'sup-A' },
              translations: [{ languageCode: 'sr', name: 'Hotel A' }],
            },
          },
          {
            ...base,
            ...
            product: {
              ...
              supplierId: null,
              sourceContract: { supplierId: 'sup-A' },
              translations: [{ languageCode: 'sr', name: 'Izlet A' }],
            },
          },
          {
            ...base,
            ...
            product: {
              ...
              supplierId: 'sup-B',
              sourceContract: null,
              translations: [{ languageCode: 'sr', name: 'Transfer B' }],
            },
          },
        ],
      });
    ...
    it('groupIndex vraća samo taj vaučer, itemId samo tu uslugu', async () => {
      ...
      await expect(service.getVoucherContent('b1', { groupIndex: 9 })).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('modify — izmena usluge + previewModify (§6, dopuna 2.9.2026)', () => {
    ...
    it('odbija izmenu kad nova usluga nije istog tipa proizvoda kao postojeća stavka', async () => {
      ...
      prisma.product.findUnique.mockImplementation(({ where }: any) =>
        Promise.resolve(
          where.id === 'p-old'
            ? { id: 'p-old', type: 'ACCOMMODATION' }
            : { id: 'p-new', type: 'TRANSFER' },
        ),
      );
    ...
    it('modify prosleđuje novi productId builder-u kad je dat (izmena usluge, isti tip)', async () => {
      ...
      expect(prisma.bookingItem.create).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ productId: 'p-new' }) }),
      );
    });
  ...
  describe('addItem/previewAddItem (§6.7, 3.9.2026)', () => {
    ...
    it('dodaje stavku, preračunava ukupno i pokreće NOVU najavu za CONTRACTED (odluka vlasnika)', async () => {
      const { service, prisma, contractPeriods, builder, supplierManifests, eventBus } =
        makeService();
      prisma.booking.findUnique.mockResolvedValue({
        id: 'b1',
        clientAccountId: 'c1',
      ...
      expect(prisma.bookingItem.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ productId: 'p-transfer', announcedAt: null }),
        }),
      );
      expect(prisma.booking.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ status: 'MODIFIED', totalPrice: 14000 }),
        }),
      );
    ...
    it('API stavka je najavljena istog trenutka i ne pokreće pripremu najave (§8.6)', async () => {
      ...
      integrations.confirmBooking.mockResolvedValue({
        status: 'CONFIRMED',
        providerBookingReference: 'REF-1',
      });
      prisma.bookingItem.create.mockResolvedValue({ id: 'item-new' });
    ...
    it('odbija dodavanje na otkazanu rezervaciju i PACKAGE proizvod', async () => {
      ...
      await expect(service.addItem('b1', dto, { userId: 'staff-1' })).rejects.toThrow(
        BadRequestException,
      );
    ...
    it('previewAddItem vraća cenu i novo ukupno, bez ijedne rezervacije/upisa', async () => {
      ...
      expect(result).toEqual({
        newPrice: 4500,
        newCurrency: 'EUR',
        bookingTotalBefore: 10000,
        bookingTotalAfter: 14500,
      });
      expect(contractPeriods.reserve).not.toHaveBeenCalled();
      expect(prisma.bookingItem.create).not.toHaveBeenCalled();
      expect(prisma.booking.update).not.toHaveBeenCalled();
  ...
  describe('doplate i popusti kao vezane stavke (§6.7a, 3.9.2026)', () => {
    ...
    it('popust ulazi sa minusom i BEZ marže (prolazi gostu 1:1)', async () => {
      ...
      prisma.ancillaryService.findUnique.mockResolvedValue(
        ancillary({ kind: 'DISCOUNT', priceBasis: 'PER_ROOM_PER_STAY', flatAmount: 2000 }),
      );
      prisma.bookingItem.create.mockResolvedValue({ id: 'anc-item' });
      prisma.booking.update.mockResolvedValue({ id: 'b1', items: [] });
    ...
    it('ON_SITE doplata dobija cenu bez marže i NE ulazi u ukupno zaduženje', async () => {
      ...
      prisma.ancillaryService.findUnique.mockResolvedValue(
        ancillary({ payable: 'ON_SITE', priceBasis: 'PER_ROOM_PER_STAY', flatAmount: 1000 }),
      );
      prisma.bookingItem.create.mockResolvedValue({ id: 'anc-item' });
      prisma.booking.update.mockResolvedValue({ id: 'b1', items: [] });
    ...
    it('odbija doplatu čiji uslov po sastavu gostiju nije ispunjen i količinu preko max_quantity', async () => {
      ...
      await expect(
        service.addAncillaryToItem('b1', 'item-1', { ancillaryServiceId: 'anc-1' } as any, {
          userId: 'staff-1',
        }),
      ).rejects.toThrow(BadRequestException);
      ...
      await expect(
        service.addAncillaryToItem(
          'b1',
          'item-1',
          { ancillaryServiceId: 'anc-1', quantity: 3 } as any,
          { userId: 'staff-1' },
        ),
      ).rejects.toThrow(BadRequestException);
      expect(prisma.bookingItem.create).not.toHaveBeenCalled();
    ...
    it('otkazivanje matične stavke otkazuje i vezanu doplatu, i kad nije navedena u itemIds', async () => {
      ...
      await service.cancel(
        'b1',
        { reason: 'test', itemIds: ['item-1'], confirmDuplicateOverride: true } as any,
        { userId: 'staff-1' },
      );
  ...
  describe('addManualItem (§6.7b, 3.9.2026)', () => {
    ...
    it('odbija izlaznu cenu manju od nabavne (zamenjena polja) i nepostojećeg dobavljača', async () => {
      ...
      bookingOk(prisma);
      await expect(
        service.addManualItem('b1', { ...dto, finalPrice: 100 }, { userId: 'staff-1' }),
      ).rejects.toThrow(BadRequestException);

      prisma.supplier.findUnique.mockResolvedValue(null);
      await expect(service.addManualItem('b1', dto, { userId: 'staff-1' })).rejects.toThrow(
        NotFoundException,
      );
  ...
  describe('addGuest/updateGuest/removeGuest (§4.3, dopuna 2.9.2026)', () => {
    it('addGuest dodaje putnika na stavku', async () => {
      ...
      prisma.bookingItemGuest.create.mockResolvedValue({
        id: 'g1',
        bookingItemId: 'item-1',
        guestFirstName: 'Ana',
        guestLastName: 'Anić',
      ...
      const result = await service.addGuest(
        'item-1',
    ...
    it('updateGuest menja ime/prezime, odbija putnika koji ne pripada stavci', async () => {
      ...
      prisma.bookingItem.findUnique.mockResolvedValue({ id: 'item-1', bookingId: 'b1' });
      prisma.booking.findUniqueOrThrow.mockResolvedValue({ id: 'b1', clientAccountId: 'c1' });
      prisma.bookingItemGuest.findUnique.mockResolvedValue({
        id: 'g1',
        bookingItemId: 'item-drugi',
        guestFirstName: 'Ana',
        guestLastName: 'Anić',
      });
    ...
    it('removeGuest briše putnika i piše audit trag', async () => {
      ...
      expect(auditLog.write).toHaveBeenCalledWith(
        expect.objectContaining({ action: 'booking_item_guest.removed', resourceId: 'g1' }),
      );
    });
  ...
  describe('transferOwnership (§6.5, 31.8.2026)', () => {
    ...
    it('korisnik koji NIJE vlasnik ni Vlasnik/Direktor ne sme da prenese vlasništvo', async () => {
      ...
      await expect(
        service.transferOwnership('b1', 'novi-vlasnik', { userId: 'neko-drugi' }),
  ...
  describe('predaja zaduženja — propose/accept/decline/cancel (§6.5, 31.8.2026)', () => {
    it('proposeHandoff od strane Vlasnika/Direktora izvršava odmah (ACCEPTED), menja assigned_to_id bez čekanja', async () => {
      ...
      prisma.bookingHandoffRequest.create.mockResolvedValue({
        id: 'h1',
        bookingId: 'b1',
        status: 'ACCEPTED',
      });
      ...
      expect(prisma.booking.update).toHaveBeenCalledWith({
        where: { id: 'b1' },
        data: { assignedToId: 'novi-zaduzeni' },
      });
    });
    ...
    it('proposeHandoff odbija novi predlog kad već postoji PENDING za istu rezervaciju (ne-bypass put)', async () => {
      ...
      prisma.userRole.findFirst.mockResolvedValue(null);
      prisma.bookingHandoffRequest.findFirst.mockResolvedValue({
        id: 'postojeci',
        status: 'PENDING',
      });

      await expect(
        service.proposeHandoff('b1', 'novi-cilj', { userId: 'agent-1' }),
      ).rejects.toThrow(BadRequestException);
    ...
    it('acceptHandoff — samo primalac sme da prihvati, tada se assigned_to_id menja', async () => {
      const { service, prisma } = makeService();
      prisma.bookingHandoffRequest.findUnique.mockResolvedValue({
        id: 'h1',
        bookingId: 'b1',
        fromUserId: 'a',
        toUserId: 'b',
    ...
    it('acceptHandoff — neko ko nije primalac ne sme da prihvati', async () => {
      ...
      await expect(service.acceptHandoff('h1', { userId: 'trece-lice' })).rejects.toThrow(
        ForbiddenException,
      );
      expect(prisma.booking.update).not.toHaveBeenCalled();
    });
    ...
    it('cancelHandoff — treće lice (ni predlagač ni Vlasnik/Direktor) ne sme da otkaže', async () => {
      ...
      await expect(service.cancelHandoff('h1', { userId: 'trece-lice' })).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('listHandoffRequests vraća listu za vidljivu rezervaciju, najnoviji prvi', async () => {
      ...
      expect(prisma.bookingHandoffRequest.findMany).toHaveBeenCalledWith({
        where: { bookingId: 'b1' },
    ...
    it('accept/decline/cancel odbijaju predlog koji više nije PENDING', async () => {
      ...
      await expect(service.acceptHandoff('h1', { userId: 'b' })).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('calendarSummary/calendarDay — spajanje dolazak/odlazak filtera sa prozorom (5.9.2026)', () => {
    ...
    it('productName filtrira preko ProductTranslation.name (svi jezici, insensitive)', async () => {
      ...
      const where = prisma.bookingItem.findMany.mock.calls[0][0].where;
      expect(where.product.translations).toEqual({
        some: { name: { contains: 'Sunce', mode: 'insensitive' } },
      });
    });
  });
