 ...
 * su ostvareni kroz direktan DI poziv, ne HTTP. M7 delegira na SubagentStubService (isti folder)
...
export class ComplianceStubsService {
  ...
  async checkCreditLimitIfSubagent(params: {
    ...
    return this.subagentStub.checkCreditLimitIfSubagent(params);
  }
  ...
  async isActiveSubagentWithinCreditLimit(clientAccountId: string): Promise<boolean> {
    return this.subagentStub.isActiveSubagentWithinCreditLimit(clientAccountId);
  }
}
