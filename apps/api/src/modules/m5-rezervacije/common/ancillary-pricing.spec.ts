...
describe('computeAncillaryAmount — osnova je PAR (osoba/soba × dan/period)', () => {
  it('po osobi i danu množi i osobama i noćima', () => {
    expect(computeAncillaryAmount(svc({ priceBasis: 'PER_PERSON_PER_NIGHT' }), ctx)).toBe(
      1000 * 3 * 7,
    );
  });
  ...
  it('po sobi i danu množi sobama i noćima, ne osobama', () => {
    expect(
      computeAncillaryAmount(svc({ priceBasis: 'PER_ROOM_PER_NIGHT' }), { ...ctx, rooms: 2 }),
    ).toBe(1000 * 2 * 7);
  });

  it('po sobi i periodu je jednom po sobi', () => {
    expect(
      computeAncillaryAmount(svc({ priceBasis: 'PER_ROOM_PER_STAY' }), { ...ctx, rooms: 2 }),
    ).toBe(1000 * 2);
  });
  ...
  it('količina (ljubimci, dodatni ležajevi) množi iznos', () => {
    expect(
      computeAncillaryAmount(svc({ priceBasis: 'PER_PET_PER_NIGHT' }), { ...ctx, quantity: 2 }),
    ).toBe(1000 * 7 * 2);
  });
});
...
describe('signedAncillaryAmount — znak nosi kind, iznos ostaje pozitivan', () => {
  ...
  it('popust ulazi sa minusom, iako je iznos upisan kao pozitivan', () => {
    expect(
      signedAncillaryAmount(svc({ kind: 'DISCOUNT', priceBasis: 'PER_ROOM_PER_STAY' }), ctx),
    ).toBe(-1000);
  });
});
...
describe('checkAncillaryOccupancy — samo za osnovu po sobi', () => {
  it('cena po osobi se ne proverava prema granicama sobe', () => {
    expect(
      checkAncillaryOccupancy(svc({ coversPersons: 1, priceBasis: 'PER_PERSON_PER_NIGHT' }), {
        adults: 4,
        children: 0,
      }),
    ).toBeNull();
  ...
  it('odbija sastav koji prelazi ukupan broj osoba', () => {
    expect(
      checkAncillaryOccupancy(svc({ priceBasis: 'PER_ROOM_PER_STAY', coversPersons: 2 }), {
        adults: 2,
        children: 1,
      }),
    ).toContain('najviše 2 osoba');
  });
  ...
  it('odbija dete starije od granice za OVU stavku', () => {
    ...
    expect(checkAncillaryOccupancy(s, { adults: 2, children: 1, childrenAges: [9] })).toContain(
      'do 6.99 godina',
    );
  ...
  it('bez poznatih uzrasta dece granica uzrasta ne obara proveru (nema šta da proveri)', () => {
    expect(
      checkAncillaryOccupancy(svc({ priceBasis: 'PER_ROOM_PER_STAY', childMaxAge: 6.99 }), {
        adults: 2,
        children: 1,
      }),
    ).toBeNull();
