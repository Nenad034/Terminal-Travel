...
const NO_MATCH: CorrespondentMatch = {
  ...
  async match(fromAddress: string): Promise<CorrespondentMatch> {
    ...
    if (guestProfile) {
      return {
        ...
        correspondentClientAccountId: guestProfile.linkedClientAccountId ?? null,
        correspondentSupplierId: null,
      };
    }
    ...
    const supplierByContactEmail = await this.prisma.supplier.findFirst({
    ...
    if (supplierByContactEmail) {
      return {
        ...
        correspondentClientAccountId: null,
        correspondentSupplierId: supplierByContactEmail.id,
      };
    }

    const supplierContact = await this.prisma.supplierContact.findFirst({
      where: { email: { equals: address, mode: 'insensitive' } },
    });
    if (supplierContact) {
      return {
        correspondentType: 'SUPPLIER',
        correspondentClientAccountId: null,
        correspondentSupplierId: supplierContact.supplierId,
      };
    }
