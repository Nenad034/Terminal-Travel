...
describe('SupplierConversationsService', () => {
  function makeService() {
    ...
    const service = new SupplierConversationsService(
      prisma as any,
      auditLog as any,
      permissions as any,
      auth as any,
  ...
  describe('grantAccess (M19 spec §9.4)', () => {
    it('zahteva M19/supplier-conversation/GRANT_ACCESS', async () => {
      ...
      await expect(service.grantAccess('c1', { userId: 'staff-2' }, 'staff-1')).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('odbija ciljanje EXTERNAL_SUPPLIER pristupa nekom ko nije STAFF', async () => {
      ...
      await expect(service.grantAccess('c1', { userId: 'contact-1' }, 'staff-1')).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('upisuje i SupplierConversationAccess i ConversationParticipant u istoj transakciji', async () => {
      ...
      prisma.supplierConversationAccess.upsert.mockResolvedValue({
        id: 'acc-1',
        conversationId: 'c1',
        userId: 'staff-2',
      });
  ...
  describe('inviteContact (M19 spec §9.2 koraci 1-3)', () => {
    ...
    it('kreira User(account_type=SUPPLIER_CONTACT), token, i popunjava linked_user_id', async () => {
      ...
      const result = await service.inviteContact(
        'c1',
    ...
    it('odbija ako kontakt već ima portal nalog', async () => {
      ...
      await expect(
        service.inviteContact('c1', { supplierContactId: 'contact-1' }, 'staff-1'),
    ...
    it('odbija ako razgovor već ima dodeljenu SUPPLIER_CONTACT osobu (§9.3 — tačno jedan)', async () => {
      ...
      withConversationAndContact(prisma);
      prisma.conversationParticipant.findMany.mockResolvedValue([
        { conversationId: 'c1', userId: 'existing-contact' },
      ]);
      prisma.user.findMany.mockResolvedValue([
        { id: 'existing-contact', accountType: 'SUPPLIER_CONTACT' },
      ]);
