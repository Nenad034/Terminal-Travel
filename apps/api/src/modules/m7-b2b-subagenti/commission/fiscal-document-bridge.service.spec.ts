...
describe('FiscalDocumentBridgeService (M7 → M10, M10 spec §5.1a)', () => {
  function makeService() {
    ...
    const service = new FiscalDocumentBridgeService(
      prisma,
  ...
  it('poziva prepareCreditNoteDraft sa amount/currency/subagentId/rebateId i stvarnim nazivom firme iz M6', async () => {
    ...
    clientAccounts.findOne.mockResolvedValue({
      id: 'acc-1',
      companyName: 'M7 Test Turagencija',
      fullName: null,
    });
  ...
  it('koristi fullName kao fallback kad companyName nije postavljen', async () => {
    ...
    await service.prepareCreditNoteDraftForRebate({
      id: 'rebate-1',
      subagentId: 'sub-1',
    ...
    expect(fiscalDocuments.prepareCreditNoteDraft).toHaveBeenCalledWith(
      expect.objectContaining({ buyerNameSnapshot: 'Petar Petrović' }),
    );
  });
  ...
  it('koristi prazan string kad M6 ClientAccount ne postoji/upit ne uspe', async () => {
    ...
    expect(fiscalDocuments.prepareCreditNoteDraft).toHaveBeenCalledWith(
      expect.objectContaining({ buyerNameSnapshot: '' }),
    );
  });
});
