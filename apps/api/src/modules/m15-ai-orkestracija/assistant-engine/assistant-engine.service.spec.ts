...
describe('AssistantEngineService', () => {
    ...
    const candidates: AssistantCandidate[] = [
      {
        ...
        body: 'Idi na M5 ekran rezervacija i klikni otkaži.',
      },
    ];

    const result = await engine.resolveAnswer({
      question: 'Kako rezervacija radi otkazivanje u sistemu?',
      candidates,
      ...baseParams,
    });
  ...
  it('kandidat ispod praga preklapanja ključnih reči se odbacuje (NONE)', async () => {
    ...
    anthropic.isConfigured.mockReturnValue(false);
    const candidates: AssistantCandidate[] = [
      { articleId: 'a1', translationId: 't1', title: 'Nesrodno', body: 'potpuno drugačija tema' },
    ];

    const result = await engine.resolveAnswer({
      question: 'Kako otkazujem rezervaciju',
      candidates,
      ...baseParams,
    });
  ...
  it('embedding rangiranje: isPriority kandidat zadržava prioritet iako ga rangiranje ne vrati', async () => {
    ...
    geminiEmbedding.embed.mockImplementation((texts: string[]) =>
    ...
    const candidates: AssistantCandidate[] = [
      {
        ...
        body: 'Otkazivanje rezervacije u M5.',
      },
    ];

    const result = await engine.resolveAnswer({
      question: 'Otkazivanje rezervacije',
      candidates,
      ...baseParams,
    });
    ...
    const candidates: AssistantCandidate[] = [
      {
        ...
        translationId: 't1',
        title: 'Otkazivanje rezervacije',
        body: 'Koraci za otkazivanje rezervacije u M5.',
        isPriority: true,
      },
    ];
  ...
  it('Anthropic poziv koji baci grešku pada nazad na heuristiku (LOW), ne obara odgovor', async () => {
    const { engine, anthropic } = makeEngine();
    anthropic.isConfigured.mockReturnValue(true);
    anthropic.getClient.mockReturnValue({
      messages: { create: jest.fn().mockRejectedValue(new Error('mreža nedostupna')) },
    });
    const candidates: AssistantCandidate[] = [
      {
        articleId: 'a1',
