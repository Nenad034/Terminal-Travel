...
describe('M10AlarmsService (M10 spec §6.2/§8.2)', () => {
  ...
  it('emituje fiscal_document_draft_stale za svaki DRAFT stariji od 24h', async () => {
    ...
    expect(eventBus.emit).toHaveBeenCalledWith('M10', 'fiscal_document_draft_stale', {
  ...
  it('emituje supplier_obligation_due_soon za svaku obavezu pred rokom', async () => {
    ...
    supplierObligations.findDueSoon.mockResolvedValue([
      { id: 'so-1', dueDate: new Date('2026-08-20') },
    ]);

    await service.checkSupplierObligationsDueSoon();

    expect(eventBus.emit).toHaveBeenCalledWith('M10', 'supplier_obligation_due_soon', {
      supplierObligationId: 'so-1',
