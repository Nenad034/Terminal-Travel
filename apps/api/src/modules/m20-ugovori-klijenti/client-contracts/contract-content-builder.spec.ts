...
describe('contract-content-builder (M20 spec §2.2/§2.3)', () => {
  ...
  function makeItem(
    overrides: Partial<BookingForContract['items'][0]> = {},
  ): BookingForContract['items'][0] {
    return {
      id: 'item-1',
      ...
      rateLine: {
        ...
        contractPeriod: { cancellationRules: [{ daysBeforeStay: 30, refundPercentage: 100 }] },
      },
      ...overrides,
    };
  }
  ...
  describe('buildContentSnapshot (§2.3)', () => {
    it('izostavlja itinerary kad nema PACKAGE/EXCURSION stavki (čist ACCOMMODATION)', () => {
      ...
      const snapshot = buildContentSnapshot({
        ...
        travelGuarantee: null,
        paymentSchedule: null,
        agency,
      });
      expect(snapshot.itinerary).toBeNull();
      expect((snapshot.accommodation as any[])[0]).toMatchObject({
        productName: 'Hotel Test',
        stars: 4,
        boardType: 'HALF_BOARD',
      });
    });
    ...
    it('koristi cancellationPolicySnapshot za API stavke bez rateLine', () => {
      const booking: BookingForContract = {
        ...
        items: [
          makeItem({
            rateLine: null,
            cancellationPolicySnapshot: { tiers: [{ daysBeforeStay: 10, refundPercentage: 50 }] },
          }),
        ],
      };
      const snapshot = buildContentSnapshot({
      ...
      expect((snapshot.cancellationTerms as any[])[0].rules).toEqual({
        tiers: [{ daysBeforeStay: 10, refundPercentage: 50 }],
      });
    });
  });
});
