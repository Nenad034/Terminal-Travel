      ...
      >
        <div className="flex items-center justify-between border-b border-border bg-panel2 px-4 py-2.5">
          <div className="flex items-center gap-2 text-sm font-semibold text-ink">
            ...
            Tok rezervacije{' '}
            {mockEntries && (
              <span className="rounded bg-warn-bg px-1.5 py-0.5 text-[11px] font-normal text-warn">
                MOCK
              </span>
            )}
          </div>
        ...
        <div className="max-h-[65vh] overflow-y-auto px-4 py-3">
          ...
          {!error && entries && entries.length === 0 && (
            <p className="text-sm text-ink-faint">Nema zabeleženih izmena za ovu rezervaciju.</p>
          )}
          {!error && entries && entries.length > 0 && (
              ...
              {entries.map((e, i) => (
                <li key={i} className="mb-4 last:mb-0">
                  ...
                  <div className="text-xs text-ink-faint">
                    {new Date(e.timestamp).toLocaleDateString('sr-RS')}{' '}
                    {new Date(e.timestamp).toLocaleTimeString('sr-RS', {
                      hour: '2-digit',
                      minute: '2-digit',
                    })}
                  </div>
                  ...
                  <div className="text-xs text-ink-dim">
                    {e.actorType === 'AI_AGENT' ? (
                      <Icon name="sparkle" className="mr-1 text-accent" />
                    ) : null}
                    {e.actorName}
                  </div>
                </li>
