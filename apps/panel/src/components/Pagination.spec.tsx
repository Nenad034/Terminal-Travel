...
describe('Pagination', () => {
  it('prikazuje "nema zapisa" kad je total 0, bez strelica', () => {
    render(<Pagination page={1} pageCount={1} total={0} shown={0} limit={10} basePath="/rezervacije/lista" searchParams={{}} />);
  ...
  it('računa ispravan raspon na poslednjoj, nepunoj strani (25 zapisa, stranica od 10 → treća strana 21–25)', () => {
    render(<Pagination page={3} pageCount={3} total={25} shown={5} limit={10} basePath="/rezervacije/lista" searchParams={{}} />);
    ...
    expect(screen.getByText((_, node) => node?.textContent === 'prikazano 21–25 od 25 zapisa')).toBeInTheDocument();
  });

  it('strelica "prethodna" je onemogućena na prvoj strani, "sledeća" je aktivan link', () => {
    render(<Pagination page={1} pageCount={3} total={25} shown={10} limit={10} basePath="/rezervacije/lista" searchParams={{ status: 'CONFIRMED' }} />);
  ...
  it('čuva ostale query parametre i ne dodaje page=1 za prvu stranu (link ostaje čist)', () => {
    render(<Pagination page={2} pageCount={3} total={25} shown={10} limit={10} basePath="/rezervacije/lista" searchParams={{ status: 'CONFIRMED' }} />);
