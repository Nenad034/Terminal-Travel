...
// Event Bus-a).
export default function RebatesPanel({
  subagentId,
  rebates,
  canApprove,
}: {
      ...
      {rebates.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {rebates.map((r) => (
            <div
              key={r.id}
              className="flex items-center justify-between rounded bg-panel2 px-3 py-2 text-xs"
            >
              <div className="text-ink-dim">
                <p>
                  ...
                  za period {new Date(r.periodStart).toLocaleDateString('sr-RS')} –{' '}
                  {new Date(r.periodEnd).toLocaleDateString('sr-RS')}
                </p>
                <p className="mt-0.5 text-ink-faint">
                  kreiran {new Date(r.createdAt).toLocaleDateString('sr-RS')}
                  {r.approvedAt
                    ? ` · odobren ${new Date(r.approvedAt).toLocaleDateString('sr-RS')}`
                    : ''}
                  {r.appliedAt
                    ? ` · primenjen (M10 knjižno odobrenje poslato) ${new Date(r.appliedAt).toLocaleDateString('sr-RS')}`
                    : ''}
                </p>
              </div>
              <div className="flex items-center gap-2">
                ...
                {r.status === 'DRAFT' && canApprove && (
                  <RebateActions subagentId={subagentId} rebateId={r.id} />
                )}
              </div>
            </div>
          ))}
