...
const SEGMENTS: {
  value: 'STAFF' | 'SUBAGENT' | 'BUSINESS_CLIENT' | 'PUBLIC_GUEST';
  segment: 'staff' | 'subagent' | 'business' | 'public';
  label: string;
}[] = [
  ...
  { value: 'SUBAGENT', segment: 'subagent', label: 'SUBAGENT (B2B portal)' },
  {
    ...
    segment: 'business',
    label: 'BUSINESS_CLIENT (korporativni self-service)',
  },
  {
    value: 'PUBLIC_GUEST',
    segment: 'public',
    label: 'PUBLIC_GUEST (anonimni/pojedinačni B2C gosti)',
  },
];
...
export default async function NoviClanakPage() {
  ...
  const allowedSegments = SEGMENTS.filter((s) =>
    hasPermission(me, 'M21', `article:${s.segment}`, 'EDIT'),
  );
