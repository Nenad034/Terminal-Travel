...
export default async function ContractPeriodDetailPage(props: {
  ...
  const period = await apiFetch<ContractPeriodDetail>(
    `/contracting/contracts/${params.id}/periods/${params.periodId}`,
  );
    ...
    <div className="p-6">
      ...
      <div className="mb-4 rounded-lg border border-border bg-panel p-5 text-xs">
          ...
          {period.totalCapacity != null ? (
            ...
            <div className="col-span-3">
              ...
              <dd className="mt-0.5 text-ink">
                Bez garantovanog kapaciteta — svaka rezervacija čeka potvrdu dobavljača (M3 spec
                §2.3).
              </dd>
            </div>
          )}
        ...
        {period.agePolicyOverride && period.agePolicyOverride.length > 0 && (
          <div className="mt-3 border-t border-border pt-3">
            <dt className="mb-1 text-ink-faint">
              Uzrasna politika — izuzetak za ovaj period (M3 spec §2.3c)
            </dt>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <RateLinesPanel
          ...
          rateLines={period.rateLines}
          canEdit={canEdit}
        />
        <CancellationRulesPanel
          contractId={params.id}
          periodId={params.periodId}
          rules={period.cancellationRules}
          canEdit={canEdit}
        />
        <OffersPanel
          contractId={params.id}
          periodId={params.periodId}
          offers={period.offers}
          canEdit={canEdit}
        />
        <AncillaryServicesPanel
          contractId={params.id}
          periodId={params.periodId}
          services={period.ancillaryServices}
          canEdit={canEdit}
        />
        <TouristTaxPanel
          contractId={params.id}
          periodId={params.periodId}
          taxInfo={period.touristTaxInfo}
          canEdit={canEdit}
        />
      </div>
    </div>
