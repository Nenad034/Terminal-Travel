...
export async function grantSupplierAccess(
  ...
  try {
    await apiFetch(`/chat/supplier-conversations/${conversationId}/access`, {
      method: 'POST',
      body: { userId },
    });
  } catch (err) {
...
export async function revokeSupplierAccess(conversationId: string, userId: string): Promise<void> {
  try {
    await apiFetch(`/chat/supplier-conversations/${conversationId}/access/${userId}`, {
      method: 'DELETE',
    });
  } catch {
...
export async function inviteSupplierContact(
  ...
  if (typeof supplierContactId !== 'string' || !supplierContactId)
    return { error: 'Izaberite kontakt-osobu.' };
  let result: { inviteToken: string };
  try {
    result = await apiFetch<{ inviteToken: string }>(
      `/chat/supplier-conversations/${conversationId}/invite-contact`,
      {
        method: 'POST',
