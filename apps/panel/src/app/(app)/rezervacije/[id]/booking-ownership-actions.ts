...
export async function transferOwnership(
  ...
  try {
    await apiFetch(`/sales/bookings/${bookingId}/transfer-ownership`, {
      method: 'POST',
      body: { newOwnerId },
    });
  } catch (err) {
    return {
      error: err instanceof ApiError ? extractMessage(err) : 'Prenos vlasništva nije uspeo.',
    };
  }
...
export async function proposeHandoff(
  ...
  try {
    await apiFetch(`/sales/bookings/${bookingId}/handoff-requests`, {
      method: 'POST',
      body: { toUserId },
    });
  } catch (err) {
...
export async function acceptHandoff(
  ...
  try {
    ...
    return {
      error: err instanceof ApiError ? extractMessage(err) : 'Prihvatanje predloga nije uspelo.',
    };
  }
  revalidatePath(`/rezervacije/${bookingId}`);
  return { error: null };
...
export async function cancelHandoff(
  ...
  try {
    ...
    return {
      error: err instanceof ApiError ? extractMessage(err) : 'Otkazivanje predloga nije uspelo.',
    };
  }
  revalidatePath(`/rezervacije/${bookingId}`);
  return { error: null };
