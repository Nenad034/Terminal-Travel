...
export async function prepareForBooking(_prev: FormState, formData: FormData): Promise<FormState> {
  ...
  try {
    const drafts = await apiFetch<unknown[]>(
      `/sales/bookings/${bookingId}/prepare-supplier-manifests`,
      { method: 'POST' },
    );
    revalidatePath(PATH);
    return {
    ...
    return {
      error: err instanceof ApiError ? extractMessage(err) : 'Priprema nacrta nije uspela.',
      notice: null,
    };
  }
}
...
export async function sendManifest(
  ...
  try {
    const updated = await apiFetch<{ status: string; sentToEmail: string | null }>(
      `/sales/supplier-manifests/${id}/send`,
      { method: 'POST' },
    );
    revalidatePath(PATH);
    return updated.status === 'SENT'
      ...
      : {
          error: null,
          notice:
            'Pokušaj je zabeležen, ali poruka NIJE otišla — lista čeka slanje. Pokušajte ponovo kad pošta proradi.',
        };
  } catch (err) {
    return {
      error: err instanceof ApiError ? extractMessage(err) : 'Slanje nije uspelo.',
      notice: null,
    };
  }
}
...
export async function confirmManifest(
  ...
  try {
    ...
    return {
      error: err instanceof ApiError ? extractMessage(err) : 'Upis potvrde nije uspeo.',
      notice: null,
    };
  }
}
...
export async function sendChangeNotice(
  id: string,
  ...
  if (!supplierEmail)
    return { error: 'Za ovu stavku nije poznata adresa dobavljača.', notice: null };
  try {
    const updated = await apiFetch<{ status: string }>(
      `/sales/supplier-change-notices/${id}/send`,
      {
        method: 'POST',
        body: { supplierEmail },
      },
...
export async function confirmChangeNotice(
  id: string,
  _prev: FormState,
  _formData: FormData,
): Promise<FormState> {
