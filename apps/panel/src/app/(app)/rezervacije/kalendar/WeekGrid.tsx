    ...
    <div className="grid grid-cols-7 gap-2">
      {dates.map((dateStr, i) => {
        ...
        return (
          <div
            key={dateStr}
            className={`flex min-h-[220px] flex-col gap-2 rounded-lg border p-2 ${isToday ? 'border-accent bg-accent-soft/30' : 'border-border bg-panel'}`}
          >
            <TabLink
              href={buildHref('day', dateStr, filters)}
              label="Kalendar rezervacija"
              className="flex items-center justify-between text-xs hover:text-accent-strong"
            >
              <span className="text-ink-faint">{WEEKDAYS_SHORT_SR[i]}</span>
              <span
                className={`font-mono ${isToday ? 'font-semibold text-accent-strong' : 'text-ink'}`}
              >
            ...
            {count === 0 ? (
              ...
              <DayAgenda detail={detail} compact />
            )}
          </div>
        );
      })}
