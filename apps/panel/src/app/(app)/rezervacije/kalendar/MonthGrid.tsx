  ...
  return (
    <div className="grid grid-cols-7 gap-1 text-center text-[11px]">
      ...
      {cells.map((dateStr) => {
        ...
        const total = d
          ? d.arrivalsCount + d.departuresCount + d.stayoversCount + d.singleDayCount
          : 0;
        return (
          <TabLink
            key={dateStr}
                ...
                <span className="flex gap-0.5">
                  {d!.arrivalsCount > 0 && (
                    <span
                      ...
                      title={`${d!.arrivalsCount} dolazak`}
                    />
                  )}
                  {d!.departuresCount > 0 && (
                    <span
                      className="h-1.5 w-1.5 rounded-full bg-warn"
                      title={`${d!.departuresCount} odlazak`}
                    />
                  )}
                  {d!.stayoversCount > 0 && (
                    <span
                      className="h-1.5 w-1.5 rounded-full bg-accent2"
                      title={`${d!.stayoversCount} u toku`}
                    />
                  )}
                </span>
