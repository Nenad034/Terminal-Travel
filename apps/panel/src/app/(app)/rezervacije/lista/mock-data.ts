...
export interface MockBookingRow {
  ...
  // `target` (23.8.2026, na zahtev vlasnika: "treba razlikovati obavestenja koje saljemo
  // klijentima i partnerima, dobavljacima po logici stvari") — problem koji čeka rok NIJE uvek
  // na strani gosta/nalogodavca: "čeka potvrdu dobavljača" znači treba pozvati/pisati DOBAVLJAČU,
  // ne klijentu. `UrgentModal` bira ispravan kontakt (buyer* ili supplier*) prema ovome umesto da
  // uvek prikaže isti.
  // NIZ, ne pojedinačan zapis (dopuna 23.8.2026, na zahtev vlasnika: "jedna rezervacija moze da
  // ima vise notifikacija... treba da se pojave 1,2,3... onoliko mini modula koliko ima
  // notifikacija") — prazan/`null` znači nema zvonca, svaka stavka niza je sopstveni razlog +
  // sopstveni `target` (mogu biti mešoviti — jedan ka dobavljaču, drugi ka klijentu, na istoj
  // rezervaciji).
  urgent: { reason: string; target: 'BUYER' | 'SUPPLIER' }[] | null;
  branch: string;
  assignedUser: string;
  supplierName: string;
  /** Stvaran izvor bi bio `Supplier.contact_email`/`.contact_phone` (M3 spec, model `Supplier`). */
...
const derivePhone = derivePhoneFromSeed;

function withDerived(
  row: Omit<
    MockBookingRow,
    'paidAmount' | 'items' | 'buyerEmail' | 'buyerPhone' | 'supplierEmail' | 'supplierPhone'
  > & {
    paidAmount?: number;
    items?: MockBookingItem[];
    buyerEmail?: string;
  ...
  // Dobavljač je odvojen pravni subjekat (M3 `Supplier`), ne isti kontakt kao gost/nalogodavac —
  // izveden odvojeno iz `supplierName` da se u UrgentModal-u nikad ne pomeša sa kontaktom kupca.
  const supplierEmail = row.supplierEmail ?? `${slugifyName(row.supplierName)}@dobavljac.local`;
  const supplierPhone = row.supplierPhone ?? derivePhoneFromSeed(row.supplierName);
  return { ...row, paidAmount, items, buyerEmail, buyerPhone, supplierEmail, supplierPhone };
}
...
function defaultItem(row: {
  bookingNumber: string;
  channel: MockBookingRow['channel'];
  productType: MockBookingRow['productType'];
  country: string;
  destinationCity: string;
  hotelName: string;
...
export const MOCK_BOOKINGS: MockBookingRow[] = [
  withDerived({
    ...
    hotelName: 'Sani Beach Hotel',
    accommodationType: 'Dvokrevetna soba, All Inclusive',
    productType: 'ACCOMMODATION',
    travelers: [
      { name: 'Marko Petrović', ageCategory: 'ADULT' },
      { name: 'Jovana Petrović', ageCategory: 'ADULT', birthYear: 1991 },
    ],
    urgent: null,
    branch: 'Beograd — centrala',
    assignedUser: 'Marija Nikolić',
    supplierName: 'Travelgate',
    partnerName: null,
  }),
  withDerived({
    bookingNumber: 'TT-2026-100043',
    ...
    country: 'Turska',
    destinationCity: 'Antalija',
    hotelName: 'Rixos Downtown',
    accommodationType: 'Porodična soba, All Inclusive',
    productType: 'ACCOMMODATION',
    travelers: [
      { name: 'Ana Jovanović', ageCategory: 'ADULT' },
      { name: 'Petar Jovanović', ageCategory: 'ADULT' },
      { name: 'Mila Jovanović', ageCategory: 'CHILD', birthYear: 2019 },
    ],
    urgent: [
      {
        reason: 'Rok za potvrdu dobavljača ističe za 24h — hotel još nije potvrdio dostupnost.',
        target: 'SUPPLIER',
      },
    ],
    branch: 'Beograd — centrala',
    assignedUser: 'Petar Stevanović',
    supplierName: 'Solvex',
    partnerName: null,
  }),
  withDerived({
    bookingNumber: 'TT-2026-100044',
    buyerName: 'Nikolić Travel d.o.o.',
    ...
    country: 'Egipat',
    destinationCity: 'Hurgada',
    hotelName: 'Steigenberger Aqua Magic',
    accommodationType: 'Dvokrevetna soba, All Inclusive',
    productType: 'PACKAGE',
    travelers: [{ name: 'Grupa 6 putnika (B2B)', ageCategory: 'ADULT' }],
    urgent: null,
    branch: 'Novi Sad',
    assignedUser: 'Ana Radulović',
    supplierName: 'WebHotelier',
    partnerName: 'Nikolić Travel d.o.o.',
  }),
  withDerived({
    bookingNumber: 'TT-2026-100045',
    buyerName: 'Milica Stanković',
    buyerType: 'FIZICKO_LICE',
    channel: 'MOBILE',
    status: 'CANCELLED',
    paymentStatus: 'UNPAID',
    ...
    hotelName: 'Hotel Avala',
    accommodationType: 'Dvokrevetna soba, Noćenje sa doručkom',
    productType: 'ACCOMMODATION',
    travelers: [{ name: 'Milica Stanković', ageCategory: 'ADULT' }],
    urgent: null,
    branch: 'Beograd — centrala',
    assignedUser: 'Marija Nikolić',
    supplierName: 'Travelgate',
  ...
  withDerived({
    bookingNumber: 'TT-2026-100046',
    buyerName: 'Đorđe Ilić',
    buyerType: 'FIZICKO_LICE',
    channel: 'PHONE',
    status: 'CONFIRMED',
    paymentStatus: 'UNPAID',
    ...
    country: 'Grčka',
    destinationCity: 'Krit',
    hotelName: 'Aldemar Royal Mare',
    accommodationType: 'Bungalov, Polupansion',
    productType: 'ACCOMMODATION',
    travelers: [
      { name: 'Đorđe Ilić', ageCategory: 'ADULT' },
      { name: 'Tijana Ilić', ageCategory: 'ADULT' },
    ],
    urgent: [
      { reason: 'Gost još nije uplatio depozit — rok istekao pre 2 dana.', target: 'BUYER' },
    ],
    branch: 'Niš',
    assignedUser: 'Petar Stevanović',
    supplierName: 'Solvex',
  ...
  withDerived({
    bookingNumber: 'TT-2026-100047',
    buyerName: 'Jelena Radovanović',
    buyerType: 'FIZICKO_LICE',
    channel: 'B2C_SITE',
    status: 'COMPLETED',
    paymentStatus: 'PAID',
    ...
    country: 'Španija',
    destinationCity: 'Majorka',
    hotelName: 'Iberostar Playa de Muro',
    accommodationType: 'Dvokrevetna soba, All Inclusive',
    productType: 'ACCOMMODATION',
    travelers: [
      { name: 'Jelena Radovanović', ageCategory: 'ADULT' },
      { name: 'Nikola Radovanović', ageCategory: 'ADULT' },
    ],
    urgent: null,
    branch: 'Beograd — centrala',
    assignedUser: 'Marija Nikolić',
  ...
  withDerived({
    bookingNumber: 'TT-2026-100048',
    buyerName: 'Stefan Pavlović',
    buyerType: 'FIZICKO_LICE',
    channel: 'INTERNAL_PANEL',
    status: 'CONFIRMED',
    paymentStatus: 'UNPAID',
    ...
    country: 'Italija',
    destinationCity: 'Rimini',
    hotelName: 'Hotel Waldorf',
    accommodationType: 'Trokrevetna soba, Polupansion',
    productType: 'ACCOMMODATION',
    travelers: [
      { name: 'Stefan Pavlović', ageCategory: 'ADULT' },
      { name: 'Milica Pavlović', ageCategory: 'ADULT' },
      { name: 'Luka Pavlović', ageCategory: 'CHILD', birthYear: 2021 },
    ],
    urgent: [
      { reason: 'Nedostaje potvrda ugovora sa klijentom (M20) pre polaska.', target: 'BUYER' },
    ],
    branch: 'Novi Sad',
    assignedUser: 'Ana Radulović',
    supplierName: 'WebHotelier',
  ...
  withDerived({
    bookingNumber: 'TT-2026-100049',
    buyerName: 'Balkan Tours d.o.o.',
    buyerType: 'PRAVNO_LICE',
    channel: 'B2B_PORTAL',
    status: 'MODIFIED',
    paymentStatus: 'PARTIALLY_PAID',
    ...
    country: 'Grčka',
    destinationCity: 'Tasos',
    hotelName: 'Alexandra Beach',
    accommodationType: 'Dvokrevetna soba, All Inclusive',
    productType: 'PACKAGE',
    travelers: [{ name: 'Grupa 8 putnika (B2B)', ageCategory: 'ADULT' }],
    urgent: null,
    branch: 'Beograd — centrala',
    assignedUser: 'Marija Nikolić',
    supplierName: 'Travelgate',
    partnerName: 'Balkan Tours d.o.o.',
  }),
  withDerived({
    bookingNumber: 'TT-2026-100050',
    buyerName: 'Vuk Nikolić',
    buyerType: 'FIZICKO_LICE',
    channel: 'MOBILE',
    status: 'CONFIRMED',
    paymentStatus: 'PAID',
    ...
    hotelName: 'Alexander The Great',
    accommodationType: 'Dvokrevetna soba, All Inclusive',
    productType: 'ACCOMMODATION',
    travelers: [
      { name: 'Vuk Nikolić', ageCategory: 'ADULT' },
      { name: 'Marija Nikolić', ageCategory: 'ADULT' },
    ],
    urgent: null,
    branch: 'Niš',
    assignedUser: 'Petar Stevanović',
  ...
  withDerived({
    bookingNumber: 'TT-2026-100051',
    buyerName: 'Ivana Đukić',
    buyerType: 'FIZICKO_LICE',
    channel: 'B2C_SITE',
    status: 'CONFIRMED',
    paymentStatus: 'UNPAID',
    ...
    hotelName: 'Hotel Splendid',
    accommodationType: 'Dvokrevetna soba, Noćenje sa doručkom',
    productType: 'ACCOMMODATION',
    travelers: [{ name: 'Ivana Đukić', ageCategory: 'ADULT' }],
    urgent: null,
    branch: 'Beograd — centrala',
    assignedUser: 'Marija Nikolić',
    supplierName: 'Travelgate',
  ...
  withDerived({
    bookingNumber: 'TT-2026-100052',
    buyerName: 'Miloš Simić',
    buyerType: 'FIZICKO_LICE',
    channel: 'PHONE',
    status: 'CANCELLED',
    paymentStatus: 'UNPAID',
    ...
    country: 'Albanija',
    destinationCity: 'Sarandë',
    hotelName: 'Bahamas Hotel',
    accommodationType: 'Dvokrevetna soba, Noćenje sa doručkom',
    productType: 'ACCOMMODATION',
    travelers: [{ name: 'Miloš Simić', ageCategory: 'ADULT' }],
    urgent: null,
    branch: 'Niš',
    assignedUser: 'Petar Stevanović',
    supplierName: 'WebHotelier',
  ...
  withDerived({
    bookingNumber: 'TT-2026-100053',
    buyerName: 'Teodora Mitrović',
    buyerType: 'FIZICKO_LICE',
    channel: 'INTERNAL_PANEL',
    status: 'PENDING_SUPPLIER_CONFIRMATION',
    paymentStatus: 'UNPAID',
    ...
    accommodationType: 'Dvokrevetna soba, Ultra All Inclusive',
    productType: 'ACCOMMODATION',
    travelers: [
      { name: 'Teodora Mitrović', ageCategory: 'ADULT' },
      { name: 'Aleksa Mitrović', ageCategory: 'BABY', birthYear: 2025 },
    ],
    urgent: [
      { reason: 'Rok za potvrdu dobavljača ističe za 24h.', target: 'SUPPLIER' },
      {
        reason: 'Nedostaje broj pasoša za bebu (Aleksa Mitrović) — rok pre polaska.',
        target: 'BUYER',
      },
    ],
    branch: 'Novi Sad',
  ...
  withDerived({
    bookingNumber: 'TT-2026-100054',
    buyerName: 'Aleksandar Kostić',
    buyerType: 'FIZICKO_LICE',
    channel: 'B2C_SITE',
    status: 'CONFIRMED',
    paymentStatus: 'PAID',
    ...
    country: 'Grčka',
    destinationCity: 'Paralija',
    hotelName: 'Mediterranean Village',
    accommodationType: 'Dvokrevetna soba, Polupansion',
    productType: 'ACCOMMODATION',
    travelers: [
      { name: 'Aleksandar Kostić', ageCategory: 'ADULT' },
      { name: 'Ksenija Kostić', ageCategory: 'ADULT' },
    ],
    urgent: null,
    branch: 'Beograd — centrala',
    assignedUser: 'Marija Nikolić',
  ...
  withDerived({
    bookingNumber: 'TT-2026-100055',
    buyerName: 'Sunrise Agencija d.o.o.',
    buyerType: 'PRAVNO_LICE',
    channel: 'B2B_PORTAL',
    status: 'CONFIRMED',
    ...
    country: 'Grčka',
    destinationCity: 'Krf',
    hotelName: 'Grecotel Corfu Imperial',
    accommodationType: 'Apartman, All Inclusive',
    productType: 'PACKAGE',
    travelers: [{ name: 'Grupa 10 putnika (B2B)', ageCategory: 'ADULT' }],
    urgent: [
      {
        reason: 'Faktura još nije poslata nalogodavcu (INVOICE_PENDING preko roka).',
        target: 'BUYER',
      },
    ],
    branch: 'Novi Sad',
    assignedUser: 'Ana Radulović',
    supplierName: 'WebHotelier',
    partnerName: 'Sunrise Agencija d.o.o.',
  }),
  withDerived({
    bookingNumber: 'TT-2026-100056',
    buyerName: 'Nataša Popović',
    buyerType: 'FIZICKO_LICE',
    channel: 'MOBILE',
    status: 'CONFIRMED',
    paymentStatus: 'UNPAID',
    ...
    country: 'Bugarska',
    destinationCity: 'Sunčev Breg',
    hotelName: 'Hotel Marina Grand Beach',
    accommodationType: 'Dvokrevetna soba, All Inclusive',
    productType: 'ACCOMMODATION',
    travelers: [
      { name: 'Nataša Popović', ageCategory: 'ADULT' },
      { name: 'Miloš Popović', ageCategory: 'ADULT' },
    ],
    urgent: null,
    branch: 'Beograd — centrala',
    assignedUser: 'Marija Nikolić',
  ...
  withDerived({
    bookingNumber: 'TT-2026-100057',
    buyerName: 'Filip Đorđević',
    buyerType: 'FIZICKO_LICE',
    channel: 'B2C_SITE',
    status: 'COMPLETED',
    paymentStatus: 'PAID',
    ...
    country: 'Crna Gora',
    destinationCity: 'Petrovac',
    hotelName: 'Hotel Palas',
    accommodationType: 'Dvokrevetna soba, Noćenje sa doručkom',
    productType: 'ACCOMMODATION',
    travelers: [{ name: 'Filip Đorđević', ageCategory: 'ADULT' }],
    urgent: null,
    branch: 'Niš',
    assignedUser: 'Petar Stevanović',
    supplierName: 'Travelgate',
  ...
  withDerived({
    bookingNumber: 'TT-2026-100058',
    buyerName: 'Katarina Vasić',
    buyerType: 'FIZICKO_LICE',
    channel: 'INTERNAL_PANEL',
    status: 'CONFIRMED',
    paymentStatus: 'UNPAID',
    ...
    country: 'Grčka',
    destinationCity: 'Nea Kalikratija',
    hotelName: 'Athos Palace',
    accommodationType: 'Dvokrevetna soba, Polupansion',
    productType: 'ACCOMMODATION',
    travelers: [
      { name: 'Katarina Vasić', ageCategory: 'ADULT' },
      { name: 'Ognjen Vasić', ageCategory: 'CHILD', birthYear: 2017 },
    ],
    urgent: null,
    branch: 'Beograd — centrala',
    assignedUser: 'Marija Nikolić',
  ...
  withDerived({
    bookingNumber: 'TT-2026-100059',
    buyerName: 'AI Agent (MCP)',
    buyerType: 'PRAVNO_LICE',
    channel: 'MCP_AGENT',
    status: 'CONFIRMED',
    paymentStatus: 'PAID',
    ...
    hotelName: 'Danai Beach Resort',
    accommodationType: 'Vila, All Inclusive',
    productType: 'ACCOMMODATION',
    travelers: [{ name: 'Rezervacija preko MCP agenta — putnici nepoznati', ageCategory: 'ADULT' }],
    urgent: null,
    branch: 'Beograd — centrala',
    assignedUser: 'sistem (MCP)',
    supplierName: 'Travelgate',
    partnerName: null,
  }),
  withDerived({
    bookingNumber: 'TT-2026-100060',
    buyerName: 'Dušan Lazić',
    buyerType: 'FIZICKO_LICE',
    channel: 'PHONE',
    status: 'MODIFIED',
    paymentStatus: 'PARTIALLY_PAID',
    ...
    country: 'Turska',
    destinationCity: 'Marmaris',
    hotelName: 'Hilton Dedeman',
    accommodationType: 'Dvokrevetna soba, All Inclusive',
    productType: 'ACCOMMODATION',
    travelers: [
      { name: 'Dušan Lazić', ageCategory: 'ADULT' },
      { name: 'Bojana Lazić', ageCategory: 'ADULT' },
    ],
    urgent: null,
    branch: 'Novi Sad',
    assignedUser: 'Ana Radulović',
  ...
  withDerived({
    bookingNumber: 'TT-2026-100061',
    buyerName: 'Marija Todorović',
    buyerType: 'FIZICKO_LICE',
    channel: 'B2C_SITE',
    status: 'CONFIRMED',
    paymentStatus: 'UNPAID',
    ...
    hotelName: 'Hotel Plaža',
    accommodationType: 'Dvokrevetna soba, Noćenje sa doručkom',
    productType: 'ACCOMMODATION',
    travelers: [{ name: 'Marija Todorović', ageCategory: 'ADULT' }],
    urgent: [{ reason: 'Rok za uplatu ostatka ističe sutra.', target: 'BUYER' }],
    branch: 'Niš',
    assignedUser: 'Petar Stevanović',
    supplierName: 'Travelgate',
    partnerName: null,
...
export function buildMockTimeline(b: MockBookingRow): TimelineEntry[] {
  ...
  const entries: TimelineEntry[] = [
    {
      ...
      action: 'booking.pending_supplier_confirmation',
      actorType: 'HUMAN',
      actorName: 'Marija Nikolić (prodaja)',
    },
  ];
  if (b.status !== 'PENDING_SUPPLIER_CONFIRMATION') {
    entries.push({
      timestamp: plusDays(created, 1).toISOString(),
      action: 'booking.confirmed',
      actorType: 'AI_AGENT',
      actorName: 'SupplierConfirmationAgent',
    });
  }
  if (b.status === 'MODIFIED') {
    entries.push({
      timestamp: plusDays(created, 3).toISOString(),
  ...
  if (b.status === 'CANCELLED') {
    entries.push({
      timestamp: plusDays(created, 2).toISOString(),
      action: 'booking.cancelled',
      actorType: 'HUMAN',
      actorName: 'Nenad Tomić (vlasnik)',
    });
  }
  if (b.paymentStatus === 'PAID' || b.paymentStatus === 'PARTIALLY_PAID') {
    entries.push({
      timestamp: plusDays(created, 4).toISOString(),
      action: 'payment.recorded',
      actorType: 'SYSTEM',
      actorName: 'sistem (uplata evidentirana)',
    });
  }
  if (b.status === 'COMPLETED') {
    entries.push({
      ...
      actorType: 'SYSTEM',
      actorName: 'sistem (datum povratka prošao)',
    });
  }
  return entries;
}
