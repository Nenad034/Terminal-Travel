...
export default async function ZnanjeDetailPage(props: { params: Promise<{ id: string }> }) {
  ...
  const osvezavanjeDospelo =
    !!article.nextRefreshDueAt &&
    // eslint-disable-next-line react-hooks/purity
    new Date(article.nextRefreshDueAt).getTime() <= Date.now();
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-4 flex items-start justify-between">
        <div>
          <h1 className="text-lg font-semibold text-ink">
            {article.translation?.title ?? article.subjectType}
          </h1>
          <p className="flex flex-wrap items-center gap-1 text-xs text-ink-faint">
            {article.subjectType}
            {article.destinationCountry
              ? ` · ${article.destinationCountry}${article.destinationCity ? `, ${article.destinationCity}` : ''}`
              : ''}
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 rounded-lg border border-border bg-panel p-4 text-xs sm:grid-cols-4">
        ...
        <Info
          label="poslednje osveženo"
          value={
            article.lastRefreshedAt
              ? new Date(article.lastRefreshedAt).toLocaleDateString('sr-RS')
              : '—'
          }
        />
        <Info
          ...
          value={
            article.nextRefreshDueAt
              ? new Date(article.nextRefreshDueAt).toLocaleDateString('sr-RS')
              : '—'
          }
      ...
      {article.status === 'PUBLISHED' && article.shareToken && (
        <div className="mb-4">
          <p className="mb-1 text-[11px] text-ink-faint">
            Javni deljeni link (M23 spec §5) — otvara samo ovaj članak, bez pretrage. Kopirajte i
            ručno pošaljite gostu (email/Viber/WhatsApp/Telegram/SMS).
          </p>
