...
export default async function MailboxManagementPage() {
  ...
  if (canView) {
    ...
    error =
      'Nemate dozvolu za uvid u sandučad (M22/mailbox/VIEW) — upravljanje sandučadima je ograničeno na Vlasnika/Direktora.';
  }
    ...
    await Promise.all(
      mailboxes.map(async (mb) => {
        try {
          accessByMailbox.set(
            mb.id,
            await apiFetch<MailboxAccessRow[]>(`/email/mailboxes/${mb.id}/access`),
          );
        } catch {
              ...
              <div className="text-sm font-medium text-ink">
                ...
                {mb.isSupplierUnifiedInbox && (
                  <span className="ml-2 rounded bg-accent-soft px-1.5 py-0.5 text-[11px] text-accent-strong">
                    jedinstveno sanduče dobavljača
                  </span>
                )}
              </div>
                ...
                <div className="mt-3 border-t border-border pt-3">
                  ...
                  {(accessByMailbox.get(mb.id) ?? []).length === 0 ? (
                    <p className="mb-2 text-[11px] text-ink-faint">
                      Niko dodatno nema dodeljen pristup (vlasnik ličnog sandučeta ga dobija
                      automatski).
                    </p>
                  ) : (
                    <div className="mb-2 flex flex-col gap-1">
                      {(accessByMailbox.get(mb.id) ?? []).map((a) => (
                        <div
                          key={a.id}
                          className="flex items-center justify-between rounded bg-panel2 px-2 py-1 text-[11px]"
                        >
