...
export async function createMcpClient(_prev: FormState, formData: FormData): Promise<FormState> {
  ...
  try {
    const created = await apiFetch<{ credential: string; clientName: string }>(
      '/mcp-admin/clients',
      {
        method: 'POST',
    ...
    return {
      error:
        err instanceof ApiError ? extractMessage(err) : 'Registracija MCP klijenta nije uspela.',
    };
  }
}

// M16 spec §3.1 — PENDING→ACTIVE, dozvola M16/mcp-client/MANAGE.
export async function activateMcpClient(
  id: string,
  _prev: FormState,
  _formData: FormData,
): Promise<FormState> {
  try {
    ...
    return {
      error: err instanceof ApiError ? extractMessage(err) : 'Aktivacija MCP klijenta nije uspela.',
    };
  }
  revalidatePath('/mcp');
  return { error: null };
...
export async function approveReadWriteMcpClient(
  id: string,
  _prev: FormState,
  _formData: FormData,
): Promise<FormState> {
    ...
    return {
      error:
        err instanceof ApiError
          ? extractMessage(err)
          : 'Odobrenje READ_WRITE pristupa nije uspelo.',
    };
  }
  revalidatePath('/mcp');
  return { error: null };
...
export async function suspendMcpClient(
  id: string,
  _prev: FormState,
  _formData: FormData,
): Promise<FormState> {
  try {
    ...
    return {
      error:
        err instanceof ApiError ? extractMessage(err) : 'Suspendovanje MCP klijenta nije uspelo.',
    };
  }
  revalidatePath('/mcp');
  return { error: null };
