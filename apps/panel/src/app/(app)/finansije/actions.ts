...
export async function prepareFiscalDocument(
  bookingId: string,
  _prev: FormState,
  _formData: FormData,
): Promise<FormState> {
  ...
  try {
    const doc = await apiFetch<{ id: string }>('/finance/fiscal-documents/draft', {
      method: 'POST',
      body: { bookingId },
    });
    documentId = doc.id;
    ...
    return {
      error:
        err instanceof ApiError ? extractMessage(err) : 'Priprema fiskalnog dokumenta nije uspela.',
    };
  }
...
export async function submitFiscalDocument(
  ...
  try {
    ...
    return {
      error:
        err instanceof ApiError ? extractMessage(err) : 'Slanje fiskalnog dokumenta nije uspelo.',
    };
  }
...
export async function stornoFiscalDocument(
  id: string,
  _prev: FormState,
  _formData: FormData,
): Promise<FormState> {
  ...
  try {
    await apiFetch('/finance/payments', {
      ...
      body: {
        ...
        ...((method === 'BANK_TRANSFER' || method === 'CARD_MANUAL') && formData.get('bankId')
    ...
    return {
      error: err instanceof ApiError ? extractMessage(err) : 'Beleženje uplate nije uspelo.',
    };
  }
...
export async function approveSupplierObligation(
  id: string,
  _prev: FormState,
  _formData: FormData,
): Promise<FormState> {
  try {
    ...
    return {
      error: err instanceof ApiError ? extractMessage(err) : 'Odobravanje obaveze nije uspelo.',
    };
  }
  revalidatePath('/finansije');
  return { error: null };
...
export async function paySupplierObligation(
  id: string,
  _prev: FormState,
  _formData: FormData,
): Promise<FormState> {
    ...
    return {
      error:
        err instanceof ApiError
          ? extractMessage(err)
          : 'Beleženje plaćanja dobavljaču nije uspelo.',
    };
  }
  revalidatePath('/finansije');
  return { error: null };
