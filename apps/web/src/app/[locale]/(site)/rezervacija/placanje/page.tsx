...
export default async function PaymentPage({
  ...
  return (
    <div className="mx-auto max-w-xl">
      ...
      {sp.greska && (
        <p className="mb-4 rounded-md bg-danger-bg p-3 text-sm text-danger">
          Plaćanje nije uspelo, pokušajte ponovo.
        </p>
      )}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <form action={payByCardAction} className="rounded-lg border border-border p-4">
          ...
          <button
            type="submit"
            className="w-full rounded-md bg-accent px-4 py-2 font-medium text-accent-ink hover:bg-accent-strong"
          >
            {t('card')}
          </button>
        </form>
        ...
        <form action={payByBankTransferAction} className="rounded-lg border border-border p-4">
          ...
          <button
            type="submit"
            className="w-full rounded-md border border-accent px-4 py-2 font-medium text-accent hover:bg-accent-soft"
          >
            {t('bankTransfer')}
          </button>
        </form>
