  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10 print:px-0 print:py-0">
      ...
      {voucher.groups.map((group) => (
        <section
          key={group.index}
          className="mb-8 break-after-page last:mb-0 last:break-after-auto"
        >
          ...
          {group.onSiteTotal > 0 && (
            ...
            <p className="mb-3 rounded border border-warn bg-warn-bg p-3 text-sm text-ink">
              {t('onSiteNotice', {
                amount: (group.onSiteTotal / 100).toLocaleString(locale, {
              ...
              <div key={i} className="rounded-lg border border-border p-4">
                ...
                <dl className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm text-ink-dim sm:grid-cols-3">
                  ...
                  <div>
                    ...
                    <dd>
                      {new Date(item.stayFrom).toLocaleDateString(locale)} –{' '}
                      {new Date(item.stayTo).toLocaleDateString(locale)}
                    </dd>
                  </div>
                  <div>
                ...
                {item.payable === 'ON_SITE' && (
                  <p className="mt-2 text-sm font-medium text-warn">
                    {t('paidOnSite', {
                      amount: (item.price / 100).toLocaleString(locale, {
                        minimumFractionDigits: 2,
                      }),
