...
export function BookingItemDetailScreen({ bookingItemId }: { bookingItemId: string }) {
  ...
  useEffect(() => {
    getCachedItinerary().then((all) =>
      setItem(all.find((i) => i.bookingItemId === bookingItemId) ?? null),
    );
  }, [bookingItemId]);
  ...
  return (
    <View style={styles.container}>
      ...
      <Pressable
        ...
        onPress={() => router.push(`/(guide)/beleska/${item.bookingId}`)}
      >
          ...
          return (
            <View style={styles.guestRow}>
              <View style={styles.guestInfo}>
                ...
                {guest.preferences ? (
                  <Text style={styles.guestMeta}>{JSON.stringify(guest.preferences)}</Text>
                ) : null}
              </View>
              <Pressable
