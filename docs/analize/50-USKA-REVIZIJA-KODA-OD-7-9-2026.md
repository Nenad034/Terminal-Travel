...
### 3.5 Dve nove liste bez straničenja, jedna sa tihim odsecanjem — REŠENO 18.9.2026
...
**REŠENO 18.9.2026** (M3 spec v1.48): obe liste vraćaju `{ data, total, … }` preko zajedničkog `common/pagination`; `expiry-notices` dobija `page`/`limit` u svom `@Query() dto` (jedini ispravan način uz `forbidNonWhitelisted`). Živo: `?limit=2` → `total 2, pageCount 1`; `?limit=999` → 400; `?page=0` → 400. Panel `/cenovnici` čita `data` i ima traku straničenja — viđeno u browseru (`npm run qa -- cenovnici`, snimak: „prikazano 1–2 od 2 uvoza“). 2 nova testa.
...
| #   | Nalaz                                                                                                                                                                                               | Dokaz                                                                                           | Predlog                                                                                                                                                                                                                                                 |
| :-- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 4.1 | ~~Zastareo komentar: `pricelist-imports.service.ts:128-131` kaže da AI ekstrakcija „zahteva odluku o AI provajderu koja još nije doneta"~~ — REŠENO 18.9.2026, komentar zamenjen tačnim opisom toka | A: `pricelist-extraction.service.ts:13,223,485` uvozi i zove `AnthropicClientService`; 24 testa | Obrisati komentar; isti rod kao a3269a95 (zastareo komentar u seed.ts)                                                                                                                                                                                  |
