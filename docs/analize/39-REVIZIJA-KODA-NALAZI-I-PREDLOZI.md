...
**Dopuna 7.9.2026 — srodna rupa zatvorena u istom potezu (zamka 13.5).** Prvi prolaz kroz ovaj nalaz je zaključao _ko sme da uđe_ (`JwtAuthGuard`), ali je ostavio otvoreno _šta sme da uradi kad uđe_: `PermissionsGuard` je čitao `@RequirePermission` isključivo sa metode (`reflector.get(..., getHandler())`), pa je isti dekorator postavljen iznad `@Controller(...)` bio **tiho ignorisan** — ograda je izgledala postavljeno, a propuštala je sve. Fail-open u kodu napisanom baš da nešto zatvori. Guard sada čita i sa klase (`getAllAndOverride([getHandler(), getClass()])`), uz test koji dokazuje da se oba nivoa čitaju.

_Klasa dokaza:_ pročitano u kodu (`common/guards/permissions.guard.ts`) + izmereno (`npx jest src/common/guards/permissions.guard.spec.ts` — 6/6 prolazi, uključujući novi test za klasni dekorator). _Obim promene vidljivosti:_ prebrojano — **nula** kontrolera danas ima klasni `@RequirePermission`, pa ispravka ne otvara ni ne zatvara nijednu zatečenu rutu; menja samo smer greške za buduće.
...
| #   | Nalaz                                                                                                   | Predlog                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| :-- | :------------------------------------------------------------------------------------------------------ | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 4.1 | ~~19 mesta koristi `key={index}` u React listama~~ — PREGLEDANO 7.9.2026, DVE ISPRAVLJENE, JEDNA DODATA | Nijedan slučaj nije bio isti — pregledano mesto po mesto umesto masovne zamene (prošli nalazi u ovom dokumentu su naučili da masovna zamena bez čitanja svakog mesta pravi lažne nalaze, dok. 40). **Ispravljeno (stvaran bag):** `RealResults.tsx:206,264` (dve liste ponuda u pretrazi) već su računale `offerKey(productId, rateLineId, providerQuoteReference)` za `data-offer-key` atribut jednu liniju niže — stabilan ključ je već postojao, samo nije bio iskorišćen kao `key`. `TerminalPanel.tsx` (AI terminal, `turns` niz) ima stvarno brisanje reda (`hideRow`, linija 381) i `CopyButton` sa sopstvenim `useState` po redu — brisanje reda iz sredine liste bi pomerilo "kopirano" indikator na pogrešan red; `Turn` nije imao nijedno stabilno polje, pa je dodato `id: crypto.randomUUID()` pri kreiranju svakog reda (jedino mesto, `send()`). **Pregledano i OSTAVLJENO namerno** (indeks je ovde tačan izbor, ne propust): `DateField.tsx`/`DateRangeField.tsx` (`WEEKDAY_LETTERS`, fiksnih 7 elemenata, nikad se ne menja/sortira/filtrira — index je idiomatski ispravan za statičnu listu); `ConfirmQuoteForm.tsx` (`Array.from({length: itemCount})` generiše N praznih polja forme, `itemCount` je fiksan po instanci forme, ne dinamički niz). **Pregledano i OSTAVLJENO kao nizak rizik, ne nula** (kontrolisani unosi — `value`/`onChange` iz podataka niza — sami ispravljaju prikaz posle svakog rendera, pa index-key ovde najviše gubi fokus/kursor pri brisanju reda iz sredine, ne "zalepi" pogrešan podatak trajno kako nalaz prvobitno tvrdi): `RoomTypesEditor.tsx` (`age_policy`, ima dodaj/ukloni), `RoomsField.tsx` (sobe u pretrazi, ima dodaj/ukloni), `SearchCriteriaForm.tsx` (noge leta, ima ukloni); `UrgentModal.tsx`, `BookingRecordClient.tsx`, `RightPanel.tsx` (×2), `PeriodsPanel.tsx`, `RateLinesPanel.tsx`, `[periodId]/page.tsx`, `BookingTimelineModal.tsx`, `vaucer/[id]/page.tsx` — sve read-only prikazi bez dodaj/ukloni u istom renderu. `AccommodationResultsMock.tsx` nije dirana — deo je mock ekrana pretrage čiji je izgled vlasnik eksplicitno stavio na čekanje (nalaz 3.6, 7.9.2026: „kasnije ćemo o tome"). **Provereno:** `tsc --noEmit` čist, `eslint .` bez novih grešaka, 13 testova panela prolazi. |
| 4.2 | ~~Nema nijednog `loading.tsx`~~ — DELIMIČNO REŠENO 7.9.2026                                             | Dodat `apps/panel/src/components/Skeleton.tsx` (deljen gradivni blok, `TableSkeleton` za liste) i `loading.tsx` za pet ekrana koje nalaz izričito imenuje kao spore — lista rezervacija, kalendar, pretraga (zove M4 provajdere), dosije rezervacije (najveći ekran, >1700 linija), izveštaji. Namerno NIJE urađeno za preostalih ~35 ruta panela — nijedna nije nazvana u nalazu, a generički skeleton na brzim formama (detalj/izmena jednog zapisa) više smeta nego pomaže (bljesak pre nego što se stigne bilo šta pročitati). Dodaje se opet kad se ionako dira taj ekran, isti princip kao ostatak poglavlja 4. Provereno: `tsc`/`build`/`eslint` čisti, i `npm run qa -- lista dosije pretraga izvestaji kalendar` (dodat `kalendar` u `tools/qa.mjs` — nedostajao) — svih pet se otvara bez greške iz browsera.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 4.3 | ~~`apps/panel`, `apps/web`, `apps/mobile` nemaju README~~ — REŠENO 7.9.2026                             | Dodat README u sve tri, isti obrazac kao `apps/api` (pokretanje, `.env`, struktura, provere pre commit-a). Usput zapaženo i ispravljeno: `apps/api/README.md` je nabrajao samo M1–M4 kao implementirane, iako ih je 21; `apps/web` nema test paket uopšte (nova stavka u `27-BACKLOG`, M8 sekcija)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
**5.1 Provera „mock ili pravo" na jednom mestu — REŠENO 7.9.2026.** Nalaz 1.1 je nastao jer se prelazak sa mock-a na prave podatke desio na pola — tabela je prešla, dva sporedna ulaza u isti zapis nisu, i ništa to nije primetilo. `docs/analize/42-STANJE-EKRANA-MOCK-ILI-PRAVO.md` — jedan red po ekranu (svih ~75 ruta panela + 17 ruta sajta, pregledano pojedinačno po tome šta stranica stvarno uvoz i poziva, ne po nazivu fajla ni komentaru — komentar je upravo ono što je zakasnilo u nalazu 1.1). Nova opservacija otkrivena ovim prolazom (nije bila poznata pre): `/rezervacije/lista` izmišlja `buyerEmail`/`buyerPhone`/`branch`/`assignedUser` bez vizuelne oznake na ekranu (za razliku od susednih polja koja imaju „(demo)" sufiks) — zavedeno u `27-BACKLOG`, M5 sekcija. Dva mrtva mock fajla potvrđena grep-om (nula uvoznika): `BookingsTable.tsx`, `FiltersModal.tsx` (rezervacije/lista) — namerno neobrisani, ista vlasnikova odluka kao `mock-data.ts`.
