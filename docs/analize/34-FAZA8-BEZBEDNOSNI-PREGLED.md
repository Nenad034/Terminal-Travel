...
- ~~IDOR pregled (30.8.2026) je pokrio samo module koji koriste `resolveCallerIdentity`~~ — rešeno 31.8.2026: opšta `VIEW_ALL` konvencija (M1 spec §3.9a) implementirana u kodu za M5 (`booking`), M6 (`client-account`/`post-trip-survey`), M10 (`client-payment-schedule`), M14 (`ticket`), M20 (`client-contract`) — "podrazumevano svi vide sve, sužavanje pojedinačna opcija", umesto ranije neusklađene/nikad sprovedene "prodajni agent vidi samo svoje". 17 novih jediničnih testova ukupno preko svih pet modula, 213/213 e2e i dalje prolaze.
