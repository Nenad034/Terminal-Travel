...
| Akcentna boja (glavna, po modu — nije ista u oba, poglavlje 2.0a) | Teal (tamni, `#80CBC4`), zasićena plava (svetli, `#0069CC`) | Dugmad, aktivna stavka u bočnoj traci, otvorena `Ctrl+K` paleta, statusi koji traže pažnju — tačna VS Code vrednost po modu, poglavlje 2.0a |
...
| Aktivan tab — ivica/tekst | Ivica `#80CBC4` (akcentna teal), tekst `#FFFFFF`; neaktivan tab tekst `#5f7a87` | Gornja ivica `#000000` (**crna, ne akcentna** — VS Code svetli mod signalizira aktivan tab crnom linijom) |
| Selekcija teksta (pozadinsko bojenje) | `#80CBC420` (poluprovidna teal, ~12%) | `#0069CC40` (poluprovidna plava, 25%) |
| Red pod hoverom (lista/stablo) | `#192227` (**puna boja**, ista kao bočna traka — ne providan sloj) | `#00000014` (crna, providna, 8%) |
| Izabran red (lista/stablo) | Pozadina `#192227` (puna), **tekst postaje akcentna `#80CBC4`** — selekcija se signalizira bojom teksta, ne samo pozadinom | Pozadina `#00000025` (crna, providna, 15%), tekst ostaje `#202020` |
