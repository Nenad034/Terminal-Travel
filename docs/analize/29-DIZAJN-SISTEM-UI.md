...
**Verzija:** 1.56 — novo poglavlje 6c.0c (3.9.2026, na zahtev vlasnika): AI asistent dobija TREĆU poziciju — dno centralne kolone, strelicom iz desnog panela, kao „Move Panel" u VS Code-u. Polje ostaje JEDNO (`AiChatBox` se montira jednom i fizički premešta); portal je odbačen jer promena odredišta remontira komponentu i gubi nedovršen tekst/istoriju (izmereno u browseru). Pozicija se pamti po korisniku (`ai_dock_position`).
...
**Verzija:** 1.54 — ispravka poglavlja 6h (2.9.2026, isti dan, na zahtev vlasnika: "leva i desna strana vizuelno da budu iste"): Beleške, Komunikacija, Reklamacije, Predstavnici i Vlasništvo prelaze na ravan prikaz kao i leva kolona. Razlika nije bila namerna nego nedovršen posao — pravilo "okvir dobija samo ono na šta se klikne" jednako važi za obe kolone. Zadržan jedan **semantički** izuzetak: beleška predstavnika sa terena (M5 §4.6) ostaje izdvojena, ali levom trakom u boji upozorenja umesto punom karticom. Dodato i pravilo da sekcija koja već ima traku naslova ne sme da sadrži još jedan okvir — kutija u kutiji je najčešći način da se hijerarhija nehotice vrati na početak.
...
**Verzija:** 1.53 — dve dopune poglavlja 6h (2.9.2026, isti dan, na zahtev vlasnika). (a) **Nova podela kolona**: levo Aranžman, Putnici, Uplate, Povezano ("šta je kupljeno, ko putuje, da li je plaćeno" — proverava se pri svakom otvaranju), desno Beleške, Komunikacija, Reklamacije, Predstavnici, Vlasništvo ("šta se dešavalo oko rezervacije" — čita se kad nešto zapne). Novac je time uz sam aranžman umesto preko ekrana od njega. Poništava raniju podelu "sam zapis levo, sve oko njega desno". (b) **Nosilac rezervacije izdvojen** iz niza sa vlasnikom i zaduženim u sopstven red, podebljan i krupniji — to su tri različite osobe u tri uloge, a stajale su kao jedan niz tačkicama razdvojenih imena u istoj veličini; gost je onaj po kome se rezervacija traži.
...
**Verzija:** 1.52 — dve dopune poglavlja 6h (2.9.2026, isti dan, na zahtev vlasnika). (a) **Šifra se nikad ne prikazuje sama, ni naziv sam** — država, tip sobe i pansion se pišu kao naziv + šifra (`Grčka (GR)`, `Standardna soba (STD)`, `Polupansion (HB)`); naziv je za onoga ko čita ekran, šifra za razgovor sa dobavljačem i vaučer. Nepoznata šifra prolazi kao sirov tekst — pogrešan naziv je gori od nikakvog. Nazivi država iz `Intl.DisplayNames` (`sr-Latn`), ne iz ručne liste. (b) Kolone Pregleda su **jednake širine**, raniji odnos 1,5:1 poništen jer je desna kolona u međuvremenu dobila dovoljno sadržaja.
...
- **Strelica ka centralnom panelu** (`arrow-left`) u zaglavlju AI sekcije desnog panela premešta polje dole; **strelica nazad** (`arrow-right`) u dnu ga vraća. Isti obrazac kao „Move Panel" u VS Code-u.
- Donji dok zauzima **samo širinu centralne kolone** — ne ide ispod bočne trake ni desnog panela, ista pozicija i pravilo kao Terminal panel (§5f). Visina se prevlači i pamti; sklapa se na naslov.
- Dok je AI dole, desni panel **ne prikazuje AI sekciju** i njegov gornji deo dobija ceo prostor; dugme za sklapanje gornjeg dela (§6c.0b) se sakriva jer nema šta da postigne.
...
- **Filteri — u levom panelu, kao dodatna sklopiva sekcija, ne traka iznad rezultata** (ispravka, 18.8.2026, na zahtev vlasnika — isti obrazac kao VS Code Explorer bočna traka, gde se "Outline"/"Timeline" pojavljuju kao dodatne sklopive celine ispod/pored glavnog stabla, ne kao traka unutar centralnog editora). Dok je tab pretrage otvoren, levi panel dobija drugu sklopivu sekciju **"Filteri"** pored/ispod postojećeg stabla vođene pretrage (poglavlje 5b) — cena, kategorija/zvezdice, usluga, sadržaji-tagovi (M5 poglavlje 3.0c.3), sortiranje. Sklopiva/proširiva nezavisno od ostatka levog panela, isti ševron-obrazac (poglavlje 5). Izmena filtera osvežava rezultate u centru **u istom tabu** (poglavlje 5a — filteri su "izmena unutar tekućih rezultata", ne nova pretraga). Razlog premeštaja: centralni panel ostaje isključivo prikaz rezultata (isti princip kao poglavlje 5b, "centar = uvek lista ili pun zapis"), kontrole nad tim šta se prikazuje pripadaju navigatoru, ne mešaju se sa sadržajem.
  ...
  - **Sekcija filtera je JEDAN BLOK, bez linija** (dopuna, 3.9.2026, vlasnikova odluka kroz dva koraka istog dana). Prvi korak je uveo tanke linije između sekcija; vlasnik je na ekranu video da sedam grupa tagova u uskoj koloni daje sedam vodoravnih poteza koje oko čita kao rešetku, ne kao grupe, i predložio da umesto linija **cela sekcija dobije nijansu koja je do tada stajala samo iza naslova** (`--sunken`). Sada: naslov i tagovi dele istu podlogu, blok je zaobljen, blokove razdvaja razmak od 12px kroz koji se vidi podloga panela. Naslov ne gubi hijerarhiju time što deli podlogu sa svojim sadržajem — nose je velika slova, podebljanje i prored.
    - **Pilule tagova unutar bloka stoje na `--panel`, ne na providnoj podlozi.** Izmereno pre primene: okvir pilule na `--sunken` daje **2,94:1 u svetlom modu**, ispod 3:1 praga za granice (§2a) — tagovi bi postali jedva vidljivi. `--border` se namerno NE dira (važi za ceo panel, ista greška je dvaput ispravljana, vidi komentar uz token u `globals.css`); umesto toga se pilula **diže** na `--panel`, gde njen okvir daje 3,41:1 i prolazi. To je i ista Material logika visine koju `--sunken` već prati (§6h): sekcija tone, ono što se klikće se diže.
    - **Odnos podloga po modovima, izmereno:** blok naspram bočne trake je 1,25:1 u tamnom i 1,05:1 u svetlom modu (bočna traka je `--panel-2`, koja je u svetlom modu skoro ista nijansa kao `--sunken`); pilula naspram bloka je 1,16:1 u svetlom i 1,05:1 u tamnom. Nijedan od ta dva sloja, dakle, ne nosi grupisanje sam — nose ga zajedno naslov, razmak i uzdignute pilule. Ako se ijedan sloj jednom pokaže preslabim, lever je odnos `--panel-2` ↔ `--sunken`, ne vraćanje linija i ne `--border`.
  - **"Aktivnosti" — sopstvena, uvek otvorena grupa, ne sklopljena kao amenity tagovi** (dopuna, 5.9.2026, na zahtev vlasnika; podaci/logika: M5 poglavlje 3.0c.3e) — kad pretraga nudi filter po `ActivityTag` (M2 §2.1c), grupa se prikazuje bez ševrona za sklapanje, jer je ovo, po vlasnikovom nalazu, sve češće primarni kriterijum izbora destinacije, ne sporedan detalj koji se traži tek kad je destinacija već poznata. Nije pinovana u traci iznad rezultata kao brzi filteri ispod — razlika je namerna: brzi filteri odlučuju prodaju u trenutku zaključivanja, "Aktivnosti" odlučuje sam izbor destinacije, ranije u toku.
  - **Kontekstualni filteri (M5 poglavlje 3.0c.3d) nose suptilnu oznaku razloga, ne tiho nestaju** (dopuna, 5.9.2026) — mala info-ikonica uz filter koji se pojavljuje/nestaje po destinaciji ili mesecu (npr. "udaljenost od mora", "blizina ski lifta"), sa kratkim tekstom ("prikazano za ovu destinaciju/period"). Bez ove oznake, nestanak filtera pri promeni destinacije lako deluje kao kvar. Namerno bez posebne grupe/isticanja (za razliku od "Aktivnosti" iznad) — cilj je samo da nestajanje ne izgleda kao greška.
  - **Brzi filteri — vrlo vidljivi, nikad sklopljeni** (dopuna, 18.8.2026, na zahtev vlasnika; podaci/logika: M5 poglavlje 3.0c.3a) — Razlog izuzetka od pravila "sklopljeno dok se ne koristi" (iznad): ovo su najčešće odlučujući filteri pri prodaji (agent na telefonu sa gostom prvo pita "da li se otkazuje bez penala" i "koja je kategorija"), pa ne smeju čekati klik da se otkriju.
    - **Mesto je izmenjeno 3.9.2026 (vlasnikova odluka):** ne pinovani na vrhu sekcije "Filteri" u levom panelu kako je ovde prvobitno stajalo, nego u **vodoravnoj traci iznad rezultata u centralnom panelu**, uz traku za sortiranje. Isti razlog, doslednije sproveden — filter koji odlučuje prodaju treba da bude u vidokrugu čoveka koji gleda u listu, ne u koloni kroz koju se skroluje.
    - U traci stoje: **Refundabilno / Nerefundabilno** (M5 3.0c.3a) i **kategorija — zvezdice** (M5 3.0c.3c). **Odmah potvrda / Upit** ostaje u levom panelu, gde je i ožičen.
    - **Kategorija je jedan tag sa pet zvezdica, ali pet NEZAVISNIH prekidača, ne skala "N i više"** — klik na drugu i treću daje hotele sa 2 i sa 3 zvezdice. Red od pet zvezdica na svakom drugom mestu na internetu znači **ocenu**, pa se dvosmislenost lomi na tri načina odjednom: razmak između zvezdica, sopstvena akcentna podloga na izabranoj, i ispisan izbor uz red ("2, 3 zvezdice"). Oblik zvezdice (★ naspram ☆) sam je premala razlika da bi nosio stanje filtera.
    - **Prekidač se nudi samo tamo gde podatak stvarno postoji** — isto pravilo kao kod sortiranja (poglavlje 6d.2), nikad kao siva neupotrebljiva dugmad.
    - **Jedan red, grupe razdvojene uspravnom crtom** (dopuna istog dana): brzi filteri, sortiranje i prekidač lista/mapa dele istu traku. Crta (`h-4 w-px bg-border`) razdvaja grupe koje rade različit posao; fiksne je visine, ne `self-stretch` — u redu koji se prelama bi se razvukla preko dva reda. Nema sopstvenog tokena: granica je granica.
...
**Sprovedeno (2.9.2026).** Raspored i sva četiri pravila ponašanja rade. Dve stvari koje su pri implementaciji morale da žive IZNAD stranice, jer je `page.tsx` server komponenta koja se u potpunosti ponovo renderuje pri svakoj promeni query stringa (isti razlog i isto mesto u `Shell.tsx` kao `GroupSearchBuilderContext`): pamćenje kriterijuma po vrsti proizvoda i snimak prethodnih ponuda za poređenje — oboje u novom `SearchStateContext.tsx`. Desni panel je i pre ovoga bio iznad stranice (`SelectionProvider`), pa ga prelazak vrste strukturno nikad nije ni dirao.

**Ono što je ostalo neurađeno, namerno:** tabela polja po vrstama (M5 §3.0g.6) nije ugrađena — svako novo polje povlači izmenu u drugom modulu (M5 §3.0g.7), pa je to zaseban zadatak. Forme prikazuju isti skup polja kao pre preseljenja, u novom rasporedu.
...
Oblik: mala oznaka "sortiraj:" pa red **dugmadi** (§6f — mali, poznat skup opcija, ne padajući meni), aktivno dugme u `bg-accent-soft`/`text-accent-strong` sa `aria-pressed`. Jednostruk izbor: klik na već aktivno dugme ga **ne** deselektuje — lista uvek mora imati neki redosled, "nesortirano" nije smisleno stanje. Broj rezultata stoji desno, uz sam kraj trake.

Opcije se razlikuju po vrsti proizvoda i **prikazuju se samo kad za njih postoji podatak** — nikad siva, neupotrebljiva dugmad za nešto što sistem ne zna (npr. "po oceni gostiju" ne postoji jer TT ocenu gostiju uopšte nema, M5 §3.0g.9).
...
- **Konkretna komponenta za taj veliki skup (2.9.2026): `Command` u `Popover`-u** — `apps/panel/src/components/ui/command.tsx` + `ui/popover.tsx` (shadcn izvorni kod kopiran u repo, nad `cmdk` i `@radix-ui/react-popover`). Kucanje filtrira, strelice gore/dole biraju, Enter potvrđuje. Prvi ugrađen primer: birač sastojaka grupnog paketa (`PackageAttributesEditor.tsx`, M2). Do tog datuma je tu stajala ručno pisana lista sa sopstvenim `<input>`-om i `.filter()`-om — ne zbog dizajnerske odluke nego zato što `npm install` na razvojnoj mašini nije radio pa `cmdk` nije mogao da se instalira; kad se to popravilo, ručna lista je zamenjena. Razlika nije samo izgled: ručna lista nije imala kretanje tastaturom, pa je unos deset sastojaka paketa tražio deset odlazaka na miš. **Za nove ekrane sa velikim skupom opcija koristiti ovu komponentu, ne pisati novu listu.**
...
**Nosilac rezervacije je izdvojen iz reda sa vlasnikom i zaduženim** (2.9.2026, na zahtev vlasnika: "ime i prezime nosioca rezervacije treba da piše boldiranim slovima i većim fontom"). To su **tri različite osobe u tri različite uloge**, a stajale su u jednom nizu razdvojene tačkicama, u istoj veličini i boji. Gost je onaj po kome se rezervacija traži i koji zove telefonom; vlasnik i zaduženi su interna raspodela posla. Zato ime gosta ide u sopstveni red, podebljano i krupnije, odmah ispod broja rezervacije — čovek pamti ime, ne šifru.
**Obe kolone se ponašaju po istom pravilu** (ispravka 2.9.2026, na zahtev vlasnika: "leva i desna strana vizuelno da budu iste, ovako desno imamo zaokružene sektore a levo ne"). Do te ispravke su Beleške, Komunikacija, Reklamacije, Predstavnici i Vlasništvo zadržali okvir po redu, dok su Aranžman, Putnici i Uplate već bili ravan spisak — desna kolona je izgledala kao skup kartica, leva kao spisak. **To nije bila namerna razlika nego nedovršen posao:** pravilo iz ovog poglavlja ("okvir dobija samo ono na šta se klikne ili što je zaseban entitet") jednako važi za obe kolone, a te liste su čist prikaz.

**Jedan namerni izuzetak ostaje: beleška predstavnika sa terena.** Ona mora biti izdvojeno vidljiva (M5 spec §4.6, raniji vlasnikov zahtev) — to je **semantička** razlika, ne ukras. Ali umesto pune kartice nosi **levu traku u boji upozorenja**: signal ostaje, a red se i dalje uklapa u ritam spiska umesto da bude kutija u njemu. Isti obrazac važi svuda gde jedan red mora da se izdvoji iz inače ravne liste.
...
**Poznata cena ove odluke:** u tamnim modovima telo sekcije postaje najsvetlija površina na ekranu. Kod ekrana zapisa to su kratke liste, ne pun ekran teksta, pa je prihvatljivo — ali ako se obrazac ikad primeni na ekran sa velikom količinom sadržaja (npr. dugačka tabela preko cele visine), tu treba ponovo proceniti, jer velika svetla površina u tamnoj temi zamara.

Zajednički okvir sa `overflow-hidden` je ono što drži oblik: traka i telo nemaju sopstvena zaobljenja nego ih dobijaju od roditelja, pa se ne mogu razići kad se sadržaj sekcije promeni.
**Na ekranu sažetka nema glavne radnje, pa nema ni punog akcentnog dugmeta** (dopuna 2.9.2026, na zahtev vlasnika: "ovo zaokruženo nema razloga da bude ovoliko naglašeno"). Dugme "Pripremi/prikaži fiskalni dokument" stajalo je kao **puno akcentno dugme preko cele širine** unutar sekcije Uplate — najglasniji element na ekranu, glasniji od iznosa u sažetku, koji su razlog zbog kog se ekran uopšte otvara.

Pravilo koje iz toga sledi: **akcentna boja je rezervisana za glavnu radnju ekrana** (poglavlje 2). Ekran koji je po definiciji sažetak (M5 §4.5 — "ovde se ništa ne menja, samo se prikazuje") **nema glavnu radnju**, pa nijedno dugme na njemu ne sme da nosi pun akcent. Uzgredne radnje tu idu kao tihe veze u boji `--accent-strong`, poravnate desno uz sekciju kojoj pripadaju.
