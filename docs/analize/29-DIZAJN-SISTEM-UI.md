...
- **Tekst na `accent-soft` pozadini mora biti `accent-strong`, nikad `accent`** — nalaz iz live-provere (17.8.2026, M17 §3.1): `accent` na `accent-soft` daje samo **3.96:1** u svetlom modu i pada AA prag, dok `accent-strong` daje 5.98:1 (svetli) / 8.86:1 (tamni). Ovaj par se pojavljuje na više mesta u panelu (bedževi, statusne oznake, AI sažetak) — vidi `27-BACKLOG-IDEJA-I-PREDLOZI.md`, sekcija "Dizajn sistem UI", za spisak mesta koja još koriste stariju, slabiju varijantu.
