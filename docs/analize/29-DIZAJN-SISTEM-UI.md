...
**Ispravka, 19.8.2026, na zahtev vlasnika** ("vizuelno identično najnovijoj verziji VS Code-a") — raniji opis ispod je pričao o toploj narandžastoj/amber akcentnoj boji, ali konkretne HEX vrednosti nikad nisu tako upisane (poglavlje 2.0a uvek je nosio teal/plavu, direktno iz stvarnih VS Code tema). Vlasnik je potvrdio (19.8.2026) da važe **brojevi**, ne raniji narativ — tekst ispod je usklađen sa poglavljem 2.0a, ne obrnuto.

Polazna tačka: hladna teal/plava porodica, direktno iz dve zvanične, uparene VS Code teme ("Dark 2026"/"Light 2026", poglavlje 2.0a) — ne izmišljena kombinacija, doslovno ono što najnovija verzija VS Code-a pokazuje kad se ta tema izabere (potvrđeno 19.8.2026 uvidom u stvarno instaliran VS Code na razvojnoj mašini — `settings.json` vlasnika već ima `"workbench.colorTheme": "Light 2026"`, znači ovo je paleta koju svakodnevno gleda).

| Uloga | Ton | Napomena |
| :---- | :---- | :---- |
| Osnovna pozadina (tamni mod) | Skoro crna, blago plavo-siva (`#121314`) | Direktno iz VS Code "Dark 2026" — ne izmišljena "toplija od crne" varijanta kako je ranija verzija ovog dokumenta opisivala |
| Osnovna pozadina (svetli mod) | Čisto bela / vrlo blago hladno-bela bočna traka | Direktno iz VS Code "Light 2026" |
| Akcentna boja (glavna, po modu — nije ista u oba, poglavlje 2.0a) | Teal-plava (tamni), zasićena plava (svetli) | Dugmad, aktivna stavka u bočnoj traci, otvorena `Ctrl+K` paleta, statusi koji traže pažnju — tačna VS Code vrednost po modu, poglavlje 2.0a |
...
### 2.0a Konkretan izvor za Horizont v2 — zvanični, upareni VS Code par tema (ispravka, 19.8.2026, na zahtev vlasnika)

**Ispravka izvora — v2 je koristio pogrešan par.** Ranija verzija (17.8.2026) parila je "Light 2026" (zvanična, ugrađena) sa "Material Theme High Contrast" (**treća strana**, `equinusocio.vsc-material-theme` ekstenzija) za tamni mod — nedosledan par, i ne ono što "najnovija verzija VS Code-a" stvarno pokazuje van kutije. Ispravljeno: tamni mod sad koristi **"Dark 2026"** (`theme-defaults/themes/2026-dark.json`) — zvanična, ugrađena tema, isti par/ista porodica kao već korišćeni "Light 2026" (`2026-light.json`). Obe vrednosti izvučene **19.8.2026 direktno iz instaliranog VS Code-a na ovoj razvojnoj mašini** (`resources/app/extensions/theme-defaults/themes/`, verzija `a5b5009513`), ne iz sećanja — potvrđeno i uvidom da vlasnikov stvaran `settings.json` već ima `"workbench.colorTheme": "Light 2026"` aktivno podešeno, znači ovo jeste paleta koju svakodnevno gleda, ne pretpostavka.

| Uloga | Tamni (Dark 2026) | Svetli (Light 2026) |
| :---- | :---- | :---- |
| Pozadina (editor/glavni panel) | `#121314` | `#FFFFFF` |
| Bočna traka / gornja traka | `#191A1B` | `#FAFAFD` |
| Tekst (glavni, u centralnom panelu) | `#BBBEBF` (10.41:1 na pozadini) | `#202020` (16.29:1 na pozadini) |
| Tekst (glavni, u bočnoj/gornjoj traci) | `#BFBFBF` (9.48:1 na `#191A1B`) | `#202020` (isto polje, `sideBar.foreground`) |
| Tekst — sekundaran/opis | `#8C8C8C` **(5.18:1 na `#191A1B` — prolazi AA, ne dostiže AAA cilj)** → `#ABABAB` (7.83:1, ista siva porodica) | `#606060` (6.29:1 na beloj) |
| Akcentna boja (dugme/velika površina) | `#297AA0` (teal-plava, `button.background`) | `#0069CC` (plava, `button.background`) |
| Tekst na akcentnoj pozadini | `#FFFFFF` (4.79:1 na `#297AA0` — iznad 3:1 praga za velike kontrolne površine, poglavlje 2a) | `#FFFFFF` (5.39:1 na `#0069CC`) |
| Ivica/border | `#2A2B2C` | `#F0F1F2` |

**Ista korekcija-po-principu kao ranije, ne kozmetička:** izvorna vrednost sekundarnog/opisnog teksta u "Dark 2026" (`#8C8C8C` na `#191A1B`) daje **5.18:1** — prolazi tvrdi AA minimum, ali ne dostiže AAA cilj (poglavlje 2a: "ako neka kombinacija mora da bira između manje verne boje i manjeg kontrasta, bira se čitljivost"). Posvetljena unutar iste neutralno-sive porodice na `#ABABAB` (7.83:1) pre upisa ovde — identičan princip kao korekcija sprovedena u prethodnoj verziji ove tabele, sad ponovljena nad tačnim, ispravno uparenim izvorom. Akcentna boja na tamnoj pozadini (`#297AA0`, belo na njoj 4.79:1) prolazi i AA za obično telo teksta i 3:1 prag za velike kontrolne površine, ali ne AAA — isti obrazac kao već prihvaćeno za svetli mod (`#0069CC`, poglavlje 2a).

**Strukturna/interakciona vernost VS Code-u** (dopuna, 19.8.2026, na zahtev vlasnika — "izgled taba", "pozadinsko bojenje teksta") — pored boja, sledeće je preuzeto **doslovno** iz istih fajlova, ne aproksimirano:

| Element | Tamni (Dark 2026) | Svetli (Light 2026) |
| :---- | :---- | :---- |
| Aktivan tab — pozadina | `#121314` (ista kao glavna pozadina — aktivan tab "utapa se" u sadržaj) | `#FFFFFF` |
| Neaktivan tab — pozadina | `#191A1B` (ista kao bočna traka) | `#FAFAFD` |
| Aktivan tab — gornja ivica (indikator) | `#3994BC` (akcentna, teal-plava) | `#000000` (**crna, ne akcentna** — VS Code svetli mod signalizira aktivan tab crnom linijom, ne bojom akcenta) |
| Selekcija teksta (pozadinsko bojenje) | `#276782DD` (poluprovidna teal-plava) | `#0069CC40` (poluprovidna plava, ista boja kao akcent, 25% neprozirnosti) |
| Red pod hoverom (lista/stablo) | `#FFFFFF14` (bela, 8% neprozirnosti) | `#00000014` (crna, 8% neprozirnosti) — **princip je isti u oba moda**: uvek providna crna/bela preko postojeće pozadine, nikad nova, neprovidna boja |
| Izabran red (lista/stablo) | `#FFFFFF22` (bela, 13% neprozirnosti) | `#00000025` (crna, 15% neprozirnosti) |

Razlika u aktivnoj ivici taba (akcent u tamnom, crna u svetlom) i providno-crno/belo pravilo za hover/selekciju su namerno preuzeti tačno ovako — to je stvarna VS Code odluka, ne nešto što bi ovaj dokument sam izmislio da izgleda slično.
