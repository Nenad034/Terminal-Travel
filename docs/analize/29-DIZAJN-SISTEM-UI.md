...
| Individualni paketi | nije `Product.type` — otvara `Itinerary` tok (M5 poglavlje 3.0d.5), sastavljanje više pretraga u jedno putovanje. Dok je aktivan, preostalih 8 ikona postaju dugmad za dodavanje/uključivanje-isključivanje segmenta tog tipa (dopuna, 18.8.2026, na zahtev vlasnika); desni panel prikazuje cenu po segmentu i tekući (okvirni) zbir, isti obrazac kao poređenje selekcije van itinerara (M5 poglavlje 3.0e.3) — podaci/tok: M5 poglavlje 3.0.4 |
