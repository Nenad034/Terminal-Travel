...
- **MARS Connector — čitanje/pisanje ka postojećem back-office ERP-u (MARS/NeoLab)** (predlog vlasnika, dostavljen kao gotov draft dokument, 21.8.2026) — modul koji bi sinhronizovao cene/dostupnost, rezervacije i fakture iz MARS ERP-a u interni cache (za Metabase izveštaje) i, u kasnijim fazama, kontrolisano pisao nazad u MARS (human-in-the-loop za rezervacije/finansije). Odloženo pre spec-a: dokument tvrdi da MARS ostaje trajni sistem zapisa za rezervacije/cene/fakture, što je u direktnoj napetosti sa već specificiranim M3/M5/M10 (koji su zamišljeni kao sopstveni sistemi zapisa, ne prikazi nad MARS-om) — mora se razjasniti da li je Connector privremeni most ka gašenju MARS-a ili trajna arhitektura pored M3/M5/M10, pre nego što bilo šta od ovoga dobije broj modula i spec. Takođe otvoreno: da li živi kao deo M4 (adapter sloj) ili kao zaseban modul; sekcije 6/7 originalnog dokumenta (TTA API contract, MARS API detalji) su sam autor označio kao placeholder. Pun originalni predlog i analiza: `docs/analize/34-PREDLOG-MARS-CONNECTOR.md`.
