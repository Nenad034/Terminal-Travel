...
- **Vođena pretraga za 9 vrsta proizvoda** (poglavlja 3.0c/3.0d, 17.8.2026) — **delimično rešeno 22.8.2026** (M17 spec v1.79): popup po tipu + "chip sa dugmetom izmeni" na vrhu centralnog panela implementirani (`SearchCriteriaPopup.tsx`/`SearchCriteriaChip.tsx`), zamenjuju raniju stalno otvorenu formu. **Nalaz pri implementaciji:** popup namerno pokazuje samo 6 opštih polja (destinacija/datumi/broj osoba) — šest tip-specifičnih API parametara koje spec §11 v1.28 pominje kao već definisane (`origin_city`/`cabin_class`/`driver_age`/`duration_nights`/`cabin_type`/`trip_cost`) **nikad nisu stvarno stigli u `SearchQueryDto`** (provereno čitanjem `search-query.dto.ts`) — spec je opisao ciljno stanje, kod ga nikad nije sproveo. Dodavanje tih polja u popup bez odgovarajućeg API parametra bi bilo dekorativno, zato nisu dodata. Sledeći pravi korak: ožičiti tih šest parametara u `SearchQueryDto`/`SearchService`, tek onda proširiti popup da ih pokaže SAMO za odgovarajući tip.
