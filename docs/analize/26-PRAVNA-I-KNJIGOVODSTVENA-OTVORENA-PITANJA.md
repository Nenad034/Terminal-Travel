...
| B7 | **Obrada ličnih podataka gosta/nalogodavca kod spoljnog LLM provajdera** — da li treba potpisan DPA, da li se podaci koriste za treniranje modela, koliko dugo se čuvaju, gde se fizički obrađuju (rezidencija podataka), i da li treba mehanizam za prenos van EU (npr. Standardne ugovorne klauzule) — pitanje se ponavlja za **svakog** provajdera pojedinačno (dopuna 19.8.2026 — sad konkretno dva: Anthropic primaran, OpenAI rezervni, M15 §6.8) | `31-AI-RIZIK-PRAVNA-ODGOVORNOST-OSIGURANJE-USKLADJENOST.md` poglavlje 5 | Proveriti pre nego što bilo koji domenski agent koji dodiruje lične podatke gosta/nalogodavca (M6, M7 subagent chat, M14, M21, M23) pređe u `ACTIVATED` status u produkciji. |
| B8 | **Transparentnost online cenovnika (mašinski čitljiv format) i zabrana lažnih recenzija** — Zakon o zaštiti potrošača, istraženo avgust 2026 — proveriti primenljivost na M2 katalog i M8 sajt pre implementacije bilo kog javnog prikaza cene/recenzije | `03-SPECIFIKACIJA-M2-KATALOG-PROIZVODA.md` poglavlje 9 | Otkriveno 19.8.2026 da je stavka do tada postojala samo u backlog indeksu (`27`), ne i u izvornoj specifikaciji — sada upisana na oba mesta. |
| B9 | **Povećanje cene posle potvrde rezervacije** — zakon navodno dozvoljava samo za gorivo/takse/kurs, ograničeno na 8% ukupne cene, uz pisano obaveštenje gostu najmanje 20 dana pre polaska; trenutno ne postoji nikakav koncept izmene cene posle `booking.confirmed` | `06-SPECIFIKACIJA-M5-REZERVACIJE.md` poglavlje 13 | Realna pravna izloženost dok se ne implementira ili svesno ne odluči da agencija cene nikad ne menja posle potvrde. |
| B10 | **"Opravdan razlog" izuzetak pri otkazivanju od strane gosta** — `cancellation_refund_percentage` preuzima dobavljačevu politiku otkazivanja bez izuzetka za iznenadnu bolest/smrt u porodici/višu silu (gost bi tad trebalo da dobije nazad sve osim stvarnih troškova) | `06-SPECIFIKACIJA-M5-REZERVACIJE.md` poglavlje 13 (odnosi se na poglavlje 6) | Zahteva potvrdu pravnika pre spec dopune. |
| B11 | **Minimalan broj učesnika kod izleta** — `min_participants` (M2) postoji samo kao opisno polje, nema procesa automatskog otkazivanja/punog povraćaja niti rokova obaveštenja (20/7/2 dana zavisno od dužine putovanja) iz zakona | `06-SPECIFIKACIJA-M5-REZERVACIJE.md` poglavlje 13 | Trenutno van obima za `PACKAGE`/`ACCOMMODATION` proizvode. |
| B12 | **Rok povraćaja novca** — `RefundInstruction` prati status ali nema rok/deadline polje niti alarm; zakon navodno traži povraćaj u roku od 14 dana od otkazivanja | `07-SPECIFIKACIJA-M10-FINANSIJE.md` poglavlje 12 | Isti obrazac kao `buyer_acceptance_deadline` (M10 poglavlje 6), mehanička dopuna kad se rok potvrdi. |
| B13 | **Rok gosta za prijavu reklamacije** — poglavlje 3 M14 prati 8-dnevni rok agencije za odgovor, ali ne ograničava kada gost sme da otvori `REKLAMACIJA` tiket u odnosu na datum povratka; zakon navodno daje gostu 15 dana posle povratka | `14-SPECIFIKACIJA-M14-HELPDESK.md` poglavlje 8 | — |
...
2. **Pravnik, jedan sastanak:** B1–B13 — B1 (gotovina/AML) je najurgentnija jer je postojeća zaštita već uklonjena iz sistema; B7 (LLM provajder) treba proveriti pre bilo koje aktivacije agenta koji dodiruje lične podatke, ne samo pri prvom sastanku; B8–B13 su svih šest iz istraživanja Zakona o zaštiti potrošača (avgust 2026) i prirodno idu zajedno kao jedna celina istog sastanka.
