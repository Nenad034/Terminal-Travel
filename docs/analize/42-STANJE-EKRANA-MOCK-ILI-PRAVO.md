...
| Ruta                                 | Status                   | Dokaz                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| :----------------------------------- | :----------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/rezervacije/lista`                 | **DELIMIČNO**            | `page.tsx` zove pravo `GET /sales/bookings`. Ali `RealBookingsTable.tsx:18,86-99` uvozi `deriveEmail`/`derivePhoneFromSeed` iz `mock-data.ts` i IZMIŠLJA `buyerEmail`, `buyerPhone`, `branch`, `assignedUser`, `hotelName`, `supplierName/Email/Phone` — API ta polja još ne vraća. Vidljivo obeleženo u kodu (komentar „Demo (bez pravog izvora)") i delimično na ekranu (`hotelName` dobija sufiks „(demo naziv)", `supplierName` „(demo)"), ali `buyerEmail`/`buyerPhone`/`branch`/`assignedUser` NEMAJU vizuelnu oznaku — izgledaju kao pravi podaci.       |
...
1. **`/rezervacije/lista` — `buyerEmail`/`buyerPhone`/`branch`/`assignedUser`.** Jedino mesto gde je izmišljen podatak BEZ vizuelne oznake na samom ekranu (za razliku od „hitno" zvona i `hotelName`, koji su self-disclosed). Nizak neposredan rizik jer ništa danas ne šalje mejl/poziv na osnovu ovih polja automatski — ali ako se doda funkcija koja to radi (npr. "pošalji podsetnik kupcu" dugme), mora prvo da se zameni pravim API poljem. Zavedeno ovde da se ne zaboravi.
