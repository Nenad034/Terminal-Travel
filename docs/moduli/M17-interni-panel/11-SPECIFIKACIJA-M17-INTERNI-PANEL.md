...
- [ ] Omnisearch polje je dostupno sa svakog ekrana; prazan upit + Enter prikazuje navigaciju filtriranu na ulogu; upit sa tekstom vraća rezultate/AI odgovor koji nikad prekoračuju prava trenutnog korisnika (poglavlje 5.5). **Delimično** — prazan upit + Enter radi (komandna paleta, `Ctrl+K`); upit sa tekstom trenutno radi lokalnu pretragu kroz navigaciju (privremena zamena), ne pravi `POST /ai-orchestration/omnisearch` — **čeka M15**, koji još ne postoji u kodu.
