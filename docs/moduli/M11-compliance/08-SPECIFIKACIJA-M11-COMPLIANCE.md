...
- **`assessForBooking` (§2.2, blokada potvrde ORGANIZATOR rezervacije preko limita garancije) i dalje baca grešku ako booking-ova valuta nema uneti `ExchangeRateSnapshot`** — otkriveno avgust 2026 kao kolateralna posledica istrage 404 bagа na `GET /travel-guarantee/utilization` (vidi promenu verzije): za razliku od `computeUtilizedAmount` (prikaz, ispravljeno), ovde bi tiho preskakanje značilo da rezervacija čiju vrednost sistem ne može da konvertuje prođe kroz zakonsku proveru limita BEZ stvarne provere — pogrešan pravac opreza. Zahteva odluku vlasnika: ili osigurati da kurs uvek postoji pre potvrde (npr. tvrd zahtev da `ExchangeRateSnapshot` bude unet za svaku aktivnu valutu), ili eksplicitno definisati bezbedno ponašanje kad kurs nedostaje (npr. blokirati potvrdu umesto propuštati je) — ne pretpostavljati jedno od to dvoje bez potvrde.
