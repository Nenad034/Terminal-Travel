...
**Verzija:** 1.10 — Straničenje + filteri na serveru za `GET /knowledge/articles` (8.9.2026, dok. 27 nastavak nalaza 2.2 iz dok. 39) — raste sa svakim novim člankom; ekran `/znanje` je `subjectType`/`status` filtrirao KLIJENTSKI nad celom listom (API ih nije ni izlagao kao query parametre). Oba filtera sad idu na server ZAJEDNO sa straničenjem — `status` filter poštuje postojeću ogradu §3.1 (ko nema `M23/article/EDIT` ne može tražiti ništa osim `PUBLISHED`, bez obzira šta pošalje kao parametar). Odgovor sad `{ data, total, page, limit, pageCount, hasMore }`. **Provera:** novi testovi u `articles.service.spec.ts`, `tsc --noEmit` čist, puna jest suita prolazi; `m23-exit-criteria.e2e-spec.ts` ažuriran (test je čitao `listSales.body.map(...)` — sad `listSales.body.data.map(...)`), 9/9 prolazi.

**Verzija:** 1.9 — novo poglavlje 3.2b (7.9.2026, vlasnikova odluka: "jedan AI asistent"): RAG tehnika (§3.2a) izdvojena u zajednički `AssistantEngineService`, deljen sa M21 (spec §5.2b) — ponašanje M23 se ne menja, spojena je isključivo mehanika selekcije/Anthropic poziva.
...
### 3.2b Deljena tehnika sa M21 (`AssistantEngineService`, 7.9.2026, vlasnikova odluka: "jedan AI asistent")

Revizija koda (`docs/analize/39-REVIZIJA-KODA-NALAZI-I-PREDLOZI.md`, nalaz 3.5) je pokazala da su M23 `KnowledgeAssistantService` i M21 `HelpAssistantService` bile dve gotovo identične implementacije iste RAG tehnike (§3.2a: embedding/ključne reči selekcija, Anthropic poziv sa prompt-injection ogradom) nad različitim sadržajem. Spojena je isključivo ta tehnika u zajednički `AssistantEngineService` (`apps/api/src/modules/m15-ai-orkestracija/assistant-engine/`) — puno obrazloženje i tačna podela odgovornosti su u M21 spec §5.2b (isti tekst, ne duplira se ovde).

**Za M23 se ništa u ponašanju ne menja**: `KnowledgeAssistantService` i dalje učitava SVE `PUBLISHED` `Article` zapise (bez audience filtera, §3.1), predaje ih motoru sa tabelom `article_translations` i sopstvenim `systemPrompt`-om (§3.2), i i dalje nudi "zahtev za istraživanje" (§3.3) umesto M21 tiketa. `AssistantCandidate.isPriority` (M21 "kritičan primer", §3.2a napomena) M23 nikad ne postavlja.

**Provereno:** jedinični testovi za deljenu tehniku sada postoje jednom (`assistant-engine.service.spec.ts`), M23-specifični test (`knowledge-assistant.service.spec.ts`) proverava samo M23-ovo (bez audience filtera, jezički fallback, ispravna tabela za embedding). Pun API `tsc`/lint čist, 1.059 unit testova i M21+M23 e2e paketi (15 slučajeva) prolaze nad pravom bazom posle izmene.

### 3.3 Kad agent ne zna — predlog istraživanja, ne tiketing

Za razliku od M21 (eskalira ka M14 tiketu), ovde nedostatak odgovora znači da **sam sadržaj** nedostaje, ne da je nešto pokvareno. Ako `confidence = NONE`, agent nudi: _"Nemam pouzdan odgovor iz objavljenih članaka — da pokrenem istraživanje iz odobrenih izvora?"_ Potvrda korisnika (`POST /questions/:id/request-research`) je nivo autonomije `AUTONOMOUS` (korisnik potvrđuje sopstveni zahtev, ne tuđu akciju).
