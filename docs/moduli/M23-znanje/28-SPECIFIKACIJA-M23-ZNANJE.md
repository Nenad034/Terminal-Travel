...
- **Endpoint za istraživanje nad postojećim člankom** — `researchFromProvidedText` je dostupan samo pri kreiranju članka (`POST /knowledge/articles`), nema `POST /knowledge/articles/:id/research` za osvežavanje/dopunu postojećeg. Otkriveno pri M17 Faza 7 implementaciji (panel `/znanje/[id]/revizije` prikazuje napomenu o ovom nedostatku umesto forme) — dodaje se u narednom, potvrđenom prolazu.
