...
- **Lighthouse / Cubilis (bivši Stardekk) — istražen na zahtev vlasnika 4.9.2026 povodom `bookingengine.mylighthouse.com/v2/terme-topolsica-topolsica`, NIŠTA NIJE USVOJENO.** Izvori: `mylighthouse.com` (proizvodna strana „Direct Bookings"), Hospitality Net/HotelTechReport o akviziciji Stardekk-a (februar 2024), Wikipedia profil kompanije, G2 Travel saopštenje o partnerstvu sa Stardekk/Cubilis, SmartHotel/Peliqan konektivna uputstva.

  **Prvo razgraničenje — ovo NIJE dobavljač ponude kao Travelgate/HyperGuest.** Link koji je vlasnik poslao je **B2C booking engine jednog hotela** (Terme Topolšica), tj. stranica na kojoj gost rezerviše direktno; `?message=SessionLost` je samo istekla sesija u pregledaču, ne greška sistema. Sam booking engine nema agencijske cene i nije mesto sa kog se kupuje kao agencija — kupovina preko njega bila bi kupovina „kao gost". Iza njega stoji **Lighthouse** (britanska kompanija, bivši OTA Insight, osnovana 2012. u Gentu), koja je u februaru 2024. kupila belgijski **Stardekk** i time i **Cubilis** — channel manager + booking engine za nezavisne hotele.

  **Gde je onda stvarna veza za TT — Cubilis kao channel manager, ne booking engine.** Cubilis povezuje hotel sa 200+ prodajnih kanala i „400+ partnera" dvosmernom XML vezom; među tim partnerima su i tur-operatori (javno saopšten primer: **G2 Travel**). Dakle put nije „kupiti API od Lighthouse-a", nego **postati kanal/partner na Cubilis-u** — posle čega bilo koji hotel koji koristi Cubilis može da uključi TT kao kanal jednim podešavanjem, bez posebne integracije po hotelu. XML kredencijali se traže od njihove podrške; javne tehničke dokumentacije nema (isti gejt kao HyperGuest i Travelfusion).

  **Arhitektonski nalaz, važi nezavisno od ove odluke: `ProviderAdapter` iz poglavlja 2 je isključivo PULL model** (`search` → `checkAvailabilityAndPrice` → `confirmBooking`, mi pitamo, provajder odgovara). Veza sa channel manager-om je **PUSH**: hotel šalje raspoloživost i cene čim se promene, a mi vraćamo rezervacije. Za to danas u M4 ne postoji ništa — ni endpoint koji prima ARI ažuriranja, ni model gde bi se ta ponuda čuvala (bliže M3 alotmanu nego M4 živoj pretrazi, jer je reč o zalihama koje hotel gura unapred, ne o upitu u trenutku pretrage). Isti nalaz važi i za HyperGuest (i oni nude PUSH, zapis iznad). **Ako se ijedan push-izvor ikad usvoji, M4 dobija drugu vrstu ulaza, ne još jedan adapter po postojećem ugovoru** — to je izmena specifikacije, ne implementaciona sitnica.

  **Za konkretan slučaj (Terme Topolšica ili bilo koji pojedinačan hotel):** jeftiniji i brži put je redovan ugovor u M3 (alotman/cenovnik ručno), a automatizacija tek ako se hotel sam ponudi da nas uključi kao kanal. Postojeći **WebHotelier adapter (poglavlje 5b)** je presedan za „hotel sa sopstvenim booking engine-om", ali on radi zato što WebHotelier izlaže **Travel Agent API sa agencijskim cenama** — kod Lighthouse/Cubilis ekvivalent tome je partnerski XML, ne booking engine sa linka.

  **Šta bi trebalo saznati pre bilo kakve odluke:** (1) da li Cubilis uopšte prima nove partnere veličine TT i pod kojim uslovima (tehnički i komercijalni); (2) koliko hotela u našim destinacijama stvarno koristi Cubilis — u Sloveniji/Hrvatskoj/Crnoj Gori, jer je snaga ovog kanala u nezavisnim hotelima, ne u lancima; (3) da li je model komisioni ili NET (ista posledica na M10 §4.4 kao kod HyperGuest-a); (4) da li se traži sertifikacija/testni prolaz pre puštanja u rad, što je kod channel manager-a pravilo, ne izuzetak.
- **HyperGuest — istražen na zahtev vlasnika 4.9.2026, NIŠTA NIJE USVOJENO** (isti gejt kao Travelfusion/D-EDGE zapisi: bez implementacione napomene dok ne postoji stvaran poslovni dogovor). Izvor: `hyperguest.com` (glavna strana, FAQ za „demand" stranu, Integration Center), AltexSoft pregled direktnih B2B hotelskih API-ja, PhocusWire/Hospitality Net o Series A rundi, Shiji saopštenje o partnerstvu (avgust 2025).

  **Šta je.** B2B distribucioni marketplace koji povezuje hotel DIREKTNO sa kupcem (agencija, tur-operator, OTA), bez bedbanke/veletrgovca u sredini — sami to zovu „Distribution 2.0". Za nas je kategorijski **isto mesto u arhitekturi kao Travelgate/Solvex** (izvor smeštajne ponude preko API-ja), ne nova vrsta sistema: ako se ikad usvoji, to je još jedan `ProviderAdapter` po ugovoru iz poglavlja 2, ne izmena M4.

  **Deklarisan obim (njihovi brojevi, neprovereni):** 25.000+ živih objekata, 380+ kupaca („demand" strana), 150+ tehnoloških integracija; osnovani 2020 (Tel Aviv, kancelarije Barselona/Buenos Ajres/Majami/Indija), Series A 23 M$ (jul 2023, Lightspeed/Viola/Thayer), ukupno ~43 M$. Partnerstvo sa Shiji Horizon (avgust 2025) im širi dovod hotela.

  **Komercijalni model — ključna razlika prema bedbankama.** Hotelu je besplatno (ne uzimaju proviziju od ponude); **naplaćuju kupcu**, „à la carte", cena po dogovoru, bez javnog cenovnika. Rate dolaze u dva oblika i to je bitno za nas: **NET** (agencija plaća hotelu, sama formira prodajnu cenu — nastupanje kao ORGANIZATOR) i **komisiona** (hotel naplaćuje gostu, agenciji isplaćuje proviziju po odjavi — nastupanje kao POSREDNIK). **Posledica za M10:** ta dva oblika ne daju isti poreski tretman (M10 §4.4 — marža naspram provizije), pa bi adapter morao da mapira vrstu rate u `ProviderConfig.default_tip_nastupanja` (poglavlje 3.1) po ponudi, ne po provajderu — što ni jedan postojeći adapter ne radi. To je najveća nepoznanica pre bilo kakve odluke, veća od tehničke integracije.

  **Tehnički.** Sopstveni „unified" API, PULL i PUSH, bez ograničenja look-to-book odnosa (za razliku od TravelgateX/HotelX, gde je L2B izričito ograničenje — vidi zapis iznad); deklarišu odgovor < 500 ms. **Javna tehnička dokumentacija ne postoji** — integracija ide preko njihovog tima („zero-coding" pristup, mejl pa oni rade), što znači da se **oblik API-ja ne može proveriti pre potpisivanja ugovora**. Isti gejt kao Travelfusion: nema self-serve sandboxa, nema javne cene.

  **Šta bi trebalo saznati pre bilo kakve odluke** (redosled po važnosti): (1) pokrivenost hotela u destinacijama koje TT stvarno prodaje — Grčka (Halkidiki, Krit), Crna Gora, Turska, Egipat, Tunis; njihovih 25.000 objekata je globalan broj, ne obećanje za naše tržište; (2) cena za kupca (fiksna pretplata? po rezervaciji? procenat?) — bez toga se ne može uporediti sa postojećim izvorima; (3) da li se ista soba pojavljuje i preko Travelgate-a/Solvex-a i preko njih, i po kojoj ceni — inače dodajemo poziv i trošak bez nove ponude; (4) postoji li test okruženje pre potpisa; (5) ko je ugovorna strana prema gostu po komisionoj rati (utiče na M20 ugovor sa klijentom i M11 garanciju putovanja).

  **Procena bez odluke:** tehnički bi bio treći-četvrti adapter po redu i ne bi tražio izmenu M4 (isti interfejs, verovatno `API_KEY`/`OAUTH2` strategija iz poglavlja 2.2). Poslovno je otvoreno pitanje: vredi tek ako donosi hotele koje postojeći izvori nemaju, ili bolju cenu na iste. Preporuka je da se pre ijednog reda koda traži pokrivenost po našim destinacijama i cenovnik — oba su pitanja za njihovu prodaju, ne za razvoj.
