...
- [ ] WebHotelier adapter implementiran, ispunjava `ProviderAdapter` interfejs u potpunosti (poglavlje 5b); `BASIC` auth strategija radi bez izmene postojeće implementacije. *(nije počelo — čeka stvarne Travel Agent kredencijale za konkretan hotel, §9; može krenuti u `use_mock` režimu i pre toga)*
- [ ] WebHotelier `cancellation_fees` (fiksan iznos + datum) ispravno mapiran u `{ days_before_stay, refund_percentage }` (poglavlje 5b) — test: poznat primer sa `fee`/`after`/`price` daje očekivan `refund_percentage`, zaokruživanje ne baca izuzetak na ivičnim vrednostima (0%, 100%).
