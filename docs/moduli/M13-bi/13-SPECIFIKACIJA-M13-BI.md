...
**Verzija:** 1.16 — "Dinamički" red ikonica svodi se na dva reda ukupno, ručno polje za dimenzije postaje klik-dugmad (6.9.2026, vlasnikov nalaz: "recenica po vrsti proizvoda pre ikona treba da se ukloni. Iza ikona staviti polje za pretragu po pojmu (ali ga osposobiti jer i dalje ne radi) i na krahu stavite strelicu za akciju ali na kraju reda. Dakle ukupno dva reda"). (1) Rečenica "po vrsti proizvoda (država → mesto → proizvod)" iznad ikonica uklonjena — obrazloženje ikonica ostaje samo kroz `title` (hover) na svakoj. (2) Slobodno tekstualno polje "dimenzije" (v1.9–v1.15, "destination_country,destination_city...") zamenjeno novim `DimensionsPicker.tsx` — dugmad za svih šest poznatih dimenzija (`DYNAMIC_DIMENSIONS`), klik dodaje/uklanja dimenziju iz niza i ODMAH šalje formu (`e.currentTarget.form?.requestSubmit()`, dugme unutar `<form>` uvek zna svoj `.form`); brojka pored aktivne dugmadi pokazuje njen redosled u nizu (poredak BIRA nivoe drill-down-a — "država→mesto→proizvod" nije isto što i "proizvod→država→mesto", zato dugmad, ne obični `<select>` bez pojma o redosledu). Rešava i uzrok ranijeg "ne radi" utiska (v1.14/v1.15) — slobodan tekst je dozvoljavao kucnute greške u imenu dimenzije koje su tiho vodile u generičku grešku izveštaja bez objašnjenja; dugmad čine pogrešnu vrednost nemogućom. (3) Za "Dinamički" tab sad tačno DVA reda ukupno u formi: red 1 (period/odnosi se na/segment, nepromenjeno iz v1.15), red 2 (ikonice → `DimensionsPicker` → dugme akcije, sve u istom flex-wrap redu, dugme UVEK na kraju). **Provera:** `tsc --noEmit` čist za `apps/panel` (bez promene na `apps/api`).
**Verzija:** 1.15 — Ispravke posle prve provere v1.14: red period/odnosi se na/segment garantovan u jednom redu, kontrast padajućih menija u tamnom režimu, ikonice bez vidljivog naziva, automatska primena filtera (5.9.2026, vlasnikov nalaz uz nov snimak ekrana: "polje za datum i dva ispod tog polja treba da budu u istom redu, istih sirina, u druga dva polja kontarst izmedju pozadine i teksta nije dobar pa se slova ne vide u tamnom modu... pored ikona za vrstu uskuge uklonite nazive samo neka ostane pojavljivanje kada se predje misem preko ikone... u istom redu stavite i strekicu... umesto ove strelice zelim ovakvu ► ali ravnokraku... polje destination_country,destination_city i dalje ne reaguje na pretragu po pojmu"). (1) Ceo filter red izdvojen iz `page.tsx` (server komponenta) u nov `IzvestajiFilterForm.tsx` (klijentska komponenta) — jedini način da polja REAGUJU odmah na promenu: forma dobija `onChange` sa automatskom primenom, isti obrazac ("automatska primena filtera") kao `rezervacije/lista/RealFilterBar.tsx` — `<select>` primenjuje odmah, tekstualna polja (dimenzije, dobavljač, provajder, država/destinacija) čekaju 600ms pauzu od poslednjeg tastera. "Primeni filter" dugme ostaje kao ručni fallback (napomena: klik na dan u kalendaru period-polja i klik na predlog iz padajuće liste NE prolaze kroz native `change` događaj — ostaju svesan, prihvaćen izuzetak, kucanje pre klika i onako zakazuje automatsku primenu). Deljene konstante/`SearchParams` premeštene u nov `constants.ts` (uvozi ih i `page.tsx` i `IzvestajiFilterForm.tsx`). (2) Period + "odnosi se na" + "segment" sad u EKSPLICITNOM redu od tri jednaka `flex-1` polja (ne opšti flex-wrap sa ostatkom forme, koji je zavisio od širine ekrana i broja OSTALIH polja pa je period ispadao sam u svom redu). (3) `<option>` u svakom `<select>` dobija eksplicitan `bg-panel text-ink` — bez toga native padajući meni (popup) u tamnom režimu često ne nasleđuje pozadinu/tekst roditelja, slova su bila nevidljiva (tamna na tamnoj/svetla na svetloj, zavisno od pregledača). (4) Ikonice vrste proizvoda ("Dinamički") gube vidljiv naziv ispod/pored — ostaje SAMO `title` (native hover tooltip, već je postojao). Dugme "primeni filter" premešteno u ISTI red kao ikonice za taj tab (ranije na dnu forme, posle svih ostalih polja). (5) Ikonica akcionog dugmeta menja se sa `arrow-right` (v1.14 — ima vidljiv "stubić", nije čist trougao) na `play` (pravi ravnokraki trougao ►, kako je vlasnik tražio) — na SVIM istim dugmadima kao v1.14 (`izvestaji`, `RealFilterBar`, `CalendarFilterBar`, `FiltersModal`, `b2b/novi`, `email`, `nadzor`, `podrska`, `pomoc`, `pomoc/pitanja`, `znanje`). **Provera:** `tsc --noEmit` čist za `apps/panel` (bez promene na `apps/api`).
**Verzija:** 1.14 — Filter red izveštaja preuređen: jednoobrazna polja, naziv unutar polja, ravnomerna širina, `dateField`/`segment`/`channel`/`productType` kao pravi `<select>`, država/destinacija sa predlogom (5.9.2026, vlasnikov nalaz uz snimak ekrana: "razlikuju se polja... polja u kojima se kuca ne reaguju a tu treba da vec postoje podaci koji se biraju... nazive polja stavite unutar polja ne iznad ili ispod... nema razloga da 'odnosi se na' i 'segment' budu drugaciji vizuelno od ostalih polja... ravnomerno rasporedite sirinu svih drugih polja prema velicini ekrana"). (1) Nov `FieldInline.tsx` (ivica+pozadina UNUTAR koje stoji sitan naziv polja + sam unos, isti oblik za SVAKO polje) zamenjuje raniji `Field` (naziv IZNAD polja) — `flex-1 min-w-[140px]` po polju raspoređuje širinu ravnomerno preko širine ekrana (flex-wrap, isti princip kao `RealFilterBar.tsx`). (2) `dateField` ("odnosi se na") i `segment` više NISU `<Link>` segmentovana dugmad (v1.12/v1.13) nego pravi `<select>` — istog izgleda kao svako drugo polje, i ne trebaju im više skrivena polja/href-helperi da prežive "primeni filter" (uklonjeni `dateFieldHref`/`segmentHref`). (3) "Kanal" (profitabilnost/prodaja) i "tip proizvoda" (prodaja) postaju `<select>` sa poznatim, fiksnim vrednostima (`CHANNEL_OPTIONS`/`PRODUCT_TYPE_OPTIONS` — M5 `M5Channel`/M2 `ProductType`) umesto slobodnog teksta koji ništa nije predlagao. (4) Država/destinacija dobijaju predlaganje dok se kuca — nov `FilterLocationFields.tsx` (kombinuje oba polja, `GET /sales/search/destinations` zahteva `country`) koristi ISTI `SuggestField.tsx`/`/api/search-suggest` mehanizam kao ekran pretrage; `SuggestField.tsx` dobija opcioni `inputClassName` (podrazumevano nepromenjeno `input w-full`) da bi mogao providno da se uklopi u `FieldInline` bez dve ivice jedna preko druge. **Poznat, svesno odložen nedostatak** (ne ćuti se): "dobavljač (ID)"/"provajder (M4)" ostaju slobodan tekst — nemaju danas gotov predlagač (M3 dobavljač/M4 provajder lista), dodavanje bi tražilo nov API poziv, van obima ovog prolaza. (5) Dugme "primeni filter" postaje kvadratni tag sa ikonicom strelice (`arrow-right`) umesto teksta (5.9.2026, vlasnikov zahtev: "sva dugmad koja pozivaju na akciju (narandzasta) umesto slova treba da imaju zatvorenu strelicu sa vrhom prema desno... skratite to dugme na cetvrtasti tag sa strelicom unutra") — isto primenjeno na SVE istovrsne filter-trake tastere u panelu (`RealFilterBar.tsx`, `CalendarFilterBar.tsx`, `FiltersModal.tsx`, `b2b/novi/page.tsx`, `email/page.tsx`, `nadzor/page.tsx`, `podrska/page.tsx`, `pomoc/page.tsx`, `pomoc/pitanja/page.tsx`, `znanje/page.tsx`) — puni detalj u dizajn dok. (isti dan). **Namerno IZOSTAVLJENO** iz ovog kruga: veliko, samostalno dugme "pretraži" u `SearchCriteriaForm.tsx` (glavni CTA vođene pretrage, već nosi ikonicu+tekst, druga težina) i "primeni filtere" u `SearchSidebarPanel.tsx` (outline stil, ne puna narandžasta pozadina) — nisu filter-trake utility dugmad kao ostatak liste. **Provera:** `tsc --noEmit` čist za `apps/panel` (bez promene na `apps/api`).
**Verzija:** 1.13 — Polje "od/do" u izveštajima postaje jedan kalendar sa dva meseca, isti kao kod pretrage (5.9.2026, vlasnikov zahtev: "od do datum stavite u jedno polje kao kod pretrage"). Dva odvojena `DateField` ("od"/"do", v1.12) zamenjena JEDNIM dugmetom (`DateRangeField.tsx`, isti već postojeći dvomesečni kalendar koji koriste `SearchCriteriaForm.tsx`/`CalendarFilterBar.tsx`, `showNightsAndQuick={false}` — ovo su nezavisne granice, ne "boravak", isti razlog kao "Dolazak od/do" tamo). Nov klijentski omotač `apps/panel/src/app/(app)/izvestaji/PeriodRangeField.tsx` (isti obrazac kao `DynamicTree.tsx` — `izvestaji/page.tsx` ostaje server komponenta, `DateRangeField` je kontrolisana komponenta pa joj treba lokalno React stanje) drži `from`/`to` i prosleđuje ih kroz skrivena polja (`nameFrom`/`nameTo`) da ih ISTA prava GET forma i dalje ponese pri "primeni filter" — bez izmene API ugovora ni `dateField`/`segment` mehanizma iz v1.12. **Provera:** `tsc --noEmit` čist za `apps/panel` (bez promene na `apps/api`).
