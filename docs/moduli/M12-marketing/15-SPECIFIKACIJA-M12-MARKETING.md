...
| target_channels             | niz enum: `M8_SITE`, `FACEBOOK`, `INSTAGRAM`, `EMAIL`, `MOBILE_PUSH` _(dodato pri specifikaciji M19)_, `B2B_SUBAGENTS` _(v1.7, poglavlje 3d)_ | `MOBILE_PUSH` koristi već postojeći mehanizam push notifikacija iz M9 (M9 specifikacija, poglavlje 5), ne novu infrastrukturu                                                                                                 |
...
| offer_booking_to            | date, nullable _(v1.7, poglavlje 3d)_                                                                                                         | rok rezervacije akcije iz M3 događaja — posle njega se sadržaj **ne objavljuje** (`EXPIRED`); `null` za sadržaj koji nije vezan za akciju                                                                                     |
| source_offer_id             | UUID, nullable _(v1.7, poglavlje 3d)_                                                                                                         | M3 `source_id` iz `pricelist.offer.expiring` — brava protiv dva nacrta za istu akciju                                                                                                                                         |
| b2b_audience                | enum: `ASSIGNED_ONLY`, `ALL_ACTIVE`, nullable _(v1.7)_                                                                                        | **samo uz `B2B_SUBAGENTS` kanal** — krug subagenata, pravilo i podrazumevana vrednost u M7 §5b.1; čovek menja pri odobravanju                                                                                                 |
...
### 3d. Akcija pred istek → nacrt objave (dopuna v1.7, 14.9.2026, na predlog vlasnika — izvor M3 §4.9)

Drugi okidač pored `product.published` (poglavlje 3), isti tok od koraka 2 nadalje: M12 se pretplaćuje na M3 događaj `pricelist.offer.expiring` (M3 §4.9.3, emituje se na **7 dana** do isteka akcije, jednom po stavci) i AI agent **automatski pravi nacrt** — `ContentPiece` sa `status = PENDING_APPROVAL`, `generated_by = AI`, `product_id` iz događaja. Nivo „Autonomno" za nacrt, **ljudsko odobrenje** za objavu (`approved_by`), kao i do sada — vlasnik 14.9.2026: _„AI nacrt — čovek odobrava"_.

**Šta nacrt sadrži i odakle.** Sve iz samog događaja, bez čitanja M3 baze: naziv objekta i destinacija, vrsta akcije i sažetak popusta („−15 % za rani buking", „7 = 6"), period boravka i — obavezno, doslovno — **rok rezervacije** (`booking_to`). Tekst mora reći „rezervacije do 30.9.", nikad „još 7 dana" bez datuma: objava se odobrava možda dva dana kasnije, i relativan broj bi tad već bio netačan (M3 §4.9.3, isto upozorenje).

**Jedan događaj → jedan nacrt, tri kanala u istom nacrtu.** Podrazumevani `target_channels` nacrta: `FACEBOOK`, `INSTAGRAM`, `B2B_SUBAGENTS` (nov kanal, poglavlje 2.1/4). `EMAIL` (gostima) **nije** podrazumevano uključen — newsletter gostima o svakom isteku akcije bi za mesec dana bio spam; čovek ga dodaje kad proceni. Čovek pri odobravanju sme da skine bilo koji kanal.

**`scheduled_publish_at` se predlaže odmah** (sutradan u 9:00 lokalno) — kod ovog okidača vreme radi protiv nas, za razliku od objave novog proizvoda gde nema žurbe. I dalje je predlog: čovek ga menja ili briše.
...
`B2B_SUBAGENTS` kanal (v1.7, poglavlje 3d) ne ide na spoljnu platformu: adapter upisuje sadržaj kao obaveštenje na B2B portalu i šalje mejl kroz zajednički `MailerService` — **spisak primalaca daje M7** (`GET /b2b/notice-recipients?productId=…&audience=…`, M7 §5b.1), M12 ga ne računa sam, jer pravila vidljivosti i dodela žive u M7. Ograda u adapteru, ne u AI-ju: tekst za ovaj kanal ne sme sadržati nabavnu cenu, ime dobavljača ni broj jedinica (M7 §5b.3) — adapter odbija objavu ako nacrt sadrži ta polja (ista vrsta provere kao `hasAiTransparencyMarker`, poglavlje 3c).
...
## 6d. Dopuna M3 i M7 specifikacija (14.9.2026, poglavlje 3d)

U M3 (§4.9) je događaj `pricelist.offer.expiring` i `OfferExpiryNotice`; u M7 (§5b) publika `b2b_audience`, ruta `/b2b/obavestenja`, `GET /b2b/notices` i `GET /b2b/notice-recipients`, `Subagent.offer_notices_by_email`. Oba upisana u istom prolazu.
