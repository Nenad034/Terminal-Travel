...
- **Tip kreveta u `beds` (poglavlje 2.3b), ne samo broj** (25.8.2026, na zahtev vlasnika: "kod tipova smeštaja treba da bude naglašeno kakvi tipovi kreveta su u pitanju... bitno je da sve bude jasno napisano jer klijenti različito tumače oznake 2+1, 2+2, 3+1") — `base_beds`/`extra_beds_max` danas nose samo BROJ kreveta, ne i njihov TIP (francuski/bračni ležaj, dva odvojena kreveta, sofa za dve osobe, pomoćni ležaj za jednu osobu...). Isti razlog kao `amenities[]` kontrolisana taksonomija (poglavlje 2.3c) — oznaka "2+1" bez opisa tipa kreveta je dvosmislena i različiti klijenti/dobavljači je različito tumače, isto poznato zbunjujuće mesto kao uzrasne kategorije (2.3b uvod). Namerno OSTAVLJENO za kasnije, vlasnikova eksplicitna odluka (zabeleženo, ne graditi sada) — kad dođe na red, verovatno dodati `bed_type` enum (npr. `DOUBLE_FRENCH`, `TWIN_SEPARATE`, `SOFA_DOUBLE`, `EXTRA_SINGLE`, proširivo bez izmene strukture, isti obrazac kao `AmenityTag`) uz svaku stavku unutar `base_beds`/`extra_beds_max`, umesto samo zbirnog broja.
