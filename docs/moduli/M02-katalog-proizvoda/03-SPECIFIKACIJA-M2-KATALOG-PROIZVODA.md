...
- [ ] `GET /catalog/public/products` vraća `400` sa jasnom porukom kad `channel` nedostaje ili nije jedna od tri dozvoljene vrednosti. *(zatečeno 3.9.2026 pri pisanju API dokumentacije: oba slučaja vraćaju `500 Internal server error` — parametar nije validiran pre upotrebe u Prisma upitu. Jedini javan endpoint u sistemu, koristi ga M8 i svaki spoljni integrator; zapisano kao zamka 13.1)*
