...
- **Prvo podešavanje 2FA — `mfa_setup_pending` token (dopuna, 4.9.2026, odluka vlasnika).** Do sada je `login` internu ulogu bez `mfaEnabled` odbijao greškom, a jedini endpoint koji 2FA uključuje (`/auth/mfa/enroll`) stajao je iza `JwtAuthGuard` — čime nijedan novozaposleni nije mogao da se prijavi (blokada i na produkciji, ne samo u testu; vidi poglavlje 9). Rešenje: kad je lozinka **tačna** a nalog je `ACTIVE` i traži 2FA koju još nema, `login` više ne baca grešku nego vraća `{ requiresMfaSetup: true, setupToken }`. Taj token je JWT `type = 'mfa_setup_pending'`, traje **10 minuta** (`MFA_SETUP_TOKEN_TTL`), i prihvataju ga isključivo dva endpointa iz poglavlja 6 (`/auth/mfa/setup/start` i `/auth/mfa/setup/confirm`) — nijedan drugi guard ga ne prihvata kao pristupni token, pa ne otvara ništa u sistemu osim same stranice za podešavanje. Uspešan `confirm` postavlja `mfaEnabled = true`, upisuje `auth.mfa_enabled` u audit log i **odmah izdaje redovan access + refresh par** (korisnik ne mora ponovo da se prijavljuje). Izabrano nad varijantom sa linkom u email pozivnici (ne zavisi od isporuke email-a i rešava i naloge koji već postoje) i nad varijantom „HR uključuje 2FA u ime korisnika" (tu bi tajni ključ video neko ko nije vlasnik naloga, čime 2FA prestaje da dokazuje identitet). Pogrešan kod pri `confirm` broji se u **isti** `failed_login_attempts` brojac i isto zaključavanje kao pogrešna lozinka i pogrešan `mfa/verify` kod — namerno bez posebnog brojaca, isti princip kao dopuna od 29.8.2026.
- **Pozivnica i reset lozinke stvarno se šalju na email (dopuna, 4.9.2026, na zahtev vlasnika).** `POST /users` je od početka vraćao `inviteToken`, ali slanje pošte nije bilo povezano ni u jednom modulu, a panel je taj token **tiho odbacivao** — nalog je nastajao u statusu `INVITED`, pozvani čovek nikad ništa nije dobio i nije mogao da postavi lozinku (zamka 5.12). Sada: (1) `UsersService.invite` šalje poruku sa linkom `/aktivacija?token=…` i vraća `emailDelivered`; (2) `AuthService.requestPasswordReset` šalje poruku sa linkom `/reset-lozinke?token=…`; (3) postoje sve tri stranice u panelu — `/aktivacija`, `/reset-lozinke`, `/zaboravljena-lozinka` (poglavlje 7). Slanje ide kroz zajednički `MailerService` (`apps/api/src/common/mail`, `nodemailer`/SMTP — master dokument poglavlje 6).

  **Rok pozivnice je odvojen od roka za reset:** `INVITE_TTL_HOURS = 48` naspram `PASSWORD_RESET_TTL_HOURS = 1`. Reset traži sam korisnik i odmah ga koristi; pozivnicu neko drugi šalje, pa rok mora preživeti put do primaoca.

  **Neuspelo slanje ne ruši radnju koja ga je izazvala** — `MailerService` nikad ne baca ka pozivaocu, nego vraća `delivered: false`. Nalog je već napravljen, pa bi izuzetak ostavio pozivaoca bez ijednog načina da nastavi; umesto toga ekran „+ Pozovi korisnika" i dalje prikazuje link za ručno prosleđivanje. **Taj rezervni put se namerno ne uklanja** uvođenjem pošte — poruka može završiti u nepoželjnoj pošti, adresa može biti pogrešna, server privremeno nedostupan.

  **`POST /auth/password/forgot` i dalje odgovara isto bez obzira na ishod** — ni na nepostojeći nalog ni na neuspelo slanje odgovor se ne menja (ista „ne otkrivati" logika kao poruka „Pogrešan email ili lozinka"; različit odgovor bi dozvolio da se pogađanjem adresa utvrdi ko ima nalog).

  **Aktivacija i reset ne izdaju sesiju** — postavljaju samo lozinku. Prijava je poseban korak koji za internu ulogu uvek prolazi kroz podešavanje/proveru 2FA, pa nov nalog ne dobija pristup panelu bez drugog faktora ni na dan otvaranja.
- **Prvi nalog (bootstrap) — `seed.ts` (dopuna, 4.9.2026, na zahtev vlasnika).** Seed je pravio uloge, dozvole i sistemske AI naloge, ali nijedan ljudski `STAFF` nalog: posle svežeg seed-a niko se nije mogao prijaviti u panel, a pozvati nekoga može samo već prijavljen korisnik — zatvoren krug (zamka 5.7). `seedBootstrapVlasnik()` pravi tačno jedan nalog sa ulogom `VLASNIK`, i ništa više; svi ostali zaposleni nastaju kroz pozivanje iz panela. Pravila su namerno stroga: lozinka **nikad nije konstanta u kodu** (uzima se iz `SEED_VLASNIK_PASSWORD`, a ako je nema — van produkcije se generiše nasumična od ~144 bita i ispiše **jednom** u izlazu seed-a, dok se na produkciji nalog uopšte ne pravi); `mfaEnabled` ostaje `false`, pa i taj nalog prolazi kroz obavezno podešavanje 2FA (nije prečica oko nje); i ponovno pokretanje seed-a **ne dira lozinku** naloga koji već postoji.
...
| `/auth/mfa/setup/start` | POST | **javan** (bez `JwtAuthGuard`) — prima `setupToken` iz `login` odgovora (poglavlje 5, dopuna 4.9.2026), generiše TOTP tajnu i vraća `otpauthUrl` (QR) + 10 rezervnih kodova; ne uključuje 2FA još (`mfaEnabled` ostaje `false` dok kod nije potvrđen) |
| `/auth/mfa/setup/confirm` | POST | **javan** — `setupToken` + prvi TOTP kod; postavlja `mfaEnabled = true` i odmah izdaje access + refresh par. Pogrešan kod uvećava isti brojac/zaključavanje kao pogrešna lozinka |
...
| `/users/directory` | GET | dopuna 31.8.2026 (M5 spec §6.5) — lagan spisak kolega (`id`+`fullName` isključivo, ACTIVE `STAFF`), BEZ `M1/user/VIEW` (koju Prodajni agent/Sales Manager/Računovođa po difoltu nemaju — isti nalaz kao M19 chat "novi razgovor" forma); dostupan svakom `STAFF` pozivaocu, prazna lista za GUEST/SUBAGENT_CONTACT/SUPPLIER_CONTACT. Koristi ga M5 §6.5 (izbor primaoca pri prenosu vlasništva/predaji zaduženja). Dopuna 2.9.2026: sa `?role=` filterom (npr. `VODIC`) odgovor dobija i `phone`/`email` — samo u toj, već suženoj varijanti (M5 spec §4.5, kartica Predstavnici). |
...
| `/roles/:id/permissions` | GET / POST / DELETE | dopuna 4.9.2026 — pregled/dodela/uklanjanje dozvola ulozi (`RolePermission`, poglavlje 3.4). Do ove dopune ta veza je postojala **isključivo** u seed skripti, pa je uloga napravljena preko `POST /roles` ostajala trajno prazna i neupotrebljiva (poglavlje 8). Traži `M1/role/EDIT`; `POST` prima listu `permissionId` vrednosti i dodaje ih bez brisanja postojećih, `DELETE` uklanja jednu. Sistemska uloga (`is_system_role = true`) se **sme** menjati (Vlasnik mora moći da doda dozvolu novog modula postojećoj ulozi), ali svaka izmena ide u audit log kao `role.permissions_changed` sa `before_state`/`after_state`. Izmena važi odmah, bez ponovne prijave nosilaca uloge — isti princip kao `UserPermissionOverride` |
...
| :---- | :---- | :---- |
| **Korisnici** | Tabela svih `User` zapisa (ime, email, dodeljene uloge, status, 2FA, poslednja prijava) sa pretragom; dugme "+ Pozovi korisnika" otvara formu (ime, email, telefon, uloge) koja kreira nalog u statusu `INVITED` i potom **prikazuje link za aktivaciju** koji pozivalac prosleđuje (dopuna 4.9.2026, dok slanje email-a nije povezano — vidi poglavlje 5) | `GET /users`, `POST /users` |
...
| **Uloge** | Kartice svih sistemskih uloga (naziv, opis/opseg iz poglavlja 4, broj nosilaca); uloge dodate kasnijim fazama (`SUBAGENT_ADMIN`, `VODIC`) prikazane kao zaključane sa napomenom u kojoj fazi se aktiviraju. Klik na karticu otvara **uređivanje dozvola te uloge** (dopuna 4.9.2026): pun katalog `Permission` zapisa grupisan po modulu, sa čekboksom po dozvoli i vidljivim brojem izabranih; čuvanje šalje samo razliku. Bez ovog ekrana dodela dozvola postoji samo u seed skripti — vidi poglavlje 8 | `GET /roles`, `GET /permissions`, `GET/POST/DELETE /roles/:id/permissions` |
...
**Prijava — treći korak "podesi 2FA" (dopuna 4.9.2026).** Ekran prijave (`apps/panel/src/app/prijava/LoginForm.tsx`) do sada je imao dva koraka (lozinka → TOTP kod). Kad `login` vrati `requiresMfaSetup` (poglavlje 5), forma prelazi na treći, novi korak i odmah poziva `mfa/setup/start`: prikazuje **QR kod** za skeniranje, `otpauth://` vezu (na telefonu je otvara sama autentifikator aplikacija), tajni ključ ispisan za ručan unos kad skeniranje nije moguće, spisak od 10 rezervnih kodova sa izričitim upozorenjem da se prikazuju **samo jednom**, i polje za prvi kod iz aplikacije. Potvrda prijavljuje korisnika bez ponovnog unosa lozinke. Ovaj ekran je jedini deo panela dostupan sa `setupToken`-om.

QR se crta bibliotekom `qrcode` (potvrđeno vlasnikom 4.9.2026, master dokument poglavlje 6) **na serveru**, u Next route handler-u — tajna tako ne mora da uđe u klijentski bundle, a slika stiže gotova kao `data:` URI. Podloga QR koda je namerno fiksno bela, ne tematska: čitači traže svetlu podlogu i tamne module, pa bi kod na tamnoj temi postao nečitljiv.

**Aktivacija naloga (`/aktivacija`, dopuna 4.9.2026).** Zasebna stranica van `(app)` grupe (pozvani još nema sesiju, pa ne sme proći kroz proveru prijave): prima `?token=` iz linka koji mu pozivalac prosledi, traži novu lozinku dva puta (min. 12 karaktera, ista pravila kao backend), i po uspehu upućuje na prijavu — bez izdavanja sesije (poglavlje 5). Nepotpun link (bez `token` parametra) dobija jasnu poruku šta nedostaje, ne prazan ekran.

**Zaboravljena lozinka (`/zaboravljena-lozinka` → `/reset-lozinke`, dopuna 4.9.2026).** Ekran prijave sada nosi link „zaboravljena lozinka?" (ranije ga nije imao jer put nije postojao — poruka se nije slala). Prva stranica traži email i **uvek** odgovara isto („ako nalog postoji, poslali smo poruku"), nikad ne kaže da li adresa postoji. Druga je odredište linka iz poruke, traži novu lozinku dva puta, i po uspehu upućuje na prijavu — bez izdavanja sesije. Promena lozinke opoziva sve postojeće sesije (§5), pa se korisnik prijavljuje iznova.
...
- [x] API dokumentacija (`docs/api/M1-core-identitet.md`) postoji sa stvarnim primerima zahteva/odgovora — obavezna stavka po CLAUDE.md. *(napisana 3.9.2026; odgovori uhvaćeni stvarnim pozivima nad lokalnom bazom, osim uspešne prijave koja je prikazana iz koda jer zahteva stvarnu lozinku naloga)*
- [x] **Novozaposleni sa internom ulogom može da se prijavi — postoji put za prvo podešavanje obavezne 2FA.** *(rešeno 4.9.2026, poglavlje 5: `login` izdaje `mfa_setup_pending` token, `mfa/setup/start` + `mfa/setup/confirm` ga troše, treći korak ekrana prijave ga vodi. Dokazano e2e testovima — pun tok, dokaz da `setupToken` ne otvara nijedan drugi endpoint, i da pogrešan kod ulazi u isto zaključavanje. QR kod se crta kao slika od 4.9.2026, bibliotekom `qrcode` — vidi poglavlje 7)*
- [x] **Postoji endpoint za dodelu dozvola ulozi.** *(rešeno 4.9.2026: `GET/POST/DELETE /iam/roles/:id/permissions`, poglavlje 6, uz ekran za uređivanje dozvola po ulozi u panelu, poglavlje 7. Dokazano e2e testovima — dozvola dodata ulozi odmah važi njenom nosiocu bez ponovne prijave i odmah prestaje kad se ukloni, izmena ide u audit log sa pre/posle stanjem, nepostojeća dozvola odbija ceo zahtev fail-closed, i zahtev bez `M1/role/EDIT` dobija 403)*
- [x] **Nalog zaposlenog se može otvoriti od nule i dovesti do prijave.** *(rešeno 4.9.2026: `seed.ts` pravi prvi `VLASNIK` nalog, pozivanje iz panela prikazuje link za aktivaciju, stranica `/aktivacija` postavlja lozinku, pozivnica traje 48h. Dokazano e2e testom koji vodi ceo lanac — poziv → odbijena prijava bez lozinke → aktivacija → prijava koja stiže do podešavanja 2FA → odbijena ponovna upotreba jednokratnog tokena)*
...
- ~~**Nov nalog zaposlenog ne može da se prijavi — 2FA je obavezna, a puta za prvo podešavanje nema**~~ (nalaz 2.9.2026) — **rešeno 4.9.2026**, odlukom vlasnika za varijantu (b): `login` vraća `mfa_setup_pending` token umesto greške; vidi poglavlje 5, 6, 7 i izlazni kriterijum. Pri rešavanju zatečena i zatvorena šira rupa: nijedan proverivac tokena nije gledao **vrstu** tokena, pa je i stariji `mfa_pending` (izdat pre TOTP koraka) radio kao pun pristupni token — sada `assertAccessTokenPayload` fail-closed odbija svaki token sa `type` poljem, na svih pet mesta koja tokene primaju.
- ~~**`seed.ts` ne pravi nijedan nalog zaposlenog**~~ (nalaz 2.9.2026) — **rešeno 4.9.2026**: `seedBootstrapVlasnik()` pravi tačno jedan `VLASNIK` nalog, sa nasumičnom lozinkom ispisanom jednom u izlazu seed-a (ili iz `SEED_VLASNIK_PASSWORD`), bez zaobilaženja obavezne 2FA. Vidi poglavlje 5 i zamku 5.7.
