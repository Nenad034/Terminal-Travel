...
**Prijava — treći korak "podesi 2FA" (dopuna 4.9.2026).** Ekran prijave (`apps/panel/src/app/prijava/LoginForm.tsx`) do sada je imao dva koraka (lozinka → TOTP kod). Kad `login` vrati `requiresMfaSetup` (poglavlje 5), forma prelazi na treći, novi korak i odmah poziva `mfa/setup/start`: prikazuje `otpauth://` vezu (na telefonu je otvara sama autentifikator aplikacija), tajni ključ ispisan za **ručan unos**, spisak od 10 rezervnih kodova sa izričitim upozorenjem da se prikazuju **samo jednom**, i polje za prvi kod iz aplikacije. Potvrda prijavljuje korisnika bez ponovnog unosa lozinke. Ovaj ekran je jedini deo panela dostupan sa `setupToken`-om.

**Poznat nedostatak (4.9.2026): QR kod se ne crta kao slika.** Panel nema biblioteku za generisanje QR-a (`apps/panel/package.json`), a uvođenje nove zavisnosti traži potvrdu vlasnika (CLAUDE.md, "Šta ne raditi"). Podesavanje je i bez toga potpuno upotrebljivo — ručan unos ključa prihvata svaka autentifikator aplikacija, a `otpauth://` veza na telefonu radi kao i skeniranje. Kad vlasnik odobri biblioteku (predlog: `qrcode`, MIT, ~50 kB, bez tranzitivnih zavisnosti), slika se dodaje uz postojeći `otpauthUrl` — nijedna druga izmena nije potrebna.
...
- [x] **Novozaposleni sa internom ulogom može da se prijavi — postoji put za prvo podešavanje obavezne 2FA.** *(rešeno 4.9.2026, poglavlje 5: `login` izdaje `mfa_setup_pending` token, `mfa/setup/start` + `mfa/setup/confirm` ga troše, treći korak ekrana prijave ga vodi. Dokazano e2e testovima — pun tok, dokaz da `setupToken` ne otvara nijedan drugi endpoint, i da pogrešan kod ulazi u isto zaključavanje. Poznat nedostatak: QR se prikazuje kao veza + ručan ključ, ne kao slika — vidi poglavlje 7)*
...
- **`seed.ts` ne pravi nijedan nalog zaposlenog** (isti nalaz, 2.9.2026) — uloge, dozvole i sistemski AI nalozi postoje, ljudski `STAFF` nalog nijedan, pa se posle svežeg seed-a niko ne može prijaviti u panel. Vezano za stavku iznad: i kad bi nalog postojao, 2FA bi ga blokirala. Zamka 5.7 u istom dokumentu.
