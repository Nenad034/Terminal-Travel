...
- **Pozivnica u praksi — link se prosleđuje ručno, rok 48h (dopuna, 4.9.2026, na zahtev vlasnika).** `POST /users` je od početka vraćao `inviteToken`, ali slanje email-a nije povezano (`UsersService.invite` to i kaže u komentaru), a panel je taj token **tiho odbacivao** — nalog je nastajao u statusu `INVITED`, pozvani čovek nikad ništa nije dobio i nije mogao da postavi lozinku. Spojeno u celinu: (1) ekran „+ Pozovi korisnika" posle kreiranja **prikazuje link** `/aktivacija?token=…` koji pozivalac prosleđuje sam, isti obrazac koji M19 već koristi za pozivnicu dobavljaču (M19 §9.7); (2) postoji stranica `/aktivacija` na kojoj pozvani postavlja svoju prvu lozinku (poglavlje 7); (3) rok pozivnice je **odvojen** od roka za reset lozinke — `INVITE_TTL_HOURS = 48` naspram `PASSWORD_RESET_TTL_HOURS = 1`. Razlog: reset traži sam korisnik i odmah ga koristi, dok pozivnicu neko drugi mora proslediti, pa bi rok od sat vremena istekao pre isporuke. Aktivacija **ne izdaje sesiju** — postavlja samo lozinku; prijava je poseban korak koji za internu ulogu uvek prolazi kroz podešavanje/proveru 2FA, pa nov nalog ne može dobiti pristup panelu bez drugog faktora ni na dan otvaranja.
