...
- [ ] Postoji endpoint za dodelu dozvola ulozi. *(zatečeno 3.9.2026 pri pisanju API dokumentacije: `POST /iam/roles` kreira ulogu, ali veza uloga↔dozvola postoji isključivo u seed skripti — uloga napravljena preko API-ja ostaje trajno prazna i neupotrebljiva. Zaobilazno rešenje danas su pojedinačni `PermissionOverride` zapisi, što ne skalira preko nekoliko korisnika)*
