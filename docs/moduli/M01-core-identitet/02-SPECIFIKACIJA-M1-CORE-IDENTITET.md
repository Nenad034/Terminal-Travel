...
| `/users/directory` | GET | dopuna 31.8.2026 (M5 spec §6.5) — lagan spisak kolega (`id`+`fullName` isključivo, ACTIVE `STAFF`), BEZ `M1/user/VIEW` (koju Prodajni agent/Sales Manager/Računovođa po difoltu nemaju — isti nalaz kao M19 chat "novi razgovor" forma); dostupan svakom `STAFF` pozivaocu, prazna lista za GUEST/SUBAGENT_CONTACT/SUPPLIER_CONTACT. Koristi ga M5 §6.5 (izbor primaoca pri prenosu vlasništva/predaji zaduženja). |
