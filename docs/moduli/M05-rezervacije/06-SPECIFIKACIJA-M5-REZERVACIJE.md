...
- **Novo (19.8.2026) — `CRUISE` specificiran (M2 spec poglavlje 2.1/2.3) ali nikad migriran u šemu.** Otkriveno pri implementaciji ikonice "Krstarenja" u M17 levom panelu (v1.27): `ProductType` enum u `schema.prisma` nema `CRUISE`, iako je M2 spec eksplicitno definiše. Čista implementaciona praznina (spec je ispravna, kod zaostaje), ne spec nedoslednost — potrebna Prisma migracija pre nego što ikonica postane funkcionalna, van obima čiste UI izmene, čeka potvrdu vlasnika po CLAUDE.md pravilu o izmeni šeme.
