...
**Ad-hoc priprema za VIŠE rezervacija (dopuna, avgust 2026, na zahtev vlasnika, v1.16):** proširenje prethodnog za slučaj kad operater bira više rezervacija odjednom (`POST /supplier-manifests/prepare-batch`, poglavlje 11), na dva moguća načina — **tačno jedan** se prosleđuje po pozivu:
- **Ručni izbor** (`booking_ids[]`) — operater u M17 čekira nekoliko konkretnih rezervacija (checkbox lista) i traži pripremu za tačno te.
- **Opseg datuma kreiranja** (`created_from`/`created_to`) — sve rezervacije čiji `Booking.created_at` pada u traženi opseg, npr. "sve rezervacije napravljene ove nedelje". Napomena: ovo je opseg **kreiranja** rezervacije, različito od `period_from`/`period_to` u periodičnom agregiranju iznad (koji je opseg **boravka**, `stay_from`/`stay_to`) — namerno, jer je svrha ovde "uhvati sve što je nedavno prodato i još nenajavljeno", ne "šta se dešava u datom terminu".
...
| `/supplier-manifests/prepare-batch` | POST | dopuna v1.16, zahteva `M5/supplier-manifest/CREATE`; isto kao gore, ali obim je `booking_ids[]` (checkbox izbor) ili `created_from`/`created_to` (opseg datuma kreiranja rezervacije) — tačno jedan od ova dva mora biti prosleđen (poglavlje 8.4) |
...
- [ ] Test (dopuna v1.16): `POST /supplier-manifests/prepare-batch` odbija poziv kad su prosleđena OBA `booking_ids`/`created_from`+`created_to`, i kad nije prosleđen nijedan; sa validnim `booking_ids[]` uzima stavke SAMO iz tih rezervacija, sa validnim opsegom datuma uzima stavke iz svih rezervacija čiji `created_at` upada u opseg — u oba slučaja grupisano po dobavljaču, isto kao pojedinačna priprema (v1.15).
