...
**Nivo:** Nivo 2 — detaljna specifikacija, dovoljna da AI agent direktno programira po njoj, uz izuzetak tačno navedenih mesta gde je potrebna potvrda pravnika/knjigovođe pre implementacije (poglavlje 8)
**Status:** Nacrt za usvajanje
**Verzija:** 1.0 — prvi zapis (8.9.2026), na zahtev vlasnika: "svakom zaposlenom treba da dodelimo ulogu [...] treba i da postoji link prema ugovoru o radu [...] želim ovaj deo da uredimo po evropskim i svetskim standardima". Predlog nastao u razgovoru (Master dokument v1.25), ovaj dokument ga razrađuje u Nivo 2 detalje.
...
M24 čuva **HR dosije zaposlenog** — organizacionu poziciju, ključne datume radnog odnosa, link ka potpisanom ugovoru o radu, evidenciju godišnjeg odmora/odsustava i rokove obuka/sertifikata. Cilj je transparentnost u odnosu poslodavac↔zaposleni (vlasnikova formulacija) i praćenje rokova koje niko ne sme da propusti (probni rad, istek ugovora na određeno, godišnji odmor).
...
- **Fajl-skladište za sam ugovor** — vidi poglavlje 3, MVP čuva samo referencu/link, ne sâm dokument.
...
| Samostalni referent prodaje | `PRODAJNI_AGENT` (postojeća) | bez izmene naziva u kodu — postojeća uloga već nosi pun skup prodajnih dozvola (cenovnik, ponude, rezervacije) koji odgovara "samostalnom" nivou |
| Referent prodaje | **nova: `REFERENT_PRODAJE`** | uži nivo — vidi otvoreno pitanje ispod |

**Otvoreno pitanje koje čeka vlasnikovu potvrdu pre implementacije dozvola (ne pre upisa uloge u katalog):** da li `REFERENT_PRODAJE` ima STVARNO uži skup dozvola od `PRODAJNI_AGENT` (npr. ne sme da menja cenu bez odobrenja, ne sme da otkazuje rezervaciju) ili je razlika čisto organizaciona titula bez uticaja na pristup. Pretpostavka za prvi prolaz (menja se lako ako pogrešna): referent NEMA `M5/booking/price-override`, `M5/booking/cancel` — ista logika kao "manje iskusan prodavac ne sme sam da menja cenu", uobičajena podela u evropskim agencijama (junior/senior sales rep).

**Prikazni naziv uloge (dopuna, van obima ove verzije, upisano da se ne izgubi):** role badge u panelu danas prikazuje doslovno ime konstante (`PRODAJNI_AGENT`, ne "Samostalni referent prodaje") — vidi `apps/panel/src/app/(app)/korisnici/[id]/page.tsx`. Uvođenje čitljivog prikaznog imena (mapa `SYSTEM_ROLES → čitljiv naziv`, npr. `{ PRODAJNI_AGENT: 'Samostalni referent prodaje', SALES_MANAGER: 'Menadžer prodaje' }`) je nezavisna, mala UI izmena — ide u istom prolazu kad se M24 implementira, da HR ekran i `/korisnici` odmah govore istim jezikom.
...
| contract_end_date | date, nullable | obavezno ako `contract_basis = ODREDJENO` — ovo je polje koje AI HR agent prati (poglavlje 4) |
...
| reports_to_user_id | UUID (FK → M1 User), nullable | neposredni rukovodilac — eksplicitno polje, odvojeno od organizacione uloge (npr. Referent prodaje u poslovnici Beograd izveštava se Šefu te poslovnice) |
| employment_contract_url | string, nullable | vidi poglavlje 3 — MVP: referenca/link, ne upload |
...
| expires_at | date, nullable | polje koje AI HR agent prati (poglavlje 4) — obavezna obuka bez roka ima `expires_at = null` i agent je ne prati |

---

## 3. Ugovor o radu — transparentnost poslodavac↔zaposleni

Vlasnikov zahtev: "treba i da postoji link prema ugovoru o radu koji su potpisali kako bi uvek bilo sve transparentno u odnosu poslodavac-zaposleni."

**MVP (ova verzija): `employment_contract_url` je REFERENCA, ne upload.** Projekat danas nema nijedno mesto gde korisnik upload-uje fajl kroz panel (proverено pri pisanju ove specifikacije — ni logo, ni bilo šta drugo, vidi M1 spec §3.9c napomenu o logu). Uvođenje pravog skladišta fajlova (gde, S3-kompatibilan servis ili lokalni disk, EU rezidencija po Master dokumentu poglavlje 9) je infrastrukturna odluka van obima ove verzije — polje čuva URL ka postojećem mestu čuvanja ugovora (deljeni disk agencije, mejl prilog, fizička arhiva sa skeniranom kopijom), ne sâm fajl. Pravo skladište je stavka u "Otvoreno za dalje" (poglavlje 8), isti obrazac kao odložena odluka o logo-uploadu.

**Vidljivost (osetljivije od bilo čega što M1 danas čuva).** Ugovor o radu tipično sadrži platu i druge lične podatke van dometa običnog RBAC kruga. Dozvola `M24/employee-record/VIEW-CONTRACT` (odvojena od opšte `M24/employee-record/VIEW`, poglavlje 5) — dodeljena isključivo Vlasniku, Direktoru, HR-u, i **samom zaposlenom za sopstveni zapis** (ownership provera, isti obrazac kao Gost koji vidi sopstvenu rezervaciju, M5 §6.6).

---

## 4. Rokovi i AI HR agent
...
| :--- | :--- |
| `M24/employee-record/VIEW` | Vlasnik, Direktor, HR — opšti HR podaci (pozicija, datumi, godišnji odmor), BEZ linka ka ugovoru |
| `M24/employee-record/VIEW-CONTRACT` | Vlasnik, Direktor, HR; zaposleni za SOPSTVENI zapis (ownership) — vidi poglavlje 3 |
...
| Endpoint | Metod | Opis |
| :--- | :--- | :--- |
| `/hr/employees/:userId` | GET | HR dosije jednog zaposlenog — `employment_contract_url` se izbacuje iz odgovora bez `VIEW-CONTRACT` dozvole (ista tehnika kao M20 `client-contracts` ownership filter) |
...
- [ ] Nove sistemske uloge (`REFERENT_PRODAJE`, `SEF_POSLOVNICE`, `FINANSIJSKI_DIREKTOR`) postoje u M1 katalogu, dodeljive kroz postojeći `/korisnici/[id]` ekran bez izmene tog ekrana.
...
- [ ] Link ka ugovoru o radu vidljiv isključivo nosiocu `VIEW-CONTRACT` ili samom zaposlenom za sopstveni zapis — dokazano testom da TREĆE lice bez dozvole ne dobija to polje ni u odgovoru API-ja (ne samo sakriveno u UI).
...
- **Da li `REFERENT_PRODAJE` ima stvarno uži skup RBAC dozvola od `PRODAJNI_AGENT`, ili je čisto organizaciona titula** — poglavlje 2.1, pretpostavka upisana, čeka vlasnikovu potvrdu pre implementacije konkretnih dozvola.
- **Pravo skladište fajla ugovora** (zamena za MVP referencu-link, poglavlje 3) — ista vrsta odluke kao logo-upload (`docs/analize/27-BACKLOG-IDEJA-I-PREDLOZI.md`), čeka stvarnu potrebu i izbor skladišta (S3-kompatibilan servis, EU rezidencija).
...
- **Da li Referent/Samostalni referent/Šef poslovnice treba da imaju svoju liniju u `ROLES_REQUIRING_MANDATORY_MFA`** (M1 §5) — verovatno DA (interne uloge), potvrditi pri implementaciji da nijedna nova uloga nije slučajno izostavljena iz obavezne 2FA liste.
- **Tačan broj dana unapred za podsetnik o isteku ugovora/probnog roka** (poglavlje 7 izlaznog kriterijuma) — razuman podrazumevan predlog je 30 dana za ugovor na određeno, 7 dana za probni rad, ali ovo je vlasnikova odluka, ne tehnička pretpostavka.
