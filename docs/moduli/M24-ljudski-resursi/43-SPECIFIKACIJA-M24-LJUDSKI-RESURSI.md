...
- [ ] Dodeljeni dani godišnjeg odmora se dodeljuju PO GODINI (`LeaveEntitlement`, poglavlje 2.2a, predlog v1.5) — NIJE implementirano ovim prolazom, čeka vlasnikovu potvrdu nacrta pre migracije/koda. Uz to: `carried_over_days` (preneti dani iz prethodne godine, rok 30.6.) postoji kao ručno potvrđeno polje sa predloženom vrednošću, ne automatski obračunato — dokaz kad se implementira: test da red bez unetog broja za godinu vraća "nije dodeljeno" (ne nasleđuje tiho iz prethodne godine), test da preneti dani prestaju da se računaju posle `carried_over_expires_at`, uživo popunjena forma po godinama u HR dosijeu.
