...
**Verzija:** 1.6 — isti bag ispravljen kao M23 spec v1.8 (21.8.2026): `selectCandidatesByEmbedding` sad prima `applyDistanceThreshold` parametar (`!this.anthropic.isConfigured()`), prag se više ne primenjuje kad Anthropic sam prepoznaje irelevantnost. Vidi M23 spec v1.8 za detalje uživo pokušaja koji je ovo otkrio (uklj. `429 RESOURCE_EXHAUSTED` — vlasnikov Gemini nalog nema preplaćen kredit).
