...
**Tehnički mehanizam (isti obrazac kao M17 "Horizont" paleta i njen prekidač tamnog/svetlog moda):** portal ne piše boje direktno u komponente — čita ih iz jednog centralnog sloja CSS promenljivih ("dizajn tokeni"), koji se pri svakom zahtevu popunjava iz `SubagentBranding` zapisa ulogovanog naloga, sa padom (fallback) na baznu "Zalazak" paletu (M8 spec §1a) kad polje nije postavljeno. Posledica: promena brenda je promena podataka, ne izmena koda — ništa u poslovnoj logici (pretraga, rezervacija, provizija, kreditni limit) se ne dodiruje.
