...
| `/b2b/obavestenja`         | Akcije pred istek koje agencija šalje ovom subagentu (poglavlje 5b.2) — klik vodi na pretragu sa popunjenim hotelom i periodom | M7 `/notices`                                                           |
...
**Potpitanje (M15 §6.5.4.6, 17.9.2026):** kad subagent pretražuje raspoloživost rečima („porodica u Grčkoj u avgustu"), a fali period ili sastav putnika, agent pita jednom za sve što fali; period i sastav iz prethodne ture u istoj sesiji važe dok ih subagent ne promeni.
...
### 2.0.5 Vizuelni identitet po subagentu — "beli-label" (dopuna, avgust 2026, na zahtev vlasnika)

**Cilj:** svaki subagent (partnerska agencija) može da portalu (poglavlje 2.0.1) da sopstveni izgled — sopstveni logo i boje — tako da izgleda kao njihov sopstveni sajt, ne kao Terminal Travel. Ovo je **samostalno podešavanje**, ne funkcija koju osoblje agencije unosi u ime subagenta (odluka vlasnika, avgust 2026) — subagent sam otpremi logo i izabere boje kroz sopstveni portal nalog.
...
**Ruta:** `/b2b/profil/brend` (dopuna tabele poglavlja 2.0.1).
...
### 2.0.6 Interfejs identičan M17, obim podataka ograničen na prodajni tok (vlasnikova odluka, 17.8.2026)
...
**Van obima ostaje svaki modul/ekran koji NIJE deo prodajnog toka jednog subagenta** — M10 finansijska knjiga agencije (van sopstvenih faktura/provizije), M13 BI izveštaji, M18 AI-trošak/operativni nadzor, M6 CRM baza svih gostiju agencije (samo sopstveni klijenti kroz poglavlje 7), podaci drugih subagenata van direktne dece u sopstvenoj hijerarhiji (poglavlje 6, već strogo razdvojeno), HR/interni audit. Ovi ekrani se u M7 portalu **ne prikazuju uopšte** (ne postoji ruta), za razliku od marže/nabavne cene/dobavljača koji se sakrivaju polje-po-polje unutar ekrana koji subagent inače vidi. Ako se pokaže potreba da subagent vidi nešto iz ove liste (npr. sopstveni deo M13 izveštaja), to je nova odluka vlasnika i dopuna ovog poglavlja, ne automatska posledica M17-identičnog interfejsa.

### 2.0.7 Franšizni subagent — pun pristup internom panelu (M17) (dopuna, 31.8.2026, na zahtev vlasnika)
...
- **Vidljivost rezervacija** — `M5/booking/VIEW_ALL` (M1 §3.9a, M5 poglavlje 6.6) je za franšizne naloge uvek ograničen na `Booking.franchise_subagent_id = actor.linked_profile_id` (M5 poglavlje 4.1/6.6) — vide sve rezervacije SVOJE franšize podrazumevano, nikad matične agencije ili druge franšize; podesivo na "samo svoje" po istom mehanizmu kao matična agencija (M1 §3.9a).
- **Brend u panelu** — franšiza koristi **postojeći `SubagentBranding` zapis** (poglavlje 2.0.5) bez izmene modela; jedina razlika je da se čita i primenjuje na puni M17 UI (ne samo na M7 portal), i logo/podaci firme se pojavljuju i na dokumentima koji izlaze ka gostu (vaučer, M5 poglavlje 6) — isti `logo_url`/`display_name` tokeni, novi potrošač.
...
| offer_notices_by_email               | boolean, default `true`                         | v1.13, poglavlje 5b.2 — da li subagent prima mejl o akcijama pred istek; portal obaveštenje ostaje uvek                                                                                                            |
...
## 5b. Obaveštenje subagentima o akciji pred istek (dopuna v1.13, 14.9.2026, na predlog vlasnika — izvor M3 §4.9)

**Šta je vlasnik tražio:** kad akciji (rani buking, 7=6, popust) ostane 7 dana do isteka, pored objave na mrežama, _„pošalje obaveštenje subagentima"_. Na pitanje da li svima ili samo onima kojima je proizvod dodeljen: _„to će zavisiti od vrste akcije, ako je važno za last minute da nešto prodamo onda ide svima."_

**Do ove verzije M7 nije imao nijedan kanal kojim agencija nešto **šalje** subagentu** — subagent je uvek sam dolazio na portal i tražio. Ovo je prvi takav kanal, i namerno ne uvodi novu infrastrukturu: obaveštenje je M12 `ContentPiece` sa kanalom `B2B_SUBAGENTS` (M12 §2.1/§4), koji AI napravi iz M3 događaja `pricelist.offer.expiring` i **čovek odobri** (M12 §3d). M7 daje samo dve stvari — **spisak primalaca** i **mesto na portalu** gde obaveštenje stoji.

### 5b.1 Ko dobija — dva kruga, čovek bira

`ContentPiece.b2b_audience` (M12 §2.1) ima dve vrednosti:

| Vrednost        | Ko dobija                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Kad je podrazumevano                                                        |
| :-------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------- |
| `ASSIGNED_ONLY` | `ACTIVE` subagenti koji taj proizvod **stvarno vide**: proizvod ima `B2B_PORTAL` u `visible_channels` (M2 §5) **i** (za taj `contract_id` postoji njihova ili opšta `SubagentCapacityAllocation` sa `status = ACTIVE` koja pokriva period boravka, **ili** nemaju nijednu dodelu uopšte — jer takav subagent vidi punu raspoloživost, 5a.2). Subagent sa dodelama na drugim hotelima, a bez dodele na ovom, **ne dobija** — njemu taj hotel ionako nije u ponudi. | kad događaj nosi `has_subagent_allocations = true` (M3 §4.9.3)              |
| `ALL_ACTIVE`    | svi `ACTIVE` subagenti kojima je proizvod vidljiv po `visible_channels`, bez obzira na dodele                                                                                                                                                                                                                                                                                                                                                                     | kad `has_subagent_allocations = false` (nema dodela → ionako svi vide isto) |

Podrazumevana vrednost je **predlog** koji AI upiše u nacrt; čovek je pri odobravanju menja jednim klikom (M17 ekran `/marketing/:id`, M12 §3d) — to je onaj „zavisi od vrste akcije" deo koji sistem ne može da zna: da li nam je važnije da ne uznemiravamo partnere ili da rasprodamo.

**Sub-subagenti (poglavlje 6):** obaveštenje ide samo direktnim primaocima po pravilu iznad; da li ga subagent prosleđuje svojoj mreži, njegova je stvar — agencija ne komunicira preko glave partnera, isto pravilo kao stroga razdvojenost vidljivosti.
...
2. **Mejl** — isti tekst na kontakt-adresu subagentovog `ClientAccount`-a (M6), preko zajedničkog `MailerService`. **Ne prolazi kroz `marketing_consent` gejt** M6 §4.2 — ovo je poslovna komunikacija sa ugovornim partnerom (`LEGAL_ENTITY`) o robi koju on prodaje, `CommunicationLog.category = TRANSACTIONAL` (M6 §2.1, dopuna 1.9.2026), ne marketing prema fizičkom licu. Ako subagent ipak ne želi mejlove ove vrste, gasi se po subagentu: `Subagent.offer_notices_by_email` (boolean, default `true`, poglavlje 2.1) — portal ostaje uvek.

### 5b.3 Šta subagent vidi u poruci, a šta ne

Isto pravilo kao 2.0.6 i 5a.3: vrstu akcije i **veličinu popusta** („−15 %", „7 = 6"), rok rezervacije (`booking_to`, doslovno kao datum), period boravka, i link na portal. **Poruka ne nosi iznos cene** — cena za subagenta zavisi od njegove provizije (poglavlje 5), pa bi jedan tekst za sve primaoce bio ili pogrešan ili bi otkrivao tuđe uslove; svoju cenu i semafor subagent vidi kad klikne, na `/b2b/pretraga`. **Nikad:** nabavnu cenu, ime dobavljača, koliko soba imamo, ni to da je ista poruka otišla drugim subagentima. M12 pri sastavljanju poruke za `B2B_SUBAGENTS` kanal ovo sprovodi na isti način kao što `EMAIL` kanal sprovodi `marketing_consent` — ograda u adapteru, ne oslanjanje na to da će AI „znati".
...
| `/notices`                                                | GET                | v1.13, poglavlje 5b.2 — objavljena `B2B_SUBAGENTS` obaveštenja za prijavljenog subagenta, najnovije prvo; `?unread=true` za brojač                                                            |
| `/notices/:contentId/read`                                | POST               | označi pročitano (za brojač u gornjoj traci portala)                                                                                                                                          |
| `/notice-recipients`                                      | GET                | **interni, zove ga M12 adapter** — `?productId=…&audience=ASSIGNED_ONLY\|ALL_ACTIVE` vraća spisak subagenata (id, mejl, `offer_notices_by_email`) po pravilu 5b.1; zahteva `M7/subagent/VIEW` |
...
- **Vizuelni identitet po subagentu (poglavlje 2.0.5)** — sopstveni domen/poddomen po subagentu, potpuno uklanjanje pomena "Terminal Travel" iz pravnog teksta (čeka potvrdu pravnika), i pun set tokena (van dve boje + logo) namerno van obima v1; implementacija u celini čeka izgradnju portal frontend-a (poglavlje 2.0.1), koji još ne postoji u kodu.
