...
- [x] Test-slučaj: potvrđena rezervacija bez izdatog fiskalnog dokumenta se ispravno prepoznaje kroz `/reconciliation/mismatches`. *(dokazano unit testom, `ReconciliationService`; `HealthSignal` se emituje preko Event Bus-a — M18 još ne postoji kao model, isti obrazac kao ostali M10 alarmi)*
...
- [x] `KNJIZNO_ODOBRENJE` dokument se ispravno priprema sa `booking_id = null` i popunjenim `related_subagent_id`/`credited_rebate_id`. *(dokazano unit testom, `prepareCreditNoteDraft`; stvarno povezivanje sa odobrenim M7 `CommissionRebate` čeka M7 implementaciju — vidi poglavlje 1.1)*
