...
- [ ] **E2E test `§5.2 — uplata dovodi Booking do PAID` prolazi.** *(PADA od 3.9.2026, zatečeno 4.9.2026 pri punom prolazu e2e paketa. Simptom: `POST /api/v1/finance/payments` vraća `404` umesto `201` — `expect(res.status).toBe(201)`, `apps/api/test/m10-exit-criteria.e2e-spec.ts:278`.*

  ***Potvrđeno da NIJE posledica izmena od 3–4.9.2026** — test pada identično i sa `git stash`-ovanim izmenama, dakle greška postoji u `f5ccb27`. Nije uzrokovana ni M4/M2 ispravkama iz istog prolaza.*

  ***Šta se zna:** jedini `404` na toj putanji je `NotFoundException(\`Booking ${dto.bookingId} nije pronađen.\`)` (`payments.service.ts:74`) — dakle problem je u **fixture-u koji pravi rezervaciju**, ne u samoj logici uplate. Prethodni pokušaj ispravke (commit `92d74a5`, „Fix M10 e2e: §5.2 uplata testira BANK_TRANSFER sa obaveznim bankId") pomerio je grešku sa `400` na `404` — dodavanje `bankId` je rešilo validaciju, ali je otkrilo problem ispod.*

  ***Glavni osumnjičeni:** `createConfirmedBookingFixture()` pravi `Supplier`/`Contract` direktno preko Prisma-e sa poljima izvedenim iz jednog `testRunId` za ceo prolaz (`taxId: TAX-${testRunId}`, `contractNumber: C-${testRunId}`), a poziva se iz VIŠE testova u istom prolazu — druga upotreba udara u jedinstveni indeks, pa fixture ili pukne ili vrati nepotpun rezultat, i `booking.id` stigne prazan/nepostojeći do endpointa. Drugi kandidat je čišćenje iz prethodnog testa koje briše rezervaciju pre ovog.*

  ***Sledeći korak pri ispravci:** ispisati `res.body` (ne samo status) da se potvrdi tačna poruka `404`, pa proveriti da li `createConfirmedBookingFixture` stvarno vraća kreiranu rezervaciju pri drugom pozivu u istom prolazu; ako je uzrok sudar jedinstvenih polja, dodati slučajan sufiks po pozivu, isti obrazac koji `ensureTestBank()` već koristi.*

  ***Napomena o načinu pokretanja:** paket prolazi 5/6 i kad se pokrene sam. Uz to, ceo e2e paket pokrenut uporedo (podrazumevano) pokazuje znatno više padova (11 paketa) nego serijski (`--runInBand`, 6 paketa) — testovi dele jednu razvojnu bazu i ometaju se međusobno, pa se rezultati punog paralelnog prolaza ne smeju čitati kao spisak stvarnih grešaka. Za ocenu stanja koristiti pojedinačno pokretanje po paketu.)*
