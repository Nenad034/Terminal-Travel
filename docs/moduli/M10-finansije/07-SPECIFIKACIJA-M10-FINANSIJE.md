...
**Ograničenje koje ostaje:** izvor je i dalje javna stranica čiji format nije ugovoren (v1.15) — dovlačenje unazad nasleđuje isti rizik, i istom zamenom (zvaničan SOAP veb-servis NBS, poglavlje 11) prestaje da bude privremeno. Kursna lista je istog dana dobila i **ekran u panelu** (`/finansije/kursna-lista`, M17 spec §4/v2.52, na zahtev vlasnika): straničen uvid sa filterom po valuti i vidljivim poreklom svakog zapisa (NBS automatski / ručno uneto), upozorenje kad je poslednji kurs stariji od tri dana, i forma za ručni unos uz `M10/exchange-rate/EDIT`. Do tada je API postojao bez ijednog ekrana. Uz ekran je ispravljeno i to da je dupli unos (isti dan i valuta) vraćao golo `500`: sada je `409` sa objašnjenjem, a postojeći zapis se NE prepisuje — kurs koji je već upotrebljen u obračunu ne sme se tiho promeniti pod nogama dokumentima koji ga citiraju.
