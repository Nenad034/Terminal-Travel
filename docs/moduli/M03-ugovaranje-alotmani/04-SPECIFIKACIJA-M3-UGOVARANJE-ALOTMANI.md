...
- [x] API dokumentacija (`docs/api/M3-ugovaranje-alotmani.md`) postoji sa stvarnim primerima zahteva/odgovora za svaki endpoint iz poglavlja 6 — obavezna stavka po CLAUDE.md. *(napisana 3.9.2026; odgovori uhvaćeni stvarnim pozivima nad lokalnom bazom, osim `offers` i `ancillary-services` gde u bazi nema redova pa su primeri sastavljeni iz modela i tako i označeni u dokumentu)*
- [x] Objašnjenje za vlasnika (`00-OBJASNJENJE-M3-ZA-VLASNIKA.md`) postoji — obavezna stavka po CLAUDE.md. *(napisano 3.9.2026)*
