...
**Verzija:** 1.44 — **akcija pred istek implementirana kraj-do-kraja** (17.9.2026, isti prolaz kao M12 v1.8 / M7 v1.14 / M17 v2.77). `OfferExpiryService` (`pricelist/offer-expiry.service.ts`): kandidati iz sve tri tabele (§4.9.1), pragovi iz `M3_OFFER_EXPIRY_DAYS_INTERNAL/MARKETING`, „manje ili jednako", tri ograde, događaj `M3 pricelist.offer.expiring` sa punim sadržajem iz §4.9.3. Tri dopune nastale pri pisanju koda: (1) `OfferExpiryNotice` nosi **snimak stavke** (naziv objekta, destinacija, vrsta, sažetak, rok, boravak) da radni spisak čita jednu tabelu, ne tri izvora; (2) „ista kombinacija" za cenu prve tranše uključuje i **popunjenost i dane u nedelji** — cena za dve osobe nije skuplja naslednica cene za jednu; (3) `has_subagent_allocations` je uvek `false` dok M7 §5a `SubagentCapacityAllocation` ne dobije tabelu. **Usput otkriveno i ispravljeno:** samovraćanje blokada (2.8b, `releaseExpiredBlocks`) je od 8.9.2026 postojalo kao metoda bez ijednog pozivaoca — nijedna blokada se nikad nije sama vratila; nov `M3DailyJobsService` (6:00) zove i nju i ovaj posao (zamka 8.16). Endpoint-i §6 + ručno `POST /pricelist/expiry-notices/run`. Panel: blok „Akcije kojima ističe rok" na vrhu `/kapaciteti` (M17 §4b.0, stanje 1 — prvi red radnog spiska; ostatak spiska još ne postoji). Dokaz: 14 jediničnih testova, uživo lanac run → 2 zapisa → događaj → M12 nacrt → odobrenje → M7 obaveštenje (17.9.2026). Izlazni kriterijumi 4.9 zatvoreni.
**Verzija:** 1.43 — **akcija pred istek: dva praga, dve publike** (14.9.2026, na predlog vlasnika: obaveštenje kad ostane npr. 7 dana do isteka ranog bukinga/popusta, da se hotel marketinški obradi i javi subagentima). Novo poglavlje **4.9**: dnevni posao gleda svaku aktivnu stavku koja donosi nižu cenu i ima rok (`PricelistOffer` oba tipa, `AncillaryService` popust, `RateLine` „prve tranše"); na **15 dana** red u radnom spisku kapaciteta (ugovarač zove hotel), na **7 dana** događaj `pricelist.offer.expiring` na koji se pretplaćuju M12 (AI nacrt objave, čovek odobrava — M12 §3d) i M7 (obaveštenje subagentima — M7 §5b). Jednom po stavci po pragu (`OfferExpiryNotice`), sa tri ograde pre marketinga (proizvod objavljen, ima šta da se proda, stavka nije ugašena). Pragovi konfiguracija, bez ekrana za izmenu (vlasnik dao fiksne vrednosti; poglavlje 8). Specifikacija bez koda — implementacija čeka posebnu odluku.
...
### 4.9 Akcija pred istek — dva praga, dve publike (dopuna v1.43, 14.9.2026, na predlog vlasnika)

**Šta je vlasnik tražio:** _„da se uvede obaveštenje kada je preostalo još na primer 7 dana do isteka važenja neke cene, popusta za rani buking… pa da se ti hoteli marketinški obrade, pošalje obaveštenje subagentima ili se reklamiraju dodatno na društvenim mrežama."_ Odluke istog dana: pragovi **15 dana** (interno) i **7 dana** (marketing); obuhvaćena je **svaka vrsta akcije koja donosi nižu cenu**; ko od subagenata dobija obaveštenje zavisi od akcije („ako je važno za last minute onda ide svima"); AI pravi nacrt, **čovek odobrava**.

**Šta je postojalo do ove verzije:** datum do kog akcija važi već je u bazi (`PricelistOffer.booking_from/booking_to`, 2.4b; `booking_to` na svakoj cenovnoj stavci, 2.11e) i od v1.30 istekla cena se **ne prodaje**. Ali sistem to sazna tek kad je kasno — niko ne gleda unapred, pa akcija koja je mogla da proda još deset soba tiho istekne.

#### 4.9.1 Šta se smatra „akcijom koja donosi nižu cenu"

Posao gleda samo **aktivne** stavke (ne ugašene po 2.4c) sa popunjenim rokom, i to:

| Izvor                                           | Uslov                                                                                                                                                                                                                                                                                                                                              | Efektivan istek                                        |
| :---------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------- |
| `PricelistOffer`, `EARLY_BOOKING`               | uvek                                                                                                                                                                                                                                                                                                                                               | `min(booking_to, deposit_deadline)` (nenull od ta dva) |
| `PricelistOffer`, `FREE_NIGHTS`                 | uvek (6=5 je niža cena po noći)                                                                                                                                                                                                                                                                                                                    | isto                                                   |
| `AncillaryService`, `kind = DISCOUNT`           | ima `booking_to` (2.11e)                                                                                                                                                                                                                                                                                                                           | `booking_to`                                           |
| `RateLine` sa `booking_to` — „cena prve tranše" | za istu kombinaciju (period, `room_type`, `board`, `price_basis`, **`occupancy`, `valid_weekdays`** — v1.44) postoji **druga** aktivna `RateLine` sa **višom** cenom čiji `booking_from` je posle ovog `booking_to` ili je bez prozora — tj. posle isteka gost stvarno plaća više. Sama `RateLine` sa prozorom bez skuplje naslednice nije akcija. | `booking_to`                                           |

Stavka bez roka (`booking_to = null`) nikad ne ulazi — nema šta da istekne.

#### 4.9.2 Dva praga, jednom po stavci

Dnevni posao (isti obrazac kao samovraćanje blokada po `hold_until`, 2.8b, i dnevna tiha provera, 2.8e) za svaku stavku iz 4.9.1 računa `dana_do_isteka = efektivan_istek − danas` i:

| Prag        | Vrednost (podešljiva) | Kome                                                                                                       | Šta radi                                                                                                                                                                                |
| :---------- | :-------------------- | :--------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `INTERNAL`  | 15 dana               | ugovarač / operativa                                                                                       | red u radnom spisku kapaciteta (`/capacity/work-queue`, poglavlje 6; M17 §4b, stanje 1) — da se hotel pozove i pita da li produžava ili sledi 2. krug, **pre** nego što marketing krene |
| `MARKETING` | 7 dana                | M12 (nacrt objave, čovek odobrava — M12 §3d) i M7 (subagenti, M7 §5b) — oba se pretplaćuju na isti događaj | emituje se Event Bus događaj `pricelist.offer.expiring`                                                                                                                                 |

**Jednom po stavci po pragu, ne svaki dan.** Uvodi se `OfferExpiryNotice { id, source_type (PRICELIST_OFFER | ANCILLARY_SERVICE | RATE_LINE), source_id, threshold (INTERNAL | MARKETING), days_left_at_emit, emitted_at, acknowledged_by (nullable, FK → M1 User), acknowledged_at }` sa jedinstvenošću `(source_type, source_id, threshold)`. **Snimak stavke (v1.44):** zapis nosi i `contract_id`, `contract_period_id` (null kad popust pokriva više perioda), `product_id`, `product_name`, `destination_country/city`, `offer_kind`, `discount_summary`, `booking_to` (efektivan istek), `stay_from/to` — ono što red u radnom spisku ispisuje, bez čitanja tri različite tabele; izvor istine za cenu ostaje stavka. Posao emituje kad je `dana_do_isteka ≤ prag` **i** zapis za taj prag još ne postoji — dakle „manje ili jednako", ne „jednako": ako server jedan dan nije radio, obaveštenje ne sme da propadne. Kad je `dana_do_isteka < 0`, ne emituje se ništa — istekla akcija se ne reklamira. Pragovi su konfiguracija (`M3_OFFER_EXPIRY_DAYS_INTERNAL = 15`, `M3_OFFER_EXPIRY_DAYS_MARKETING = 7`), ne brojevi u kodu; ekran za njihovu izmenu se **ne pravi** u ovom prolazu (svesno — vlasnik je dao fiksne vrednosti, vidi poglavlje 8).

**Tri ograde pre emisije `MARKETING` praga** — reklamirati nešto što ne može da se proda je gore nego ne reklamirati:

1. proizvod na koji se ugovor odnosi je `ACTIVE` u M2 (poglavlje 3);
2. za period boravka postoji bar jedan dan sa `za_prodaju ≥ 1` (2.8c) ili je period `ON_REQUEST` — rasprodata akcija se ne najavljuje;
3. stavka nije ugašena i nije zamenjena (2.4c) — ako je hotel u međuvremenu produžio akciju, produžena je **nova** stavka sa sopstvenim rokom i sopstvenim obaveštenjem, stara je ugašena i tiho ispada.

`INTERNAL` prag nema ograde 1 i 2 — ugovarača zanima i akcija na proizvodu koji još nije objavljen.

**Zadržan `MARKETING` kandidat ne dobija zapis** — sutra se gleda ponovo (kapacitet se oslobodi, proizvod se objavi), pa obaveštenje stiže čim ograde propuste, dok god je `dana_do_isteka ≥ 0`. Posao ide u 6:00 (`M3DailyJobsService`, zajedno sa 2.8b samovraćanjem blokada) i može se pokrenuti ručno (`POST /pricelist/expiry-notices/run`, idempotentno).

#### 4.9.3 Šta nosi događaj

`pricelist.offer.expiring` nosi sve što primalac treba da bi napravio poruku **bez** dodatnog čitanja M3 baze (princip #2 master dokumenta): `source_type`, `source_id`, `contract_id`, `product_id`, `supplier_id`, naziv objekta i destinacija (iz M2 preko API-ja, u trenutku emisije), `offer_kind` (`EARLY_BOOKING`/`FREE_NIGHTS`/`DISCOUNT`/`FIRST_TRANCHE`), sažetak popusta u čitljivom obliku (npr. „−15 %", „7=6", „−20 € po sobi"), `booking_to` (efektivan istek), `days_left`, `stay_from`/`stay_to` perioda, i `has_subagent_allocations` (da li za taj proizvod postoji bar jedna M7 `SubagentCapacityAllocation` — M7 §5b to koristi da odredi publiku; **uvek `false` dok §5a ne dobije tabelu**, v1.44), plus `notice_id`.

**Poruka je vezana za `booking_to`, ne za boravak.** „Rezervišite do 30.9." je istinito; „još 7 dana" računato od datuma boravka je laž koja se lako potkrade u tekstu objave — zato događaj nosi oba datuma odvojeno i imenovano.

#### 4.9.4 Nivo autonomije

`INTERNAL` — **Autonomno** (čisto informativno, kao 4.3). `MARKETING` — M3 samo emituje činjenicu; šta se od nje pravi i ko to odobrava je posao M12 (nacrt, „AI predlaže, čovek odobrava") i M7. M3 sam nikad ne šalje ništa ni gostu ni subagentu ni dobavljaču.
...
| `/capacity/work-queue`                                       | GET                | početno stanje ekrana pre pretrage — samo ono što traži pažnju danas: prekoračeni kapaciteti, blokade kojima ističe rok, stop-sale koji se otvara, rokovi povrata, tipovi soba sa 0–2 preostale jedinice, **akcije kojima ističe rok** (`OfferExpiryNotice`, prag `INTERNAL`, još nepotvrđene — poglavlje 4.9)                                                                                                                                                                       |
| `/pricelist/expiry-notices`                                  | GET                | `OfferExpiryNotice` lista (poglavlje 4.9) — `?threshold=INTERNAL\|MARKETING&acknowledged=false`; zahteva `M3/contract/VIEW`                                                                                                                                                                                                                                                                                                                                                          |
| `/pricelist/expiry-notices/:id/acknowledge`                  | POST               | ugovarač potvrđuje da je video (`acknowledged_by/at`) — red nestaje iz radnog spiska; zahteva `M3/contract/EDIT`                                                                                                                                                                                                                                                                                                                                                                     |
| `/pricelist/expiry-notices/run`                              | POST               | ručno pokretanje dnevnog posla 4.9 (idempotentno — drugi poziv istog dana ne pravi drugi zapis); zahteva `M3/contract/EDIT`                                                                                                                                                                                                                                                                                                                                                          |
...
- [x] **Akcija pred istek — interno (4.9):** `EARLY_BOOKING` sa `booking_to` za 15 dana se pojavljuje u bloku „Akcije kojima ističe rok" na `/kapaciteti` (`GET /pricelist/expiry-notices?threshold=INTERNAL&acknowledged=false`; `/capacity/work-queue` kao celina još ne postoji, M17 §4b.0); posle `acknowledge` nestaje; posao pokrenut dvaput istog dana ne pravi drugi zapis. (`offer-expiry.service.spec.ts`; uživo 17.9.2026: run → 1 INTERNAL + 1 MARKETING, drugi run → 0.)
- [x] **Akcija pred istek — marketing (4.9):** na 7 dana emituje se `pricelist.offer.expiring` **tačno jednom**; za rasprodat period, neobjavljen proizvod ili ugašenu stavku se **ne** emituje; stavka istekla juče se ne emituje. (isti spec, 14 testova; uživo: M12 nacrt nastao iz događaja.)
- [x] **Cena prve tranše (4.9.1):** `RateLine` sa `booking_to` i skupljom naslednicom ulazi; ista bez naslednice ne ulazi; naslednica za drugu popunjenost ne računa se. (isti spec.)
...
- **Radni spisak kapaciteta kao celina (`/capacity/work-queue`, poglavlje 6; M17 §4b.0 stanje 1)** — 17.9.2026 postoji samo njegov prvi red (akcije pred istek, blok na vrhu `/kapaciteti`); prekoračenja, blokade kojima ističe rok, stop-sale koji se otvara, rokovi povrata i tipovi soba sa 0–2 jedinice još nemaju ni endpoint ni blok.
- **Ekran za pragove akcija pred istek** (poglavlje 4.9.2, 15 i 7 dana) — vrednosti su konfiguracija bez ekrana, jer je vlasnik 14.9.2026 dao fiksne brojeve. Ako se pokaže da se menjaju, pravi se ekran u `/podesavanja`, ne menja se kod.
- **Prag po vrsti akcije** (poglavlje 4.9) — pragovi su isti za rani buking (rok od meseci) i za popust koji traje tri dana; ako se uvede „last minute" akcija sa kratkim rokom, 7-dnevni prag joj ne pomaže — treba prag kao procenat trajanja ili poseban po `offer_kind`.
