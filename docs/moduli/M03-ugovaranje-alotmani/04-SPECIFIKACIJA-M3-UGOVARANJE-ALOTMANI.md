...
| `RateLine` sa `booking_to` — „cena prve tranše" | za istu kombinaciju (period, `room_type`, `board`, `price_basis`) postoji **druga** aktivna `RateLine` sa **višom** cenom čiji `booking_from` je posle ovog `booking_to` ili je bez prozora — tj. posle isteka gost stvarno plaća više. Sama `RateLine` sa prozorom bez skuplje naslednice nije akcija. | `booking_to`                                           |
...
`pricelist.offer.expiring` nosi sve što primalac treba da bi napravio poruku **bez** dodatnog čitanja M3 baze (princip #2 master dokumenta): `source_type`, `source_id`, `contract_id`, `product_id`, `supplier_id`, naziv objekta i destinacija (iz M2 preko API-ja, u trenutku emisije), `offer_kind` (`EARLY_BOOKING`/`FREE_NIGHTS`/`DISCOUNT`/`FIRST_TRANCHE`), sažetak popusta u čitljivom obliku (npr. „−15 %", „7=6", „−20 € po sobi"), `booking_to` (efektivan istek), `days_left`, `stay_from`/`stay_to` perioda, i `has_subagent_allocations` (da li za taj proizvod postoji bar jedna M7 `SubagentCapacityAllocation` — M7 §5b to koristi da odredi publiku).
...
- [ ] **Akcija pred istek — interno (4.9):** `EARLY_BOOKING` sa `booking_to` za 15 dana se pojavljuje u `/capacity/work-queue`; posle `acknowledge` nestaje; posao pokrenut dvaput istog dana ne pravi drugi zapis.
- [ ] **Akcija pred istek — marketing (4.9):** na 7 dana emituje se `pricelist.offer.expiring` **tačno jednom**; za rasprodat period, neobjavljen proizvod ili ugašenu stavku se **ne** emituje; stavka istekla juče se ne emituje.
- [ ] **Cena prve tranše (4.9.1):** `RateLine` sa `booking_to` i skupljom naslednicom ulazi; ista bez naslednice ne ulazi.
