...
- Da li `PACKAGE` proizvodi (iz M2) mogu imati sopstveni ugovor u M3 nezavisno od komponenti koje ga čine, ili se uvek sastavljaju od već ugovorenih komponenti — otvoreno dok se ne dođe do stvarnog paket-aranžmana u praksi.
  - **Predlog rešenja (31.8.2026, na osnovu poređenja sa TCT-IMC Flight Package API principa — `docs/analize/38-ANALIZA-TCT-IMC-API.md` §5.1; čeka potvrdu vlasnika, dotiče M2/M5 granicu pa verovatno traži i `tt-architecture-core` prolaz pre nego što se smatra odlukom):**
    **`PACKAGE` nikad ne dobija sopstveni M3 `Contract`/`ContractPeriod`/`RateLine`. Cena paketa se uvek izvodi iz cena njegovih `included_products[]` (M2 poglavlje 2.3), izračunato iznova u trenutku pravljenja Ponude — isti princip koji M5 već primenjuje za `Itinerary → Quote` konverziju** (M5 spec poglavlje 3.0.3: "cena se ovde računa iznova, ne prepisuje se `preview_final_price`"). Razlog da se ovo predlaže kao pravilo, ne samo kao mogućnost: TT već ima DVA nezavisna "paket" mehanizma — M2 `Product.type = PACKAGE` (unapred kuriran, prodaje se kao jedan katalog proizvod) i M5 `Itinerary` (ad-hoc sastavljanje po gostu, M5 spec §3.0d.5 potvrđuje da je "Individualni paketi" već taj isti tok, ne nov tip). Ako bi kurirani M2 `PACKAGE` dobio sopstvenu M3 cenu, TT bi imao dva različita pravila cenovanja paketa u sistemu istovremeno — ovaj predlog ih svodi na jedno: **M3 nikad ne cenuje paket direktno, M5 uvek sabira cenu iz komponenti**, bez obzira da li je paket nastao kroz kurirani M2 katalog ili ad-hoc `Itinerary`.
    - **Stvaran rad ako se ovo prihvati, ne samo formalnost:** `QuoteItemBuilderService` (`apps/api/src/modules/m5-rezervacije/quotes/quote-item-builder.service.ts`) danas tretira `PACKAGE` identično kao `ACCOMMODATION` (`ROOM_BASED_TYPES = ['ACCOMMODATION', 'PACKAGE']`) — što pretpostavlja suprotno, da `PACKAGE` ima sopstvenu sobu/cenu za direktno građenje jedne stavke. Prihvatanje ovog predloga zahteva izmenu builder-a da za `PACKAGE` proizvod umesto jedne stavke gradi **po jednu `QuoteItem` za svaki `included_products[]` unos** (svaka sa sopstvenim `product_id`/`base_cost` iz odgovarajućeg M3/M4 izvora), grupisane pod istim `Quote`/paket referencom radi prikaza gostu kao jedne celine.
    - **Otvoreno unutar ovog predloga, ne rešeno ovde:** kako TT modeluje popust na nivou paketa (stvaran "paket je jeftiniji od zbira pojedinačnih komponenti" slučaj, čest u praksi) — predlog je da se to reši kroz **postojeći M5 `MarkupRule` mehanizam** (uslovno pravilo koje se primenjuje kad su stavke grupisane pod istim paket-proizvodom), ne kroz nov M3 entitet — ali tačan oblik tog pravila nije razrađen ovde, čeka posebnu odluku ako se osnovni princip prihvati.
    - **Usklađivanje sa već postojećim §3.0d.6 (M5 spec) — individualni naspram grupnih paketa.** Isto pravilo (paket = zbir sastojaka) važi za oba, razlika je samo u tome ŠTA su sastojci i KAD se biraju: kod individualnog paketa (M5 `Itinerary`) gost/agent bira sastojke uživo iz trenutno dostupnog tržišta (obično M4 živ API ili obična M3 `CONTRACTED` stavka); kod grupnog paketa (M5 §3.0d.6, `PACKAGE` sa `stay_from` ograničenim na termine iz `ContractPeriod` gde je `allotment_mode IN (FIXED, CHARTER)`) sastojci su već unapred spojeni jer ih je agencija kupila zajedno kao jedan `CHARTER`/`FIXED_LEASE` ugovor — ali i tu cena paketa i dalje dolazi od cena tih istih ugovorenih sastojaka (leta + smeštaja), ne od posebne pakovane cene na nivou `PACKAGE` proizvoda.
    - **Mešanje fiksnih (M3 `CONTRACTED`) i dinamičkih (M4 API) sastojaka u istom paketu — potvrđeno, ne zahteva novu mehaniku.** Pošto se `PACKAGE` sastoji od `included_products[]`, a svaki uključeni proizvod već nezavisno nosi sopstveni `source_type` (`CONTRACTED` ili `API`, isto polje koje `QuoteItem`/`BookingItem` već koriste za ne-paket ponude, M5 spec poglavlje 3.0f/4.2), sastavljanje jedne `Quote` sa mešavinom fiksnih i dinamičkih stavki je već ono što M5 danas radi za obične (ne-paket) ponude. Prihvatanje osnovnog principa ovog predloga automatski nasleđuje ovu sposobnost za `PACKAGE` — ne treba poseban mehanizam "mešoviti paket", to je prirodna posledica pravila "paket = zbir nezavisno cenjenih sastojaka".
...
- **Poređenje sa industrijskom praksom (31.8.2026, istraživanje na zahtev vlasnika)** — pre poređenja, bitna razlika: postoje dva različita problema koji se oba zovu "unos cene hotela". *Sell-side distribucija* (hotel/PMS → OTA) ima usvojen standard (OTA ARI, `OTA_HotelRateAmountNotifRQ`) jer hotel ima razlog da se standardizuje — isti feed ide na desetine OTA kanala. *Buy-side ugovaranje* (agencija ← dobavljač, ono što M3 rešava) **nema** široko usvojen standard — svaki dobavljač-agencija ugovor je pojedinačan pregovor, bez mrežnog efekta koji bi gurnuo format. To objašnjava zašto je AI-ekstrakcija iz neurednih PDF/Excel dokumenata norma u ovoj niši, ne izuzetak — M3 nije odstupio od prakse, ušao je u prostor gde standarda nema.
