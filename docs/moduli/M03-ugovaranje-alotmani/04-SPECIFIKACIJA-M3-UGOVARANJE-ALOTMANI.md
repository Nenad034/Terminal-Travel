...
**Napomena o obimu ovog prolaza:** implementirano je polje modela + rezolucija u M5 obračunu cene (`apps/api/src/modules/m5-rezervacije/common/occupancy.ts`, `computeRoomBaseCost`). **Panel ekran za ručan unos ovog polja pri kreiranju cenovnika ne postoji još** (M3 nema poseban ekran za pojedinačan ugovor/period uopšte, vidi backlog `docs/analize/27-BACKLOG-IDEJA-I-PREDLOZI.md` M3 sekcija) — ovaj prolaz omogućava da polje POSTOJI i da se PRIMENI kad je popunjeno (preko API-ja ili budućeg AI uvoza cenovnika, poglavlje 4.2), unos kroz panel čeka taj ekran.
