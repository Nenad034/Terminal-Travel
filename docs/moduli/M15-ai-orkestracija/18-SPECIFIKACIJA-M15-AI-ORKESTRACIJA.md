...
**Napomena (v1.9):** stavke ispod označene kao potvrđene odnose se na omnisearch prvi prolaz (M17 kanal), ne na pun M15 Faza 7 okvir — vidi napomenu u poglavlju 6.5 o zasebnom `ModuleAgentActivation` gate-u.
...
- [ ] Pokušaj AI agenta da izvrši akciju klasifikovanu kao `NEVER_AUTONOMOUS` se odbija na nivou koda, čak i ako bi M1 dozvola to teorijski dozvolila. **Čeka poseban prolaz** — registar iz poglavlja 4 nije seedovan van omnisearch-a u ovom prolazu, pa nema `NEVER_AUTONOMOUS` red da se testira.
- [ ] Agent Inbox ispravno prikazuje sve stavke na čekanju iz svih modula koji ih trenutno proizvode. **Čeka poseban prolaz** — `GET /inbox` nije implementiran u ovom prolazu (van eksplicitnog obima zadatka).
...
- [ ] Registar akcija (`AgentActionType`) sadrži sve akcije nabrojane u poglavlju 4 ovog dokumenta, sa tačnim nivoom. **Delimično** — tabela ima pun oblik (šema spremna), ali seedovan je samo `omnisearch.query`; ostatak registra dolazi sa svakim modulom kad dobije domenskog agenta.
...
- [ ] `POST /omnisearch` iz M7/M8 konteksta nikad ne vraća identitet dobavljača. **Čeka poseban prolaz** — M7/M8 kanali nisu u obimu (samo M17 ovaj prolaz, poglavlje 6.5.5).
