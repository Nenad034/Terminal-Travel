...
**Verzija:** 1.8 — §3.1b, nova "compose" sposobnost (18.9.2026, na zahtev vlasnika: "ai agentu omoguci da moze da salje mejlove iz aplikacije") — `EmailThreadsService.composeNewThread()` + `POST /email/threads/compose`, zatvara otvorenu stavku od 23.8.2026 ("napiši i pošalji nov mejl proizvoljnom primaocu"). Prvi stvaran potrošač je M15 §6.5.4.8 (`compose_email` alat, predlog-pa-odobrenje preko `pendingEmailDraft`) — AI put uvek `send:false` (samo nacrt, `senderType:'AI_DRAFT'`, isti oblik kao postojeći nacrt odgovora), stvarno slanje i dalje ide preko postojećeg `sendDraft()`. Ručan ljudski compose (`send:true` direktno u M22 ekranu) je metod tehnički podržava, UI za njega namerno nije napravljen u ovom prolazu (poglavlje 10).
**Verzija:** 1.7 — `EmailMessage.delivered_at` i `SendResult.delivered` (§2.4), `SmtpEmailProviderAdapter` sa izborom po sandučetu preko `provider_connection_ref` (§10). Rešava nalaz 1.2 iz `39-REVIZIJA-KODA-NALAZI-I-PREDLOZI.md`: poruka se više ne beleži kao poslata kad isporuke nije bilo; mock adapter sada vraća `delivered: false` umesto izmišljenog identifikatora. Slanje se lokalno može stvarno videti (mailpit, `http://localhost:8025`). Dovlačenje pristigle pošte (`fetchNewMessages`) i izbor pravog provajdera ostaju otvoreni (§10).
**Verzija:** 1.6 — Podsetnik u poglavlju 10 (29.8.2026, na zahtev vlasnika): `EmailMessage` nema pojam priloga (preduslov za bilo koji scenario gde je prilog bitan), i predlog dedikovanog sandučeta za ulazne račune dobavljača (novo sanduče, npr. `racuni@`) čiji prilozi automatski postaju kandidati za M10 `SupplierInvoiceImport` (§8.6) — zatvara pitanje "možemo li da prosledimo mejl umesto da štampamo/skeniramo/uploadujemo". Bez izmene koda, čist podsetnik za sledeći prolaz na ovom modulu.
...
### 3.1b Compose — sastavljanje novog razgovora sa proizvoljnim primaocem (dopuna, 18.9.2026, na zahtev vlasnika — "ai agentu omoguci da moze da salje mejlove iz aplikacije")

**Nedostatak koji ovo rešava.** Do danas je M22 umeo isključivo da ODGOVORI unutar POSTOJEĆE niti (poglavlje 4, `EmailThreadsService.createMessage()`/`sendDraft()`) — nema "napiši nov mejl proizvoljnom primaocu koji do sad nije pisao nama". Ova dopuna dodaje TAČNO tu jednu sposobnost, ne menja ništa u postojećem odgovaranju.

**`EmailThreadsService.composeNewThread(dto, actorUserId, { send })`:**

1. Isti dvostruki gate kao svaki drugi M22 endpoint (poglavlje 7): `M22/email-thread/REPLY` (grubo, na nivou kontrolera) + `MailboxAccess.accessLevel = REPLY` za TAČNO `dto.mailboxId` (fino, `EmailThreadsService.requireAccess`) — sastavljanje mejla zahteva isti nivo pristupa kao odgovaranje, ne novu, širu dozvolu.
2. Upisuje novu `EmailThread` (`mailboxId`, `subject`, `correspondentType: dto.correspondentType ?? 'OTHER'` — proizvoljan spoljni primalac po pravilu nema unapred poznat tip u sistemu, poglavlje 3.1 pravilo tačnog poklapanja i dalje važi za SVAKU sledeću dolaznu poruku u toj niti, samo prva odlazna poruka nema šta da poklopi).
3. Upisuje prvu `EmailMessage` (`direction: OUTBOUND`, `toAddresses: dto.toAddresses`, `body: dto.body`):
   - `send: true` (ljudski, ručan unos direktno u M22 ekranu) → `senderType: 'STAFF'`, `sentBy: actorUserId` odmah, poziva `providerFactory.getAdapter(mailbox).sendMessage(mailbox, {toAddresses, subject, body})` **sa STVARNIM `toAddresses`** (za razliku od `createMessage()` odgovora, koji ih ostavlja praznim jer se oslanja na `In-Reply-To`/`References` zaglavlja postojeće niti — ovde nema prethodne poruke na koju se nadovezati).
   - `send: false` (AI predlog, isključivo poziva ga M15 `compose_email` tok, §6.5.4.8 te specifikacije, NIKAD direktno korisnik) → `senderType: 'AI_DRAFT'`, `sentBy: null` — **identičan oblik kao postojeći AI nacrt ODGOVORA** (poglavlje 4), ništa se ne šalje. Isto, već postojeće `sendDraft()` dugme "pošalji" (poglavlje 4, `POST /email/threads/:id/messages/:messageId/send`) kasnije šalje ovaj nacrt — nema novog endpoint-a za slanje, ponovo se koristi postojeći.
4. Audit log: `email_message.sent` ili `email_message.drafted` (isto ime akcije kao `createMessage()` — ovo je isti tip radnje, samo bez prethodne niti, ne nova kategorija za audit izveštaje).

**Namerno izostavljeno u ovom prolazu:** dugme "Novi mejl" u M22 ekranu za korak 1 iznad (ljudski ručan `send: true` put) — metod ga tehnički već podržava, ali ekran/forma nije napravljena jer je vlasnikov zahtev bio o AI agentu, ne o ručnom unosu. Otvorena stavka, poglavlje 10.
...
| `/threads/compose`                        | POST       | novi razgovor proizvoljnom primaocu (poglavlje 3.1b, 18.9.2026) — zahteva `REPLY` za `mailboxId` iz tela zahteva; `send:false` (AI predlog preko M15 §6.5.4.8) daje nacrt, `send:true` (ručan unos) šalje odmah                                                      |
...
- **`EmailMessage` nema pojam priloga** (podsetnik vlasnika, 29.8.2026, povodom pitanja "možemo li samo da prosledimo mejl kad nam račun stigne, bez štampanja/skeniranja/uploada") — model poruke (poglavlje 2.4) danas čuva samo `body` (tekst), nijedno polje za priložene fajlove (PDF/Excel/slika). Ovo je preduslov za dole opisan predlog, ne samo za njega — bilo koji budući scenario gde je prilog bitan (ugovor prosleđen mejlom, slika oštećenja od gosta i sl.) danas nema gde da se sačuva. Predlog: nov `EmailAttachment` zapis (`id`, `message_id` FK, `filename`, `content_type`, `file_url` — isto EU cloud skladište kao ostali dokumenti u sistemu, `size_bytes`), niz na `EmailMessage`.
- **Dedikovano sanduče za ulazne račune dobavljača, automatski uvoz u M10** (isto pitanje, 29.8.2026; vlasnik potvrdio: **novo, posebno sanduče** — npr. `racuni@`/`fakture@terminaltravel.rs`, ne prošireno postojeće sanduče za najave rezervacija iz poglavlja 6/M5 poglavlje 8.8, da se ne mešaju dva različita toka istog dobavljača) — kad `EmailAttachment` iznad postoji, pravilo je jednostavno i deterministički: svaki prilog koji stigne na TO KONKRETNO sanduče (`Mailbox.mailbox_type = SHARED`, posebno obeležen, npr. novo polje `Mailbox.purpose` ili samo prepoznavanje po adresi) automatski postaje kandidat za M10 `SupplierInvoiceImport` (M10 spec §8.6) sa `source_format` određenim po ekstenziji/tipu fajla (PDF/EXCEL/WORD/`SCANNED_PDF` za skeniran PDF) — isti nivo autonomije koji M10 već predviđa (ekstrakcija "Autonomno", upis u obavezu tek posle ljudske potvrde, §8.6.4). M22 ovde samo prosleđuje fajl dalje (isti princip kao poglavlje 6 — "M22 je transportni sloj, ne zamenjuje entitet drugog modula"), ne dodaje sopstvenu poslovnu logiku. Otvoreno pre spec-a: da li se `Mailbox` prepoznaje po fiksnoj listi adresa (jednostavnije, dovoljno za jedno sanduče) ili dobija eksplicitno polje namene; da li mejl bez ijednog priloga na ovom sandučetu treba upozorenje timu ("stiglo je nešto na sanduče za račune bez fajla") ili se tiho ignoriše kao obična prepiska.
- **Izbor konkretnog email provajdera/API-ja za SANDUČAD** (Gmail API, Microsoft Graph/Outlook, generički IMAP/SMTP) — ovaj dokument definiše samo generički `EmailProviderAdapter` (isti obrazac kao `ProviderAdapter` M4 / `PaymentGatewayAdapter` M10), tako da izbor provajdera ne zahteva izmenu ostatka modula. Bira se pri implementaciji.

  **Razgraničenje (dopuna 4.9.2026):** od tog datuma sistem **ume da šalje poštu** — uveden je `nodemailer`/SMTP i zajednički `MailerService` (`apps/api/src/common/mail`, master dokument poglavlje 6), kojim M1 šalje pozivnicu i reset lozinke a M18 uzbune. To **ne zatvara** ovu stavku: sistemska pošta ide sa JEDNE adrese pošiljaoca (`MAIL_FROM`), dok M22 traži slanje **u ime pojedinačnog sandučeta** preko konekcije tog sandučeta (`Mailbox.provider_connection_ref`, §2.1) i — što SMTP uopšte ne radi — **dovlačenje** pristigle pošte (`fetchNewMessages`). Zato `MockEmailProviderAdapter` ostaje jedina implementacija ovog interfejsa; zamena je i dalje jedna nova klasa, ostatak modula se ne menja.

  **Dopuna 5.9.2026 — SMTP adapter za slanje, izbor provajdera i dalje otvoren.** Dodat je `SmtpEmailProviderAdapter`: šalje **u ime sandučeta** (`from` = `Mailbox.address`) preko SMTP konekcije, čime je slanje iz M22 prestalo da bude pretvaranje. Bira se po sandučetu — `Mailbox.provider_connection_ref = "smtp:env"` znači „koristi SMTP podešavanje okruženja"; svako drugo (ili nepodešen `SMTP_HOST`) i dalje dobija mock. Time je zatvorena **polovina** ove stavke:

  - **slanje** — rešeno za SMTP put; lokalno ide u `mailpit` (docker-compose, SMTP 1025, pregled na `http://localhost:8025`), pa se svaka poslata poruka može pročitati bez ijednog spoljnog naloga i bez ijednog stvarnog mejla ka dobavljaču;
