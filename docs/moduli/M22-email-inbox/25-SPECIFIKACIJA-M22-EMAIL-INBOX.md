...
| `/threads` | GET | lista niti, filtrirano po `mailbox_id`/`status`/`correspondent_type` — samo sandučad na koja korisnik ima pristup |
| `/threads/:id` | GET | detalji sa svim `EmailMessage` porukama |
