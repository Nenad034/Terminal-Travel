...
**Prošireno "Bez modela" — puna provera celog M15 registra (dopuna 18.8.2026, na zahtev vlasnika).** Iznad navedena tri primera nisu bila iscrpna lista — pri ponovnom pregledu **svake** `AUTONOMOUS` stavke iz M15 registra (poglavlje 4 tog dokumenta) protiv teksta njene izvorne specifikacije, još sedam akcija je već opisano kao deterministička obrada, ne slobodna jezička generacija — samo to do sada nije bilo eksplicitno skupljeno na jedno mesto radi implementacije:
