...
- **`IN_APP` isporuka** — enum vrednost postoji, dispečer prepoznaje kanal ali ne isporučuje ništa — čeka M19 (interni real-time chat), koji tek treba da bude implementiran.
- **`HELP_AGENT_ABUSE_PATTERN` detekcija** — izvor je M21 (Centar za pomoć), koji još ne postoji u kodu; signal tip je rezervisan u enumu, bez funkcionalnog detektora.
...
- **M17 panel ekran za M18** — ovaj prolaz je isključivo backend (`/api/v1/ops`); interni panel prikaz (upozorenja/kanali/kvote) je poseban naredni korak, isti obrazac kao ostale M17 faze.
