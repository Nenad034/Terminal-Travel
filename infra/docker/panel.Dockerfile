# apps/panel (Next.js) — pakovanje za server.
#
...
#
# Gradi se IZ KORENA repozitorijuma (`docker build -f infra/docker/panel.Dockerfile .`).
...
COPY --from=build --chown=node:node /repo/apps/panel/.next/standalone ./
COPY --from=build --chown=node:node /repo/apps/panel/.next/static ./apps/panel/.next/static
COPY --from=build --chown=node:node /repo/apps/panel/public ./apps/panel/public
EXPOSE 3000
