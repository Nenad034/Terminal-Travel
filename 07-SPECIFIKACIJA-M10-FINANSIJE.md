...
- [ ] Uvoz ulazne fakture dobavljača (`SupplierInvoiceImport`) ispravno ekstrahuje redove i predlaže mapiranje ka `SupplierObligation` sa `match_confidence ≥ 85%` bez ljudske intervencije; ispod praga red ostaje `PENDING` za ručno mapiranje (poglavlje 8.6.3).
- [ ] Potvrda reda (`CONFIRMED`/`MANUALLY_MATCHED`) ispravno upisuje `invoice_reference` i po potrebi koriguje `amount_original`/`amount_rsd_at_invoice` na ciljanom `SupplierObligation`, samo uz ljudsku potvrdu (`M10/supplier-invoice-import/REVIEW`) — nijedan `SupplierObligation` se ne menja automatski iz `READY_FOR_REVIEW` statusa (poglavlje 8.6.4).
