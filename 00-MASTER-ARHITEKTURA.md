...
3. **Spoljni provajderi su adapteri, ne temelj.** Travelgate, SEF, ESIR, eTurista, budući avio/GDS API — svaki od njih je adapter iza internog, provajder-nezavisnog modela podataka. Menjanje ili dodavanje provajdera nikad ne sme zahtevati izmenu poslovne logike ili korisničkog interfejsa (razrađeno u poglavlju 5 i u ranije pripremljenom Travelgate predlogu).  
...
| M4 | **Integracije spoljnih API konekcija** | Sloj adaptera ka spoljnim provajderima (Travelgate za hotele, budući avio/GDS, transferi, aktivnosti). Detaljno razrađeno u ranijem Travelgate predlogu, koji se ovde tretira kao specifikacija ovog modula. | M1, M2 |
| M5 | **Rezervacije i tok prodaje** | Search → Ponuda → Potvrda → Upravljanje rezervacijom, bez obzira da li proizvod dolazi iz M3 ili M4. | M1, M2, M3, M4 |
...
| M8 | **Sajt agencije (B2C prikaz)** | Javni web prikaz kataloga i tok rezervacije za krajnje goste; back office isključivo u M2/M5. | M1, M2, M5, M6, M10, M20 *(M6/M10 dodati pri specifikaciji M8 — nalog gosta i kartično plaćanje; M20 dodat naknadno — prihvatanje ugovora pre plaćanja)* |
...
| M11 | **Regulatorni modul (Compliance)** | eTurista/CIS prijava gostiju, boravišna taksa, garancija putovanja (YUTA), evidencije za inspekciju. Namerno odvojen od M10 jer ima svoje rokove i API-je. | M1, M5, M6 |
...
| M14 | **Podrška / Helpdesk** | Tiketing za goste i subagente. | M1, M5, M6, M7 |
...
| M19 | **Komunikaciona platforma** | Interni real-time tim-chat (zaposleni ↔ zaposleni) — jedina zaista nova komunikaciona celina; komunikacija sa gostima/subagentima ostaje u M14/M6, samo dobija chat-stil prikaz. Desktop iskustvo kroz M17 (PWA), mobilno kroz novi tab u M9 — ne grade se nove samostalne aplikacije. Dodat naknadno (jul 2026). | M1, M14, M17, M9, M18 |
...
| M21 | **Centar za pomoć (baza znanja + AI asistent)** | Uputstvo za korišćenje same platforme (ne za putovanje) — za interni tim (kanal M17) i B2B subagente (kanal M7), sa AI asistentom koji odgovara isključivo iz objavljenog sadržaja, eskalira ka M14 kad ne zna, i predlaže nove članke na osnovu stvarnih pitanja koja ostanu bez odgovora. Dodat naknadno (avgust 2026), na zahtev vlasnika. | M1, M14, M15, M7, M17 |

Napomena: M15, M16, M17, M18, M19, M20 i M21 nisu greška u numeraciji — namerno su izdvojeni jer njihovo uvođenje prati sazrevanje ostalih modula, ne ide paralelno sa njihovom izgradnjom. M16 dodatno zavisi od toga da M2/M5 imaju čist, dokumentovan API (princip \#2 i \#3 iz poglavlja 3\) — ako taj API nije čist, M16 postaje mnogo skuplji projekat nego što bi trebalo da bude. M17, za razliku od M15/M16, mora postojati već od Faze 0/1 (poglavlje 8) — kasni broj je posledica toga kad je formalno imenovan, ne toga kad se gradi. M18 i M19 su naknadno dodati (jul 2026) na zahtev vlasnika — deo M18 funkcija (praćenje signala, obaveštenja) je nezavisan od M15 i može se graditi čim postoji dovoljno modula da se prati (praktično od Faze 2/3), dok trend-agent i model-tiering deo M18-a, kao i M19 u celosti, zahtevaju da M15 i M14/M9 već postoje. M20 je, kao i M17, primer kasnog broja sa ranom fazom (Faza 2) — naknadno formalizovan (jul 2026, poređenjem sa ranijim paralelnim dokumentom projekta) modul koji je zakonski neophodan od početka produkcijskog rada, ne od početka razvoja. M21 je poprečan kao M17/M18/M19 — sadržaj za tim može početi od Faze 0 (raste sa svakim modulom, isti obrazac kao M17), dok sadržaj za subagente prirodno čeka M7 (Faza 4); AI asistent deo zahteva da M15 okvir postoji.
...
## 5\. Referentna arhitektura sistema
                    ...
                    │  Travelgate | SEF | ESIR | eTurista | YUTA   │
...
| **Faza 2 — Zakonska usklađenost** | M10 (Finansije: SEF/ESIR integracija, PDV po sistemu marže, obaveze prema dobavljačima), M11 (Compliance: eTurista/CIS, boravišna taksa, garancija putovanja), M20 (Ugovori sa klijentima). | Svaka rezervacija iz Faze 1 može zakonito da se fakturiše, fiskalizuje, prijavi nadležnim sistemima i prati kroz zakonski obavezan ugovor sa gostom — bez ovoga agencija ne sme da posluje produkcijski, bez obzira koliko su drugi moduli razvijeni. |
...
- Usklađenost sa rokovima: SEF/e-fakture, ESIR fiskalizacija, eTurista prijava/odjava i boravišna taksa (do 5\. u mesecu za prethodni mesec), godišnje obnavljanje garancije putovanja kod YUTA.  
